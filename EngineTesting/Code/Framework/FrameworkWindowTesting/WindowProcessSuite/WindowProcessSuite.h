///	Copyright (c) 2010-2023
///	Threading Core Render Engine
///
///	作者：彭武阳，彭晔恩，彭晔泽
///	联系作者：94458936@qq.com
///
///	标准：std:c++20
///	引擎测试版本：0.9.0.12 (2023/06/13 23:08)

#ifndef FRAMEWORK_WINDOW_TESTING_WINDOW_PROCESS_SUITE_H
#define FRAMEWORK_WINDOW_TESTING_WINDOW_PROCESS_SUITE_H

#include "WindowMessageInterfaceTesting.h"
#include "WindowMessageTesting.h"
#include "WindowProcessHandleTesting.h"
#include "WindowProcessManagerTesting.h"

#endif  // FRAMEWORK_WINDOW_TESTING_WINDOW_PROCESS_SUITE_H