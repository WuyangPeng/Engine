...
namespace Physics
{
	...
	Fluid2Db<Real>
		::Fluid2Db(Real x0, Real y0, Real x1, Real y1, Real dt,
	...
	void Fluid2Db<Real>::DoSimulationStep()
	{
		...
		for (j = 1; j < mJMax; ++j)
		{
			for (i = 1; i < mIMax; ++i)
			{
				...
				Vector2D velAdvect = b0*(a0*v00 + a1*v10) + b1*(a0*v01 + a1*v11);
				...
				Vector2D twoVel00 = ((Real)2)*mVelocity0[j][i];
				Vector2D velXX = mVelocity0[j][i + 1] - twoVel00 + mVelocity0[j][i - 1];
				Vector2D velYY = mVelocity0[j + 1][i] - twoVel00 + mVelocity0[j - 1][i];
				Vector2D velSource = SourceVelocity(mTime, mX[i], mY[j],i, j);
	...
	template <typename Real>
	Real Fluid2Db<Real>::GetX0() const
	{
		return mX0;
	}

	template <typename Real>
	Real Fluid2Db<Real>::GetY0() const
	{
		return mY0;
	}

	template <typename Real>
	Real Fluid2Db<Real>::GetX1() const
	{
		return mX1;
	}

	template <typename Real>
	Real Fluid2Db<Real>::GetY1() const
	{
		return mY1;
	}

	template <typename Real>
	Real Fluid2Db<Real>::GetDt() const
	{
		return mDt;
	}

	template <typename Real>
	Real Fluid2Db<Real>::GetDx() const
	{
		return mDx;
	}

	template <typename Real>
	Real Fluid2Db<Real>::GetDy() const
	{
		return mDy;
	}

	template <typename Real>
	Real Fluid2Db<Real>::GetTime() const
	{
		return mTime;
	}

	template <typename Real>
	int Fluid2Db<Real>::GetIMax() const
	{
		return mIMax;
	}

	template <typename Real>
	int Fluid2Db<Real>::GetJMax() const
	{
		return mJMax;
	}

	template <typename Real>
	const Real* Fluid2Db<Real>::GetX() const
	{
		return mX;
	}

	template <typename Real>
	const Real* Fluid2Db<Real>::GetY() const
	{
		return mY;
	}

	template <typename Real>
	Real** Fluid2Db<Real>::GetDensity() const
	{
		return mDensity0;
	}

	template <typename Real>
	Mathematics::Vector2D<Real>** Fluid2Db<Real>::GetVelocity() const
	{
		return mVelocity0;
	}
