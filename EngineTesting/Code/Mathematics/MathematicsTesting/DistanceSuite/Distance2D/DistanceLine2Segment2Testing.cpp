...
// 作者：彭武阳，彭晔恩，彭晔泽
// 
// 引擎测试版本：0.0.0.2 (2019/08/30 11:26)
...
UNIT_TEST_SUBCLASS_COMPLETE_DEFINE(Mathematics, DistanceLine2Segment2Testing) 

void Mathematics::DistanceLine2Segment2Testing
	::MainTest()
{	
	...
	::BaseTest()
{
	...
	for (auto loop = 0; loop < testLoopCount; ++loop)
	{
		...
		FloatDistanceLine2Segment2 distance(FloatLine2(lhsOrigin, lhsDirection), FloatSegment2(rhsExtent, rhsOrigin, rhsDirection, 1e-5f));
	...
	::StaticTest()
{
	...
	for (auto loop = 0; loop < testLoopCount; ++loop)
	{
		...
		DoubleDistanceLine2Segment2 distance(DoubleLine2(lhsOrigin, lhsDirection), DoubleSegment2(rhsExtent, rhsOrigin, rhsDirection));
		...
		{
			// 最近点在lhs上某个点和rhs终点
			ASSERT_APPROXIMATE(squaredResult.GetDistance(), DoubleVector2DTools::VectorMagnitudeSquared(squaredResult.GetLhsClosestPoint() - distance.GetSegment().GetEndPoint()),1e-10);
			ASSERT_APPROXIMATE(squaredResult.GetContactTime(),0.0, 1e-10);
			...
			ASSERT_TRUE(squaredResult.GetDistance() <=  DoubleVector2DTools::VectorMagnitudeSquared(squaredResult.GetLhsClosestPoint() + 0.01 * lhsDirection - distance.GetSegment().GetEndPoint()));
			ASSERT_TRUE(squaredResult.GetDistance() <= DoubleVector2DTools::VectorMagnitudeSquared(squaredResult.GetLhsClosestPoint() - 0.01 * lhsDirection - distance.GetSegment().GetEndPoint()));
		}
		else
		{
			...
			ASSERT_APPROXIMATE(squaredResult.GetDistance(), DoubleVector2DTools::VectorMagnitudeSquared(squaredResult.GetLhsClosestPoint() - distance.GetSegment().GetBeginPoint()),1e-10);
			ASSERT_APPROXIMATE(squaredResult.GetContactTime(),0.0, 1e-10);
			...
			ASSERT_TRUE(squaredResult.GetDistance() <=  DoubleVector2DTools::VectorMagnitudeSquared(squaredResult.GetLhsClosestPoint() + 0.01 * lhsDirection - distance.GetSegment().GetBeginPoint()));
			ASSERT_TRUE(squaredResult.GetDistance() <= DoubleVector2DTools::VectorMagnitudeSquared(squaredResult.GetLhsClosestPoint() - 0.01 * lhsDirection - distance.GetSegment().GetBeginPoint()));
		}
		...
		DoubleDistanceLine2Segment2 firstParallelDistance(DoubleLine2(lhsOrigin, lhsDirection), DoubleSegment2(rhsExtent, rhsOrigin, lhsDirection));
		...
		double testDistanceSquared = DoubleVector2DTools::VectorMagnitudeSquared(firstParallelDistance.GetLine().GetOrigin() - firstParallelDistance.GetSegment().GetBeginPoint());
		ASSERT_TRUE(distanceSquared <= testDistanceSquared + 1e-10);

		testDistanceSquared = DoubleVector2DTools::VectorMagnitudeSquared(firstParallelDistance.GetLine().GetOrigin() - firstParallelDistance.GetSegment().GetEndPoint());
		ASSERT_TRUE(distanceSquared <= testDistanceSquared + 1e-10);

		testDistanceSquared = DoubleVector2DTools::VectorMagnitudeSquared(firstParallelDistance.GetLine().GetOrigin() - squaredResult.GetRhsClosestPoint());
		ASSERT_TRUE(distanceSquared <= testDistanceSquared + 1e-10);
		...
		DoubleDistanceLine2Segment2 secondParallelDistance(DoubleLine2(lhsOrigin, lhsDirection), DoubleSegment2(rhsExtent, rhsOrigin, lhsDirection));
		...
		testDistanceSquared = DoubleVector2DTools::VectorMagnitudeSquared(secondParallelDistance.GetLine().GetOrigin() - secondParallelDistance.GetSegment().GetBeginPoint());
		ASSERT_TRUE(distanceSquared <= testDistanceSquared + 1e-10);

		testDistanceSquared = DoubleVector2DTools::VectorMagnitudeSquared(secondParallelDistance.GetLine().GetOrigin() - secondParallelDistance.GetSegment().GetEndPoint());
		ASSERT_TRUE(distanceSquared <= testDistanceSquared + 1e-10);

		testDistanceSquared = DoubleVector2DTools::VectorMagnitudeSquared(secondParallelDistance.GetLine().GetOrigin() - squaredResult.GetRhsClosestPoint());
		ASSERT_TRUE(distanceSquared <= testDistanceSquared + 1e-10);
	...
	::DynamicTest()
{
	...
	for (auto loop = 0; loop < testLoopCount; ++loop)
	{
		...
		double distanceSquared = squaredResult.GetDistance();
		double testDistanceSquared = DoubleVector2DTools::VectorMagnitudeSquared(firstParallelDistance.GetLine().GetOrigin() + t * lhsVelocity -firstParallelDistance.GetSegment().GetBeginPoint() - t * rhsVelocity);
		ASSERT_TRUE(distanceSquared <= testDistanceSquared + 1e-10);

		testDistanceSquared = DoubleVector2DTools::VectorMagnitudeSquared(firstParallelDistance.GetLine().GetOrigin() + t * lhsVelocity - firstParallelDistance.GetSegment().GetEndPoint() - t * rhsVelocity);
		ASSERT_TRUE(distanceSquared <= testDistanceSquared + 1e-10);

		testDistanceSquared = DoubleVector2DTools::VectorMagnitudeSquared(firstParallelDistance.GetLine().GetOrigin() + t * lhsVelocity - squaredResult.GetRhsClosestPoint());
		ASSERT_TRUE(distanceSquared <= testDistanceSquared + 1e-10);
		...
		distanceSquared = squaredResult.GetDistance();
		testDistanceSquared = DoubleVector2DTools::VectorMagnitudeSquared(secondParallelDistance.GetLine().GetOrigin() + t * lhsVelocity - secondParallelDistance.GetSegment().GetBeginPoint() - t * rhsVelocity);
		ASSERT_TRUE(distanceSquared <= testDistanceSquared + 1e-10);

		testDistanceSquared = DoubleVector2DTools::VectorMagnitudeSquared(secondParallelDistance.GetLine().GetOrigin() + t * lhsVelocity - secondParallelDistance.GetSegment().GetEndPoint() - t * rhsVelocity);
		ASSERT_TRUE(distanceSquared <= testDistanceSquared + 1e-10);

		testDistanceSquared = DoubleVector2DTools::VectorMagnitudeSquared(secondParallelDistance.GetLine().GetOrigin() + t * lhsVelocity - squaredResult.GetRhsClosestPoint());
		ASSERT_TRUE(distanceSquared <= testDistanceSquared + 1e-10);
