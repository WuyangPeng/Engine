...
AncientBooks::ImperialCourtContainer::ConstImperialCourtSharedPtr AncientBooks::ImperialCourtContainer::GetImperialCourt(int key) const
{
    ...
    if (const auto iter = std::ranges::lower_bound(imperialCourt, std::make_shared<ImperialCourtBase>(key), function);
        iter != imperialCourt.cend() && (*iter)->GetKey() == key)
    {
        return *iter;
    }
    else
...
int AncientBooks::ImperialCourtContainer::GetContainerSize() const
{
    USER_CLASS_IS_VALID_CONST_9;

    return boost::numeric_cast<int>(imperialCourt.size());
}
