...
namespace Mathematics
{
    ...
    IntpAkimaUniform2<Real>::IntpAkimaUniform2(int xBound, int yBound, Real xMin, Real xSpacing, Real yMin, Real ySpacing, Real** F)
    {
        ...
        Real** xSlope = nullptr;  // NEW2<Real>(xBound + 3, yBound);  //xslope[y][x]
        Real** ySlope = nullptr;  // NEW2<Real>(yBound + 3, xBound);  //yslope[x][y]
        ...
        Real** FX = nullptr;  // NEW2<Real>(xBound, yBound);
        Real** FY = nullptr;  //  NEW2<Real>(xBound, yBound);
        ...
        Real** FXY = nullptr;  // NEW2<Real>(xBound, yBound);

        unsigned int ix0 = xBoundM1, ix1 = ix0 - 1, ix2 = ix1 - 1;
        unsigned int iy0 = yBoundM1, iy1 = iy0 - 1, iy2 = iy1 - 1;

        // corners
        FXY[0][0] = (Real{ 0.25 }) * invDXDY * (((Real)9) * F[0][0] - (static_cast<Real>(12)) * F[0][1] + (static_cast<Real>(3)) * F[0][2] - (static_cast<Real>(12)) * F[1][0] + (static_cast<Real>(16)) * F[1][1] - ((Real)4) * F[1][2] + (static_cast<Real>(3)) * F[2][0] - ((Real)4) * F[2][1] + F[2][2]);

        FXY[0][xBoundM1] = (Real{ 0.25 }) * invDXDY * (((Real)9) * F[0][ix0] - (static_cast<Real>(12)) * F[0][ix1] + (static_cast<Real>(3)) * F[0][ix2] - (static_cast<Real>(12)) * F[1][ix0] + (static_cast<Real>(16)) * F[1][ix1] - ((Real)4) * F[1][ix2] + (static_cast<Real>(3)) * F[2][ix0] - ((Real)4) * F[2][ix1] + F[2][ix2]);

        FXY[yBoundM1][0] = (Real{ 0.25 }) * invDXDY * (((Real)9) * F[iy0][0] - (static_cast<Real>(12)) * F[iy0][1] + (static_cast<Real>(3)) * F[iy0][2] - (static_cast<Real>(12)) * F[iy1][0] + (static_cast<Real>(16)) * F[iy1][1] - ((Real)4) * F[iy1][2] + (static_cast<Real>(3)) * F[iy2][0] - ((Real)4) * F[iy2][1] + F[iy2][2]);

        FXY[yBoundM1][xBoundM1] = (Real{ 0.25 }) * invDXDY * (((Real)9) * F[iy0][ix0] - (static_cast<Real>(12)) * F[iy0][ix1] + (static_cast<Real>(3)) * F[iy0][ix2] - (static_cast<Real>(12)) * F[iy1][ix0] + (static_cast<Real>(16)) * F[iy1][ix1] - ((Real)4) * F[iy1][ix2] + (static_cast<Real>(3)) * F[iy2][ix0] - ((Real)4) * F[iy2][ix1] + F[iy2][ix2]);

        // x-edges
        for (ix = 1; ix < xBoundM1; ++ix)
        {
            FXY[0][ix] = (Real{ 0.25 }) * invDXDY * ((static_cast<Real>(3)) * (F[0][ix - 1] - F[0][ix + 1]) - ((Real)4) * (F[1][ix - 1] - F[1][ix + 1]) + (F[2][ix - 1] - F[2][ix + 1]));

            FXY[yBoundM1][ix] = (Real{ 0.25 }) * invDXDY * ((static_cast<Real>(3)) * (F[iy0][ix - 1] - F[iy0][ix + 1]) - ((Real)4) * (F[iy1][ix - 1] - F[iy1][ix + 1]) + (F[iy2][ix - 1] - F[iy2][ix + 1]));
        }

        // y-edges
        for (iy = 1; iy < yBoundM1; ++iy)
        {
            FXY[iy][0] = (Real{ 0.25 }) * invDXDY * ((static_cast<Real>(3)) * (F[iy - 1][0] - F[iy + 1][0]) - ((Real)4) * (F[iy - 1][1] - F[iy + 1][1]) + (F[iy - 1][2] - F[iy + 1][2]));

            FXY[iy][xBoundM1] = (Real{ 0.25 }) * invDXDY * ((static_cast<Real>(3)) * (F[iy - 1][ix0] - F[iy + 1][ix0]) - ((Real)4) * (F[iy - 1][ix1] - F[iy + 1][ix1]) + (F[iy - 1][ix2] - F[iy + 1][ix2]));
        }
        ...
        mPoly = nullptr;  // NEW2<Polynomial>(xBoundM1, yBoundM1);
        for (iy = 0; iy < yBoundM1; ++iy)
        {
            for (ix = 0; ix < xBoundM1; ++ix)
            {
