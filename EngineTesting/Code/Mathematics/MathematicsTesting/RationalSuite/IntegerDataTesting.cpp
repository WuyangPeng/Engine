...
void Mathematics::IntegerDataTesting
	::MainTest()
{
	...
	::CopyTest()
{
	...
	IntegerData<3> firstIntegerData(data);
	...
	IntegerData<3> secondIntegerData(firstIntegerData);
	...
	IntegerData<3> thirdIntegerData;
	...
	IntegerData<8> fourthIntegerData(thirdIntegerData);

	for (auto i = 0u;i < data.size();++i)
	{
		ASSERT_EQUAL(thirdIntegerData[i],fourthIntegerData[i]);
	}

	for (auto i = boost::numeric_cast<int>(data.size());i < 8 * 2;++i)
	{
		ASSERT_EQUAL(fourthIntegerData[i],0u);
	}
}
