...
namespace ThreeDMathPrimerForGraphicsAndGameDevelopment
{
	...
	public:
		...
		float xSize() noexcept
		{
			return max.x - min.x;
		}
		float ySize() noexcept
		{
			return max.y - min.y;
		}
		float zSize() noexcept
		{
			return max.z - min.z;
		}
		...
		Vector3	corner(int i) const noexcept;
		...
		void add(const Vector3& p) noexcept;

		// Add an AABB to the box

		void add(const AABB3& box) noexcept;

		// Transform the box and compute the new AABB

		void setToTransformedBox(const AABB3& box, const Matrix4x3& m) noexcept;
		...
		Vector3	closestPointTo(const Vector3& p) const noexcept;
		...
		float rayIntersect(const Vector3& rayOrg, const Vector3& rayDelta, Vector3* returnNormal = 0) const noexcept;

		// Classify box as being on one side or the other of a plane

		int	classifyPlane(const Vector3& n, float d) const noexcept;

		// Dynamic intersection with plane

		float intersectPlane(const Vector3& n, float planeD, const Vector3& dir) const noexcept;
	};
	...
	bool intersectAABBs(const AABB3& box1, const AABB3& box2, AABB3* boxIntersect = 0) noexcept;
