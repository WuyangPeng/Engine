	...
	::Adjust()
{
	...
	IntegerDataAnalysis denominatorAnalysis{ m_AbsDenominator };
	m_DenominatorInit = denominatorAnalysis.GetLeadingBlock();
	auto leadingDigit = denominatorAnalysis.ToInt(m_DenominatorInit);
	...
	::Calculate(int quotientInit)
{
	...
	// 获取余数最先的三个索引
	uint32_t numerator0{ 0 };
	uint32_t numerator1{ 0 };
	uint32_t numerator2{ 0 };
	if (0 < m_RemainderHat)
	{
	...
	auto product = multiplication.GetMultiplication();
