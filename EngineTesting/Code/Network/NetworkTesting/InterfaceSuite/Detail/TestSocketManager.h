...
namespace Network
{
	...
	public:
		...
		int GetAsyncConnectCount() const;
		int GetAsyncReceiveCount() const;
		int GetAsyncAcceptorCount() const;
		int GetAsyncSendCount() const;

		void SetServerWeakPtr(const ServerSharedPtr& ptr);

	private:
		virtual void InitEvent(uint64_t socketID);
