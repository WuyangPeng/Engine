...
namespace System
{
	...
	{
		// 该函数分配大到足以容纳格式化的消息缓冲区，并在由buffer指定的地址的指针放置到分配的缓冲区。
		...
		// 如果这source句柄为nullptr，当前进程的应用程序的图像文件将被搜索。
		FromHModule = FORMAT_MESSAGE_FROM_HMODULE,

		// 搜索所请求的消息中的系统信息表资源。如果标志同时也指定了FORMAT_MESSAGE_FROM_HMODULE，
		// 那么函数会先在source指定的模块中搜索请求的消息，如果搜索不到，就去搜索系统消息表资源。
		FromSystem = FORMAT_MESSAGE_FROM_SYSTEM,

		// 参数arguments不是va_list的结构，arguments代表了它是一个指针数组的值。
		// 这个标志不能与64位整型值一起使用。
	...
	enum class FormatMessageWidth
	{
		...
		// 该函数将忽略消息定义文本中的常规换行符。该函数将消息定义文本中的硬编码换行符存储到输出缓冲区中。该函数不会生成新换行符。
