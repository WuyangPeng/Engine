...
Mathematics::CylinderFit3Update<Real>
	...
	:m_Points{ points }, m_InverseRadiusSqrare{ static_cast<Real>(1) },m_Axis{ guessAxis }, m_Center{ guessCenter },
...
void Mathematics::CylinderFit3Update<Real>
	::UpdateInverseRadiusSqrare()
{
	Real deltaCrossAxisLengthSquaredSum { };
	Real deltaCrossAxisLengthQuarticSum { };

	for (const auto& data: m_UpdateData)
	{
		Real deltaCrossAxisLengthQuartic = data.GetDeltaCrossAxisLengthQuartic();
	...
	MATHEMATICS_ASSERTION_2(Real{} <= m_Exactly && m_Exactly <= static_cast<Real>(1), "误差值错误！");
}
...
{
	if (Real{} <= m_InverseRadiusSqrare && Real{} <= m_Exactly && Real{} <= m_Epsilon &&  m_Axis.IsNormalize(m_Epsilon))
	{
		return true;
	}
	else
