...
Framework::ObjectMotionRotateTrackBall
	...
	:m_MotionObject(motionObject),
     m_Camera(camera), 
	 m_BeginXTrack(m_BeginXTrack),
	 m_BeginYTrack(m_BeginYTrack),
	 m_EndXTrack(m_EndXTrack),
	 m_EndYTrack(m_EndYTrack),
	 m_SaveRotate(m_SaveRotate),
	 m_LocalRotate()
{
	Calculate();
...
void Framework::ObjectMotionRotateTrackBall
	::Calculate()
{
	float beginX = m_BeginXTrack;
	float beginY = m_BeginYTrack;
	float endX = m_EndXTrack;
	float endY = m_EndYTrack;

	// 获得球面上的第一个向量。
    float length = Mathf::Sqrt(beginX * beginX + beginY * beginY); 
	float beginZ = 0.0f;
	float endZ = 0.0f;
    ...
    else  // 向量是平行的。
    {
        ...
        {       
			// π弧度旋转。
            float invLength = Mathf::InvSqrt(beginX * beginX + beginY * beginY);
	...
	// 它必须被转换成世界坐标。
	// 再一次,我用相机顺序(D,U,R)。
    AVector worldAxis =  axis[0] * m_Camera->GetDirectionVector() +
  		                 axis[1] * m_Camera->GetUpVector() +
						 axis[2] * m_Camera->GetRightVector();

    Matrix trackRotate(worldAxis, angle);

	// 计算出新的局部旋转。如果对象是场景的根,	
	// 新的旋转是轨迹球应用在对象被旧局部旋转的增量旋转。
	// 如果对象不是场景的根,你要转换的增量式旋转要在改变父坐标空间的基础上。
    const Spatial* parent = m_MotionObject->GetParent();
    if (parent)
    {
        const Matrix parWorRotate = parent->GetWorldTransform().GetRotate();
