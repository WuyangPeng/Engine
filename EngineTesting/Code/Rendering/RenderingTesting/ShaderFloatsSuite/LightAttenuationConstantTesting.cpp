...
// ◊˜’ﬂ£∫≈ÌŒ‰—Ù£¨≈ÌÍ ∂˜£¨≈ÌÍ ‘Û
// 
// “˝«Ê≤‚ ‘∞Ê±æ£∫0.0.0.3 (2019/09/06 15:22)

#include "LightAttenuationConstantTesting.h"
...
UNIT_TEST_SUBCLASS_COMPLETE_DEFINE(Rendering, LightAttenuationConstantTesting) 

void Rendering::LightAttenuationConstantTesting
	::MainTest()
{ 
	...
	::InitTest()
{
	...
	for (int loop = 0; loop < GetTestLoopCount(); ++loop)
	{
		...
		const LightAttenuationConstant secondShaderFloat(firstLight);
		...
		LightAttenuationConstant thirdShaderFloat(firstLight);
	...
	::CopyTest()
{
	...
	for (int loop = 0; loop < GetTestLoopCount(); ++loop)
	{
		...
		LightAttenuationConstant secondShaderFloat(firstShaderFloat);
...
// 		LightAttenuationConstantSharedPtr firstShaderFloat(new LightAttenuationConstant(firstLight));
// 		
// 		for (int i = 0; i < 4;++i)
// 		{	 
...
// 		
// 		LightAttenuationConstantSharedPtr secondShaderFloat =
// 			inTopLevel[0].PolymorphicDowncastObjectSharedPtr<LightAttenuationConstantSharedPtr>();
// 		
...
// 
// 		ASSERT_APPROXIMATE(firstLight->GetConstant(), firstShaderFloat[0], 1e-8f);
// 		ASSERT_APPROXIMATE(firstLight->GetLinear(), firstShaderFloat[1], 1e-8f);
// 		ASSERT_APPROXIMATE(firstLight->GetQuadratic(), firstShaderFloat[2], 1e-8f);
// 		ASSERT_APPROXIMATE(firstLight->GetIntensity(), firstShaderFloat[3], 1e-8f);
// 	}
}
