...
void Rendering::BoundImpl
    ::SetCenter (const APoint& center)
{
...
const Rendering::BoundImpl::APoint& Rendering::BoundImpl
    ::GetCenter () const
{
...
Rendering::BoundImpl::NumericalValueSymbol Rendering::BoundImpl
...
void Rendering::BoundImpl
    ::GrowToContain (const BoundImpl& bound)
{
	RENDERING_CLASS_IS_VALID_1;
	...
	auto radiusDifference = bound.m_Radius - m_Radius;
	auto radiusDifferenceSqruared = radiusDifference * radiusDifference;
...
const Rendering::BoundImpl Rendering::BoundImpl
...
void Rendering::BoundImpl
	::ComputeFromData(const vector<APoint>& data) 
{
	RENDERING_CLASS_IS_VALID_1;
...
void Rendering::BoundImpl
	::ComputeFromData(const std::vector<Vector3D>& data)
{
	RENDERING_CLASS_IS_VALID_1;
    ...
    ::TestIntersection (const APoint& origin, const AVector& direction, float tmin, float tmax) const
{
	...
	// 测试球-线段相交
	auto segmentExtent = 0.5f * (tmin + tmax);
	...
	auto tmp0 = segmentExtent * segmentExtent + a;
	auto tmp1 = 2.0f * dot * segmentExtent;
	auto qm = tmp0 - tmp1;
	auto qp = tmp0 + tmp1;
    if (qm * qp <= 0.0f)
    {
