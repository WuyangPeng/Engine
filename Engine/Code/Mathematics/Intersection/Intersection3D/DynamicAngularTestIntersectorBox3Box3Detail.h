...
// ◊˜’ﬂ£∫≈ÌŒ‰—Ù£¨≈ÌÍ ∂˜£¨≈ÌÍ ‘Û
// 
// “˝«Ê∞Ê±æ£∫0.0.0.2 (2019/07/13 10:55)
	...
	::Test ()
{
	...
	auto stepsize = this->GetTMax()/(Real)numSteps;
    ...
    for (auto istep = 1; istep <= numSteps; ++istep)
    {
		...
		auto subTime = stepsize*(Real)istep;
		...
		subBox0 = Box3{ subBox0.GetCenter(),vector3DOrthonormalize0.GetUVector(),
						...
						subBox0.GetFirstExtent(),subBox0.GetSecondExtent(),subBox0.GetThirdExtent() };
		...
		subBox1 = Box3{ subBox1.GetCenter(),vector3DOrthonormalize1.GetUVector(),
						...
						subBox1.GetFirstExtent(),subBox1.GetSecondExtent(),subBox1.GetThirdExtent() };
    }
