...
Mathematics::StaticTestIntersectorTriangle3Cone3<Real>
	::StaticTestIntersectorTriangle3Cone3(const Triangle3& triangle, const Cone3& cone, const Real epsilon)
	...
	::Test()
{
    ...
    int onConeSide = 0;
    Real p0Test = Math<Real>::GetValue(0), p1Test = Math<Real>::GetValue(0), p2Test = Math<Real>::GetValue(0);
        ...
        if (c2 < Math<Real>::GetValue(0))
        {
            ...
            if (onConeSide & 1)
            {
                if (onConeSide & 2)
                {
                    ...
                    if (Math<Real>::GetValue(0) <= c1 && c1 <= -c2 && c1*c1 >= p0Test*c2)
                    {
						this->SetIntersectionType(IntersectionType::Other);
						return;
                    }
                ...
                else
                {
                    ...
                    if (Math<Real>::GetValue(0) <= c1 && c2*AdD0 <= c1*AdE &&  c1*c1 >= p0Test*c2)
                    {
						this->SetIntersectionType(IntersectionType::Other);
						return;
                    }
    ...
    if (onConeSide > 0)
    {
        ...
        if (NdA >= Math<Real>::GetValue(0))
        {
            if (NcUdE0 <= Math<Real>::GetValue(0))
            {
				NcUdE1 = Vector3DTools::DotProduct(NcU,edge1);
                if (NcUdE1 >= Math<Real>::GetValue(0))
                {
