...
（3） 字符串拼接宏：用于在宏中拼接字符串。
（4） 未使用标志宏：用于消除未使用参数的警告。
（5） Unicode字符using声明：根据是否定义宏UNICODE来决定字符类型是char还是wchar_t。
（6） 标记过时函数宏：跨平台的宏来标记一个函数或类为过时。
（7） 打印编译器信息宏：打印正在使用的编译器信息。
（8） Windows平台相关宏：将Windows宏封装成constexpr函数或平台通用宏，以实现非Windows平台的调用。 
（9） 版本号：引擎的版本号。
（10）导出标志宏：根据不同平台的标准，定义类或函数的导入或导出符号。
（11）安全标识符宏：Windows平台安全标识符所需要的宏，主要用于安全标识符的初始化。
（12）网络宏：用于描述符fd的相关操作。
（13）gl扩展宏：保证opengl头文件的包含顺序，以及opengl函数和opengl宏的定义。
	  ...
	  opengl插件宏。
	  opengl工具宏。
（14）PragmaWarning：消除特定的编译器警告。
（15）平台头文件：用于包含平台相关的头文件。
（16）枚举的转换运算符：实现枚举和整型类型之间的转换。枚举转换成整型无需指定转换结果类型，其他转换需要指定结果类型。
（17）枚举宏：用于定义枚举所需的运算符。
（18）Noexcept：封装一个捕获所有异常的函数。
