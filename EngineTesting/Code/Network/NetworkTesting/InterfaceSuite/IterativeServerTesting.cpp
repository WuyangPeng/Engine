...
Network::IterativeServerTesting
	::IterativeServerTesting(const OStreamShared& osPtr)
...
void Network::IterativeServerTesting
	::MainTest()
{
	...
	::IterativeServerTest()
{
	...
	ConfigurationStrategy serverConfigurationStrategy{ WrappersStrategy::ACE, ConnectStrategy::TCP, ServerStrategy::Iterative,
													   ...
													   configurationSubStrategy, ConfigurationParameter{}, SocketSendMessage::Default, "127.0.0.1", m_Port + m_Increase };
...
void Network::IterativeServerTesting
	::ClientThread(ClientSharedPtr client, TestSocketManagerSharedPtr testSocketManagerSharedPtr)
{
	uint64_t socketID{ 0 };
	for (;;)
	{
	...
	for (;;)
	{
		...
		auto volatile asyncAcceptorCount = testSocketManagerSharedPtr->GetAsyncAcceptorCount();
