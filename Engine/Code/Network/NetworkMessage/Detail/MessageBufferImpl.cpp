...
Network::MessageBufferImpl
	::MessageBufferImpl(ParserStrategy parserStrategy) noexcept
...
Network::MessageBufferImpl& Network::MessageBufferImpl
	::operator=(const MessageBufferImpl& rhs) noexcept
{
...
Network::MessageBufferImpl& Network::MessageBufferImpl
	::operator=(MessageBufferImpl&& rhs) noexcept
{
	NETWORK_CLASS_IS_VALID_1;
...
Network::MessageBufferImpl
	::~MessageBufferImpl() noexcept
{
	NETWORK_SELF_CLASS_IS_VALID_1;
}

#ifdef OPEN_CLASS_INVARIANT
bool Network::MessageBufferImpl
...
int Network::MessageBufferImpl
	::GetCurrentReadIndex() const noexcept
{
...
void Network::MessageBufferImpl
...
void Network::MessageBufferImpl
	::AddCurrentReadIndex(int stepping)
{
	NETWORK_CLASS_IS_VALID_1;
	...
	{
		THROW_EXCEPTION(SYSTEM_TEXT("增加读索引越界。"));
	}
}

int Network::MessageBufferImpl
	::GetCurrentWriteIndex() const noexcept
{
	NETWORK_CLASS_IS_VALID_CONST_1;
...
void Network::MessageBufferImpl
	::AddCurrentWriteIndex(int stepping)
{
	NETWORK_CLASS_IS_VALID_1;
	...
	{
		THROW_EXCEPTION(SYSTEM_TEXT("增加写索引越界。"));
	}
}

const char* Network::MessageBufferImpl
	::GetCurrentReadBufferedPtr() const
{
	NETWORK_CLASS_IS_VALID_CONST_1;
...
char* Network::MessageBufferImpl
	::GetCurrentReadBufferedPtr()
{
	NETWORK_CLASS_IS_VALID_1;
...
const char* Network::MessageBufferImpl
	::GetCurrentWriteBufferedPtr() const
{
	NETWORK_CLASS_IS_VALID_CONST_1;
...
int Network::MessageBufferImpl
	::GetReceiveCount() const noexcept
{
	NETWORK_CLASS_IS_VALID_CONST_1;
	...
	::SetReceiveCount(int receiveCount)
{
	...
	{
		THROW_EXCEPTION(SYSTEM_TEXT("接收数据越界。"));
	}
}

void Network::MessageBufferImpl
	::DecreaseReceiveCount(int receiveCount)
{
	if (m_ReceiveCount == -1)
	{
	...
	{
		THROW_EXCEPTION(SYSTEM_TEXT("接收数据数量不足。"));
	}
}
	...
	::GetMessageLength() const
{
	...
	if (CORE_TOOLS_STREAM_SIZE(int32_t{}) <= GetCurrentWriteIndex())
	{
	...
	{
		THROW_EXCEPTION(SYSTEM_TEXT("无法获取数据长度。"));
	}
}

bool Network::MessageBufferImpl
	::IsMessageReceiveEnd() const
{
	NETWORK_CLASS_IS_VALID_CONST_1;
...
void Network::MessageBufferImpl
	::CheckingMessageContentSize()
{
	NETWORK_CLASS_IS_VALID_1;

	auto bytesTotal = GetRemainingWriteCount();
	auto totalLength = GetMessageLength();
	...
	::Write(int itemSize, int itemsNumber, const void* data)
{
	...
	{
		THROW_EXCEPTION(SYSTEM_TEXT("可写入的缓冲区大小不足！"));
	}
...
void Network::MessageBufferImpl
	::PushBack(const ConstImplPtr& messageBuffer)
{
	NETWORK_CLASS_IS_VALID_1;

	auto writeIndex = messageBuffer->GetCurrentWriteIndex();
...
Network::ParserStrategy Network::MessageBufferImpl
	::GetParserStrategy() const noexcept
{
	NETWORK_CLASS_IS_VALID_CONST_1;
