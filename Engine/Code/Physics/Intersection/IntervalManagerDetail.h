...
#ifndef PHYSICS_INTERSECTION_INTERVAL_MANAGER_DETAIL_H
#define PHYSICS_INTERSECTION_INTERVAL_MANAGER_DETAIL_H
...
namespace Physics
{
	...
	template <typename Real>
	void IntervalManager<Real>::SetInterval(int i, Real minValue,Real maxValue)
	{
		PHYSICS_ASSERTION_0(0 <= i && i < (int)mInterval->size(), "Invalid index\n");
	...
	template <typename Real>
	void IntervalManager<Real>::GetInterval(int i, Real& minValue,Real& maxValue) const
	{
		PHYSICS_ASSERTION_0(0 <= i && i < (int)mInterval->size(), "Invalid index\n");
	...
	template <typename Real>
	const std::set<Mathematics::EdgeKey>& IntervalManager<Real>::GetOverlap() const
	{
		return mOverlap;
	}
	...
	template <typename Real>
	bool IntervalManager<Real>::Endpoint::operator< (const Endpoint& endpoint)	const
	{
...
#endif // PHYSICS_INTERSECTION_INTERVAL_MANAGER_DETAIL_H