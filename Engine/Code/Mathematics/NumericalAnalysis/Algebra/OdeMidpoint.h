...
#ifndef MATHEMATICS_NUMERICAL_ANALYSIS_ALGEBRA_ODE_MIDPOINT_H
#define MATHEMATICS_NUMERICAL_ANALYSIS_ALGEBRA_ODE_MIDPOINT_H
...
namespace Mathematics::Algebra
{
    ...
    public:
        OdeMidpoint(Real tDelta, const FunctionType& f);

        CLASS_INVARIANT_DECLARE;

        /// 使用dx/dt = F(t,x)从x(t)估计x(t + tDelta)。
        /// 您可以允许xIn和xOut是同一个对象。
        void Update(Real tIn, const TVector& xIn, Real& tOut, TVector& xOut) override;
    };
}

#endif  // MATHEMATICS_NUMERICAL_ANALYSIS_ALGEBRA_ODE_MIDPOINT_H
