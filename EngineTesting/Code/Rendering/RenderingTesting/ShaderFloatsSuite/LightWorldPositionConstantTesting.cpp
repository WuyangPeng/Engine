...
// ◊˜’ﬂ£∫≈ÌŒ‰—Ù£¨≈ÌÍ ∂˜£¨≈ÌÍ ‘Û
// 
// “˝«Ê≤‚ ‘∞Ê±æ£∫0.0.0.3 (2019/09/06 15:25)
...
#include "Rendering/ShaderFloats/LightWorldPositionConstant.h"
...
void Rendering::LightWorldPositionConstantTesting
	::MainTest()
{ 
	...
	::InitTest()
{
	...
	for (int loop = 0; loop < GetTestLoopCount(); ++loop)
	{
		...
		LightWorldPositionConstant firstShaderFloat(firstLight);
		...
		const LightWorldPositionConstant secondShaderFloat(firstLight);
		...
		LightWorldPositionConstant thirdShaderFloat(firstLight);
	...
	::CopyTest()
{
	...
	for (int loop = 0; loop < GetTestLoopCount(); ++loop)
	{
		...
		LightWorldPositionConstant secondShaderFloat(firstShaderFloat);
...
// 		LightWorldPositionConstantSharedPtr firstShaderFloat(new LightWorldPositionConstant(firstLight));
// 		
// 		for (int i = 0; i < 4;++i)
// 		{	 
...
// 		
// 		LightWorldPositionConstantSharedPtr secondShaderFloat =
// 			inTopLevel[0].PolymorphicDowncastObjectSharedPtr<LightWorldPositionConstantSharedPtr>();
// 		
