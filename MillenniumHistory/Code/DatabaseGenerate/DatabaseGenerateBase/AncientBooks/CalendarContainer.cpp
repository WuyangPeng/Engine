...
AncientBooks::CalendarContainer::ConstCalendarSharedPtr AncientBooks::CalendarContainer::GetCalendar(int key) const
{
    ...
    if (const auto iter = std::ranges::lower_bound(calendar, std::make_shared<CalendarBase>(key), function);
        iter != calendar.cend() && (*iter)->GetKey() == key)
    {
        return *iter;
    }
    else
...
int AncientBooks::CalendarContainer::GetContainerSize() const
{
    USER_CLASS_IS_VALID_CONST_9;

    return boost::numeric_cast<int>(calendar.size());
}
