...
namespace Physics
{
    ...
    {
        mEquations = nullptr;  //     NEW1<Equation>(mNumEquations);
        const int numEquationsP1 = mNumEquations + 1;
        for (int i = 0; i < mNumEquations; ++i)
        {
            mEquations[i].C = nullptr;  //     NEW1<double>(numEquationsP1);
            mEquations[i].W = nullptr;  //    NEW1<double>(numEquationsP1);
            mEquations[i].Z = nullptr;  //    NEW1<double>(numEquationsP1);
        }
    }
    ...
    bool LCPSolverManager::EquationAlgorithm(int& equation)
    {
        ...
        int** found = nullptr;  //     NEW2<int>(2, mNumEquations + 1);
    ...
    void LCPSolverManager::Solve(char basicVariable, int basicVariableIndex)
    {
        ...
        replacement.C = nullptr;  //                NEW1<double>(numEquationsP1);
        replacement.W = nullptr;  //     NEW1<double>(numEquationsP1);
        replacement.Z = nullptr;  //     NEW1<double>(numEquationsP1);
