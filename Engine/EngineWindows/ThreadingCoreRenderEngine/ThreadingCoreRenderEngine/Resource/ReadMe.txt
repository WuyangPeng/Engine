...
12. 使用Doxygen的注释风格。
13. 禁止注释掉代码，必需保留不执行的代码使用预处理器。
14. 定义宏为空时，使用static_cast<void>(0)，可防止宏被用作右值。
15. 使用using代替typedef。
16. 整型转换不使用static_cast，转换失败抛出异常使用boost::numeric_cast，不能抛出异常使用gsl::narrow_cast消除警告。
17. 使用make_shared和make_unique来创建智能指针，而不是new。
18. 使用必要的noexcept、final和override。
19. 对直接抛出异常的函数使用[[noreturn]] 。
20. 对必须检查返回值的函数使用[[nodiscard]]。
21. 对可能不会使用的参数使用[[maybe_unused]]。
22. 对于没有break的case标签使用[[fallthrough]]。
23. 对于内置类型，优先使用bool、char、int、double和必要的size_t。
...
16. 错误使用异常而非返回码。
17. 使用Impl模式封装类，通过定义ClassShareType来标识类是属于：
   ...
   （5）类禁止复制。
18. 所有类必须有单元测试，并通过测试。单元测试包括完整测试、固定值测试、随机值测试和迭代测试，每个测试一个概念。单元测试一般不测试类不变式和断言。
