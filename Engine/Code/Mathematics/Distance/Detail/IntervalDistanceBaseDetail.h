...
Mathematics::IntervalDistanceBase<Real, Vector>
	...
	:m_Distance{ distance }, m_TMin{ tMin }, m_TMax{ tMax }, m_LhsVelocity{ lhsVelocity },
	 m_RhsVelocity{ rhsVelocity }, m_DistanceResult{ Real{} }, m_BeginT{ tMin }, m_EndT{ tMax },
	 m_BeginDistanceResult{ Real{} }, m_BeginDerivativeDistanceResult{ Real{} }, m_EndDistanceResult{ Real{} }, m_EndDerivativeDistanceResult{ Real{} }
{
...
{ 
    ...
    {   
		// 距离为有效值零。对象是在最初位置接触。
		m_BeginDistanceResult.Set(Real{}, m_BeginT);
	...
	if (Real{} <= m_BeginDerivativeDistanceResult)
    {
   		// 距离在[0,tmax]增加。 
	...
	::CheckEndMonotonicFunction()
{
    ...
    {
		// 距离为有效值零。
		m_EndDistanceResult.Set(Real{}, m_EndT);
	...
	if (m_EndDerivativeDistanceResult <= Real{})
    {
		// 距离在[0,tmax]增加。 
	...
	::Iteration()
{
	...
	for (auto loop = 0; loop < maximumIterations; ++loop)
	{
		...
		{
			// 距离为有效值零。
			m_DistanceResult.SetDistance(Real{});
