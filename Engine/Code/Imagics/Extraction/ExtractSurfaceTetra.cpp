...
Imagics::ExtractSurfaceTetra ::ExtractSurfaceTetra(int xBound, int yBound, int zBound, int* data) noexcept
    : mXBound(xBound),
      mYBound(yBound),
      mZBound(zBound),
...
float Imagics::ExtractSurfaceTetra ::GetFunction(const Mathematics::Vector3F& P) const noexcept
{
    ...
    const float dx = P.GetX() - x;
    const float dy = P.GetY() - y;
    const float dz = P.GetZ() - z;
...
Mathematics::Vector3F Imagics::ExtractSurfaceTetra ::GetGradient(const Mathematics::Vector3F& P) const noexcept
{
    const int x = (int)P.GetX();
    if (x < 0 || x >= mXBound - 1)
    {
    ...
    const float dx = P.GetX() - x;
    const auto dy = P.GetY() - y;
    const auto dz = P.GetZ() - z;
    ...
    if ((x & 1) ^ (y & 1) ^ (z & 1))
    {
        ...
        {
            // 1205
            interp.SetX(+f100 - f000);
            interp.SetY(-f100 + f110);
            interp.SetZ(-f100 + f101);
        }
        else if (dx - dy + dz <= 0.0f)
        {
            // 3027
            interp.SetX(-f010 + f110);
            interp.SetY(+f010 - f000);
            interp.SetZ(-f010 + f011);
        }
        else if (dx + dy - dz <= 0.0f)
        {
            // 4750
            interp.SetX(-f001 + f101);
            interp.SetY(-f001 + f011);
            interp.SetZ(+f001 - f000);
        }
        else if (dx + dy + dz >= 2.0f)
        {
            // 6572
            interp.SetX(+f111 - f011);
            interp.SetY(+f111 - f101);
            interp.SetZ(+f111 - f110);
        }
        else
        {
            // 0752
            interp.SetX(0.5f * (-f000 - f011 + f101 + f110));
            interp.SetY(0.5f * (-f000 + f011 - f101 + f110));
            interp.SetZ(0.5f * (-f000 + f011 + f101 - f110));
        }
    }
    else
    {
        ...
        {
            // 0134
            interp.SetX(-f000 + f100);
            interp.SetY(-f000 + f010);
            interp.SetZ(-f000 + f001);
        }
        else if (dx + dy - dz >= 1.0f)
        {
            // 2316
            interp.SetX(+f110 - f010);
            interp.SetY(+f110 - f100);
            interp.SetZ(-f110 + f111);
        }
        else if (dx - dy + dz >= 1.0f)
        {
            // 5461
            interp.SetX(+f101 - f001);
            interp.SetY(-f101 + f111);
            interp.SetZ(+f101 - f100);
        }
        else if (-dx + dy + dz >= 1.0f)
        {
            // 7643
            interp.SetX(-f011 + f111);
            interp.SetY(+f011 - f001);
            interp.SetZ(+f011 - f010);
        }
        else
        {
            // 6314
            interp.SetX(0.5f * (f111 - f010 + f100 - f001));
            interp.SetY(0.5f * (f111 + f010 - f100 - f001));
            interp.SetZ(0.5f * (f111 - f010 - f100 + f001));
        }
    }
...
void Imagics::ExtractSurfaceTetra ::ExtractContour(int level, std::vector<Mathematics::Vector3F>& vertices, std::vector<Mathematics::TriangleKey>& triangles)
{
...
void Imagics::ExtractSurfaceTetra ::MakeUnique(std::vector<Mathematics::Vector3F>& vertices, std::vector<Mathematics::TriangleKey>& triangles)
{
...
void Imagics::ExtractSurfaceTetra ::OrientTriangles(std::vector<Mathematics::Vector3F>& vertices,
...
void Imagics::ExtractSurfaceTetra ::ComputeNormals(const std::vector<Mathematics::Vector3F>& vertices, const std::vector<Mathematics::TriangleKey>& triangles, std::vector<Mathematics::Vector3F>& normals)
{
...
int Imagics::ExtractSurfaceTetra ::AddVertex(VtxMap& vertexMap, int xNumer, int xDenom,
...
void Imagics::ExtractSurfaceTetra ::AddEdge(VtxMap& vertexMap, ESet& edgeSet,
...
void Imagics::ExtractSurfaceTetra ::AddTriangle(VtxMap& vertexMap, ESet& edgeSet,
                                                ...
                                                int yNumer2, int yDenom2, int zNumer2, int zDenom2)
{
    ...
    const Mathematics::Vector3F pos0(
        xNumer0 / (float)xDenom0,
        yNumer0 / (float)yDenom0,
        zNumer0 / (float)zDenom0);

    const Mathematics::Vector3F pos1(
        xNumer1 / (float)xDenom1,
        yNumer1 / (float)yDenom1,
        zNumer1 / (float)zDenom1);

    const Mathematics::Vector3F pos2(
        xNumer2 / (float)xDenom2,
        yNumer2 / (float)yDenom2,
        zNumer2 / (float)zDenom2);

    const Mathematics::Vector3F edge0 = pos1 - pos0;
    const Mathematics::Vector3F edge1 = pos2 - pos0;
    const Mathematics::Vector3F normal = Mathematics::Vector3ToolsF::CrossProduct(edge0, edge1);

    // Choose triangle orientation based on gradient direction.
    const Mathematics::Vector3F centroid = (pos0 + pos1 + pos2) / 3.0f;
    const Mathematics::Vector3F grad = GetGradient(centroid);
    if (Mathematics::Vector3ToolsF::DotProduct(grad, normal) <= 0.0f)
    {
...
void Imagics::ExtractSurfaceTetra ::ProcessTetrahedron(VtxMap& vertexMap,
...
Imagics::ExtractSurfaceTetra::Vertex ::Vertex(int xNumer, int xDenom, int yNumer,
...
bool Imagics::ExtractSurfaceTetra::Vertex ::operator<(const Vertex& vertex) const noexcept
{
...
void Imagics::ExtractSurfaceTetra::Vertex ::GetTriple(Mathematics::Vector3F& meshVertex) const noexcept
{
    meshVertex.SetX(float(XNumer) / float(XDenom));
    meshVertex.SetY(float(YNumer) / float(YDenom));
    meshVertex.SetZ(float(ZNumer) / float(ZDenom));
}
