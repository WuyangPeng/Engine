...
// ◊˜’ﬂ£∫≈ÌŒ‰—Ù£¨≈ÌÍ ∂˜£¨≈ÌÍ ‘Û
// 
// “˝«Ê∞Ê±æ£∫0.0.0.2 (2019/07/11 11:30)
	...
	::GetSquared() const
{
    ...
    if (direction.GetXCoordinate() > Real{})
    {
        if (direction.GetYCoordinate() > Real{})
        {
            if (direction.GetZCoordinate() > Real{})  // (+,+,+)
            {
        ...
        {
            if (direction.GetZCoordinate() > Real{})  // (+,0,+)
            {
    ...
    else
    {
        ...
        {
            if (direction.GetZCoordinate() > Real{})  // (0,0,+)
            {
	...
	::Face (int i0, int i1, int i2, Vector3D& pnt, const Vector3D& dir, const Vector3D& PmE, Real& sqrDistance) const
{
    ...
    if (dir[i0]*PpE[i1] >= dir[i1]*PmE[i0])
    {
        ...
        else
        {
            ...
            if (tmp <= ((Real)2)*lenSqr*mBox.GetExtent(i1))
            {
                t = tmp/lenSqr;
    ...
    else
    {
        if (dir[i0]*PpE[i2] >= dir[i2]*PmE[i0])
        {
            ...
            if (tmp <= ((Real)2)*lenSqr*mBox.GetExtent(i2))
            {
                t = tmp/lenSqr;
