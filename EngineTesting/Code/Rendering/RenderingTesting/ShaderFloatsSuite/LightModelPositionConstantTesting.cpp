...
///
///	标准：std:c++20
///	引擎测试版本：0.9.0.12 (2023/06/12 15:22)

#include "LightModelPositionConstantTesting.h"
...
Rendering::LightModelPositionConstantTesting::LightModelPositionConstantTesting(const OStreamShared& stream)
    : ParentType{ stream }
{
    RENDERING_SELF_CLASS_IS_VALID_1;
}

CLASS_INVARIANT_PARENT_IS_VALID_DEFINE(Rendering, LightModelPositionConstantTesting)

void Rendering::LightModelPositionConstantTesting::DoRunUnitTest()
{
    ASSERT_NOT_THROW_EXCEPTION_0(MainTest);
}

void Rendering::LightModelPositionConstantTesting::MainTest()
{
...
void Rendering::LightModelPositionConstantTesting::InitTest()
{
    ...
    for (int loop = 0; loop < GetTestLoopCount(); ++loop)
    {
        ...
        const LightModelPositionConstant secondShaderFloat(firstLight);
        ...
        LightModelPositionConstant thirdShaderFloat(firstLight);
...
void Rendering::LightModelPositionConstantTesting::CopyTest()
{
    std::default_random_engine generator;
    const std::uniform_real<float> firstFloatRandomDistribution(-100.0f, 100.0f);
    const std::uniform_real<float> secondFloatRandomDistribution(0.0f, 1.0f);
    ...
    for (int loop = 0; loop < GetTestLoopCount(); ++loop)
    {
        ...
        LightModelPositionConstant secondShaderFloat(firstShaderFloat);
...
void Rendering::LightModelPositionConstantTesting::StreamTest() noexcept
{
}

void Rendering::LightModelPositionConstantTesting::UpdateTest() noexcept
{
}
