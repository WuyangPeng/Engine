...
///
///	标准：std:c++20
///	引擎测试版本：0.8.1.3 (2022/10/14 21:17)
...
{
    ASSERT_NOT_THROW_EXCEPTION_0(FormatLastErrorUseDllMoudleAndUseBufferTest);
}

void System::FormatErrorMessageUseDllMoudleAndUseBufferTesting::FormatLastErrorUseDllMoudleAndUseBufferTest()
{
    ...
    for (auto flag = WindowError::Success; flag <= WindowError::AppHang; ++flag)
    {
        ...
        {
            const auto size = FormatErrorMessage(module, flag, buffer.data(), boost::numeric_cast<WindowsDWord>(buffer.size()));
            ASSERT_LESS(0u, size);
