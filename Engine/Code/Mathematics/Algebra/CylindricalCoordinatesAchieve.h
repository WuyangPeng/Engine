...
Mathematics::CylindricalCoordinates<Real>::CylindricalCoordinates(Real r, Real φ, Real y)
    : r{ r },
      φ{ φ },
      y{ y }
{
...
{
    return Math::GetValue(0) <= r && Math::GetValue(0) <= φ && φ < Math::GetTwoPI();
}
...
void Mathematics::CylindricalCoordinates<Real>::SetCartesian(Real xCoordinate, Real yCoordinate, Real zCoordinate) noexcept(gAssert < 3 || gMathematicsAssert < 3)
{
    ...
    φ = Math::GetZeroTolerance() < r ? Math::ASin(xCoordinate / r) : Real{};
    y = yCoordinate;
}
...
Mathematics::Vector3<Real> Mathematics::CylindricalCoordinates<Real>::GetCartesian() const noexcept
{
    ...
    return Vector3{ r * Math::Sin(φ), y, r * Math::Cos(φ) };
}
