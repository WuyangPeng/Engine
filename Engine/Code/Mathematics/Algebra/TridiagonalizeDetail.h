...
Mathematics::Tridiagonalize<Real>
	...
	:m_InputMatrix{ matrix }, m_OutputMatrix{ MatrixTypeFlags::Identity }, m_Reflection{ false }
{
	...
	::Init( )	
{
	...
	if (Math::sm_ZeroTolerance <= Math::FAbs(m02))
	{
		...
		auto invLength = AlgebraTraits::UnitValue / m_Subdiagonal[0];
		m01 *= invLength;
		m02 *= invLength;
		auto temp = static_cast<Real>(2) * m01 * m12 + m02 * (m22 - m11);
		m_Diagonal[1] = m11 + m02 * temp;
		m_Diagonal[2] = m22 - m02 * temp;
		m_Subdiagonal[1] = m12 - m01 * temp;

		m_OutputMatrix(0,0) = AlgebraTraits::UnitValue;
		m_OutputMatrix(0,1) = Real{ };
		m_OutputMatrix(0,2) = Real{ };
		m_OutputMatrix(1,0) = Real{ };
		m_OutputMatrix(1,1) = m01;
		m_OutputMatrix(1,2) = m02;
		m_OutputMatrix(2,0) = Real{ };
...
{	
	...
	{	
		...
		auto ratio = (m_Diagonal[1] - m_Diagonal[0]) / (static_cast<Real>(2) * m_Subdiagonal[0]);
		auto root = Math::Sqrt(AlgebraTraits::UnitValue + ratio * ratio);
		...
		{
			ratio = a / b;
			sinValue = Math::InvSqrt((AlgebraTraits::UnitValue + ratio * ratio));
			cosValue = ratio * sinValue;
		}
		else
		{
			ratio = b / a;
			cosValue = Math::InvSqrt(AlgebraTraits::UnitValue + ratio * ratio);
		...
		auto tmp0 = (m_Diagonal[1] - m_Diagonal[2]) * sinValue + static_cast<Real>(2) * m_Subdiagonal[1] * cosValue;
		...
		{
			ratio = a / b;
			root = Math::Sqrt(AlgebraTraits::UnitValue + ratio * ratio);
			m_Subdiagonal[1] = b * root;
			sinValue = AlgebraTraits::UnitValue / root;
			cosValue = ratio * sinValue;
		}
		else
		{
			...
			cosValue = AlgebraTraits::UnitValue / root;
			sinValue = ratio * cosValue;
		}
		...
		m_Diagonal[2] += tmp0;
		tmp0 = (m_Diagonal[0] - tmp2) * sinValue + static_cast<Real>(2) *  tmp1 * cosValue;
