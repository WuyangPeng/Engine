...
{
    RENDERING_CLASS_IS_VALID_CONST_1;
    RENDERING_ASSERTION_0(0 <= target && target < numTargets, "索引错误！");
    RENDERING_ASSERTION_0(0 <= aVertices && aVertices < numVertices, "索引错误！");

    const int index = aVertices + target * numVertices;

    RENDERING_ASSERTION_0(0 <= index && index < boost::numeric_cast<int>(vertices.size()), "索引错误！");
...
{
    RENDERING_CLASS_IS_VALID_CONST_1;
    RENDERING_ASSERTION_0(0 <= key && key < numKeys, "索引错误！");
...
{
    RENDERING_CLASS_IS_VALID_CONST_1;
    RENDERING_ASSERTION_0(0 <= target && target < numTargets - 1, "索引错误！");
    RENDERING_ASSERTION_0(0 <= key && key < numKeys, "索引错误！");

    const auto index = target + key * (numTargets - 1);

    RENDERING_ASSERTION_0(0 <= index && index < boost::numeric_cast<int>(weights.size()), "索引错误！");
