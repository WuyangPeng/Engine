...
（21）while循环安全计数器宏：while循环安全计数器宏。
...
24.	ModuleInitializerTerminator（模块自动及终止）
...
25.	SyntacticParser（语法分析器）

封装了语法分析器。
待实现功能：
（1）语法分析器：实现一个语法分析器。

26.	PerformanceProfiler（性能剖析/统计采集）
...
30.	RegularExpression（正则表达式）

封装了正则表达式。
待实现功能：
（1）正则表达式：封装正则表达式的实现。

31.	MessageEvent（消息事件）
