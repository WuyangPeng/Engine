...
// ◊˜’ﬂ£∫≈ÌŒ‰—Ù£¨≈ÌÍ ∂˜£¨≈ÌÍ ‘Û
//
// “˝«Ê∞Ê±æ£∫0.0.0.3 (2019/07/29 17:34)
...
namespace Physics
{
    ...
    RectangleManager<Real>::RectangleManager(std::vector<Mathematics::AxesAlignBoundingBox2<Real>>& rectangles)
        ...
        for (i = 0; i < endpSize; ++i)
        {
            ...
            if (endpoint.Type == 0)  // an interval 'begin' value
            {
                ...
                for (/**/; iter != end; ++iter)
                {
                    ...
                    const Mathematics::AxesAlignBoundingBox2<Real>& r0 = (*mRectangles)[activeIndex];
                    const Mathematics::AxesAlignBoundingBox2<Real>& r1 = (*mRectangles)[index];
    ...
    template <typename Real>
    void RectangleManager<Real>::SetRectangle(int i, const Mathematics::AxesAlignBoundingBox2<Real>& rectangle)
    {
    ...
    template <typename Real>
    void RectangleManager<Real>::GetRectangle(int i, Mathematics::AxesAlignBoundingBox2<Real>& rectangle) const
    {
        PHYSICS_ASSERTION_0(0 <= i && i < (int)mRectangles->size(), "Invalid index\n");
            ...
            while (i >= 0 && key < endpoint[i])
            {
                ...
                else
                {
                    if (e1.Type == 0)
                    {
                        ...
                        const Mathematics::AxesAlignBoundingBox2<Real>& r0 =
                            (*mRectangles)[e0.Index];

                        const Mathematics::AxesAlignBoundingBox2<Real>& r1 =
