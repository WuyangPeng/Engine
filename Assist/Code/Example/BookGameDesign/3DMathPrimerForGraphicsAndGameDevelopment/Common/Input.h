...
namespace ThreeDMathPrimerForGraphicsAndGameDevelopment
{
	...
	public:
		...
		bool isPressed(int keyCode) noexcept;

		// Simulate a keypress.  Returns the previous key state

		bool press(int keyCode) noexcept;
		...
		bool getNextEvent(KeyEvent* event) noexcept;

		// Queue an event.

		void queueEvent(const KeyEvent& event) noexcept;
