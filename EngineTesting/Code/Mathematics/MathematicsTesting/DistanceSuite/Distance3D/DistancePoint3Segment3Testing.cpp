...
void Mathematics::DistancePoint3Segment3Testing::BaseTest()
{
    ...
    for (auto loop = 0; loop < aTestLoopCount; ++loop)
    {
        ...
        ASSERT_APPROXIMATE_USE_FUNCTION(Approximate<float>, distance.GetSegment(), Segment3F(rhsExtent, rhsOrigin, rhsDirection, 1e-5f), 1e-8f);
    }
}
...
void Mathematics::DistancePoint3Segment3Testing::StaticTest()
{
    ...
    for (auto loop = 0; loop < aTestLoopCount; ++loop)
    {
        ...
        {
            ASSERT_TRUE(Vector3ToolsD::GetLength(rhsDifference) <= rhsExtent + 1e-10);
