...
// private
void CoreTools::CFileManagerImpl
	::Open()
{
...
CoreTools::CFileManagerImpl::OffType CoreTools::CFileManagerImpl
	::GetFileLength() const
{
	...
	{
		THROW_EXCEPTION((Error::Format(SYSTEM_TEXT("文件“%1%”不存在！")) % m_FileName).str());
	}
...
string CoreTools::CFileManagerImpl
	::GetString(int count)
{
	CORE_TOOLS_CLASS_IS_VALID_1;
...
CoreTools::CFileManagerImpl::PosType CoreTools::CFileManagerImpl
	::GetPosition()
{
	CORE_TOOLS_CLASS_IS_VALID_1;
	...
	{
		THROW_EXCEPTION((Error::Format(SYSTEM_TEXT("获取文件“%1%”的位置失败！")) % m_FileName).str());
	}
}

bool CoreTools::CFileManagerImpl
	::SetPosition(PosType position) noexcept
{
	CORE_TOOLS_CLASS_IS_VALID_1;
...
long CoreTools::CFileManagerImpl
	::Tell() noexcept
{
	CORE_TOOLS_CLASS_IS_VALID_1;
