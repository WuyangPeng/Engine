...
void NaturalSpline2<Real>::CreateFreeSpline ()
{
    ...
    for (i = 1; i < mNumSegments; ++i)
    {
		auto numer = ((Real)3)*(mA[i+1]*dt[i-1] - mA[i]*d2t[i] +mA[i-1]*dt[i]);
		auto invDenom = ((Real)1)/(dt[i-1]*dt[i]);
    ...
    for (i = mNumSegments-1; i >= 0; --i)
    {
        ...
        inv = ((Real)1)/dt[i];
        mB[i] = inv*(mA[i+1] - mA[i]) - oneThird*dt[i]*(mC[i+1] + ((Real)2)*mC[i]);
...
void NaturalSpline2<Real>::CreateClampedSpline ( const Vector2D<Real>& derivativeStart, const Vector2D<Real>& derivativeFinal)
{
	...
	auto inv = ((Real)1)/dt[0];
    alpha[0] = ((Real)3)*(inv*(mA[1] - mA[0]) - derivativeStart);
    inv = ((Real)1)/dt[mNumSegments-1];
    alpha[mNumSegments] = ((Real)3)*(derivativeFinal -  inv*(mA[mNumSegments] - mA[mNumSegments-1]));
    for (i = 1; i < mNumSegments; ++i)
    {
		auto numer = ((Real)3)*(mA[i+1]*dt[i-1] - mA[i]*d2t[i] + mA[i-1]*dt[i]);
		auto invDenom = ((Real)1)/(dt[i-1]*dt[i]);
...
void NaturalSpline2<Real>::CreateClosedSpline ()
{
    ...
    mC[mNumSegments] = ((Real)3)*(inv0*(mA[1] - mA[0]) - inv1*(mA[0] - mA[mNumSegments-1]));
...
{
    Vector2D<Real> velocity = mB[key] + t*(mC[key]*((Real)2) + mD[key]*(((Real)3)*t));
