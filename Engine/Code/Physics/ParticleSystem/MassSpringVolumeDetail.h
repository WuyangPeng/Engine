...
namespace Physics
{
        ...
        : ParticleSystemManager<Real, TVector>(numSlices * numRows * numCols, step)
    {
        ...
        mPositionGrid = nullptr;  //NEW1<TVector**>(mNumSlices);
        mVelocityGrid = nullptr;  //NEW1<TVector**>(mNumSlices);
        for (int slice = 0; slice < mNumSlices; ++slice)
        {
            mPositionGrid[slice] = nullptr;  //NEW1<TVector*>(mNumRows);
            mVelocityGrid[slice] = nullptr;  //NEW1<TVector*>(mNumRows);
        ...
        mConstantsS = nullptr;  //NEW3<Real>(mNumCols, mNumRows, mNumSlicesM1);
        mLengthsS = nullptr;  //NEW3<Real>(mNumCols, mNumRows, mNumSlicesM1);
        mConstantsR = nullptr;  // NEW3<Real>(mNumCols, mNumRowsM1, mNumSlices);
        mLengthsR = nullptr;  //NEW3<Real>(mNumCols, mNumRowsM1, mNumSlices);
        mConstantsC = nullptr;  // NEW3<Real>(mNumColsM1, mNumRows, mNumSlices);
        mLengthsC = nullptr;  // NEW3<Real>(mNumColsM1, mNumRows, mNumSlices);
    }
