///	Copyright (c) 2010-2020
///	Threading Core Render Engine
///
///	作者：彭武阳，彭晔恩，彭晔泽
///	联系作者：94458936@qq.com
///
///	标准：std:c++17
///	引擎版本：0.5.2.5 (2020/12/03 15:57)

#ifndef MATHEMATICS_APPROXIMATION_ORTHOGONAL_LINT_FIT3_DETAIL_H
#define MATHEMATICS_APPROXIMATION_ORTHOGONAL_LINT_FIT3_DETAIL_H

#include "OrthogonalLineFit3.h" 

#if !defined(MATHEMATICS_EXPORT_TEMPLATE) || defined(MATHEMATICS_INCLUDED_ORTHOGONAL_LINE_FIT3_ACHIEVE)

    #include "OrthogonalLineFit3Achieve.h"

#endif  //  !defined(MATHEMATICS_EXPORT_TEMPLATE) || defined(MATHEMATICS_INCLUDED_ORTHOGONAL_LINE_FIT3_ACHIEVE)

#endif // MATHEMATICS_APPROXIMATION_ORTHOGONAL_LINT_FIT3_DETAIL_H
