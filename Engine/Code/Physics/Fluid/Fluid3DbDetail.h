...
namespace Physics
{
		...
		for (k = 1; k < mKMax; ++k)
		{
			for (j = 1; j < mJMax; ++j)
			{
				for (i = 1; i < mIMax; ++i)
				{
					...
					int i0 = (int)Mathematics::Math<Real>::Floor(iPrevious);
					int i1 = i0 + 1;
					...
					int j0 = (int)Mathematics::Math<Real>::Floor(jPrevious);
					int j1 = j0 + 1;
					...
					int k0 = (int)Mathematics::Math<Real>::Floor(kPrevious);
					int k1 = k0 + 1;
					...
					Vector3D velAdvect = c0*(b0*(a0*v000 + a1*v100) + b1*(a0*v010 + a1*v110)) + c1*(b0*(a0*v001 + a1*v101) + b1*(a0*v011 + a1*v111));
					...
					Vector3D twoVel00 = ((Real)2)*mVelocity0[k][j][i];
					Vector3D velXX = mVelocity0[k][j][i + 1] - twoVel00 + mVelocity0[k][j][i - 1];
					Vector3D velYY = mVelocity0[k][j + 1][i] - twoVel00 + mVelocity0[k][j - 1][i];
					Vector3D velZZ = mVelocity0[k + 1][j][i] - twoVel00 + mVelocity0[k - 1][j][i];
					Vector3D velSource = SourceVelocity(mTime, mX[i], mY[j], mZ[k], i, j, k);
	...
	template <typename Real>
	Real Fluid3Db<Real>::GetX0() const
	{
		return mX0;
	}

	template <typename Real>
	Real Fluid3Db<Real>::GetY0() const
	{
		return mY0;
	}

	template <typename Real>
	Real Fluid3Db<Real>::GetZ0() const
	{
		return mZ0;
	}

	template <typename Real>
	Real Fluid3Db<Real>::GetX1() const
	{
		return mX1;
	}

	template <typename Real>
	Real Fluid3Db<Real>::GetY1() const
	{
		return mY1;
	}

	template <typename Real>
	Real Fluid3Db<Real>::GetZ1() const
	{
		return mZ1;
	}

	template <typename Real>
	Real Fluid3Db<Real>::GetDt() const
	{
		return mDt;
	}

	template <typename Real>
	Real Fluid3Db<Real>::GetDx() const
	{
		return mDx;
	}

	template <typename Real>
	Real Fluid3Db<Real>::GetDy() const
	{
		return mDy;
	}

	template <typename Real>
	Real Fluid3Db<Real>::GetDz() const
	{
		return mDz;
	}

	template <typename Real>
	Real Fluid3Db<Real>::GetTime() const
	{
		return mTime;
	}

	template <typename Real>
	int Fluid3Db<Real>::GetIMax() const
	{
		return mIMax;
	}

	template <typename Real>
	int Fluid3Db<Real>::GetJMax() const
	{
		return mJMax;
	}

	template <typename Real>
	int Fluid3Db<Real>::GetKMax() const
	{
		return mKMax;
	}

	template <typename Real>
	const Real* Fluid3Db<Real>::GetX() const
	{
		return mX;
	}

	template <typename Real>
	const Real* Fluid3Db<Real>::GetY() const
	{
		return mY;
	}

	template <typename Real>
	const Real* Fluid3Db<Real>::GetZ() const
	{
		return mZ;
	}

	template <typename Real>
	Real*** Fluid3Db<Real>::GetDensity() const
	{
		return mDensity0;
	}

	template <typename Real>
	Mathematics::Vector3D<Real>*** Fluid3Db<Real>::GetVelocity() const
	{
		return mVelocity0;
	}
}
