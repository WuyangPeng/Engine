...
int Mathematics::CubicRootsQr<Real>::operator()(int maxIterations, Real c0, Real c1, Real c2, int& numRoots, Roots& roots) const
{
    ...
    Matrix a{ Roots{ Real{}, Real{}, -c0 },
              Roots{ Math::GetValue(1), Real{}, -c1 },
              Roots{ Real{}, Math::GetValue(1), -c2 } };

    /// 当c1 = c2 = 0时避免QR循环，当c1和c2接近零时避免缓慢收敛。
    const Roots v{ Math::GetValue(1),
...
template <typename Real>
requires(std::is_arithmetic_v<Real>)
int Mathematics::CubicRootsQr<Real>::operator()(int maxIterations, Matrix& a, int& numRoots, Roots& roots) const
{
    numRoots = 0;
...
template <typename Real>
requires(std::is_arithmetic_v<Real>)
void Mathematics::CubicRootsQr<Real>::DoIteration(const Roots& v, Matrix& a) const
{
    ...
    auto multW = Math::GetValue(-2) / (w.at(0) * w.at(0) + w.at(1) * w.at(1));
...
std::array<Real, N> Mathematics::CubicRootsQr<Real>::House(const std::array<Real, N>& x) const
{
    ...
    {
        auto sign = x.at(0) >= Real{} ? Math::GetValue(1) : Math::GetValue(-1);
...
requires(std::is_arithmetic_v<Real>)
template <int N>
void Mathematics::CubicRootsQr<Real>::RowHouse(int rMin, int rMax, int cMin, int cMax, const std::array<Real, N>& v, const std::array<Real, N>& mv, Matrix& a) const
{
    /// 仅使用元素cMin到cMax。
    Roots w{};
...
requires(std::is_arithmetic_v<Real>)
template <int N>
void Mathematics::CubicRootsQr<Real>::ColHouse(int rMin, int rMax, int cMin, int cMax, const std::array<Real, N>& v, const std::array<Real, N>& mv, Matrix& a) const
{
    /// 仅使用元素rMin到rMax。
    Roots w{};
...
template <typename Real>
requires(std::is_arithmetic_v<Real>)
void Mathematics::CubicRootsQr<Real>::GetQuadraticRoots(int i0, int i1, const Matrix& a, int& numRoots, Roots& roots) const
{
    ...
    auto halfTrace = trace * Math::GetRational(1, 2);
    ...
    {
        auto sign = (trace >= Real{} ? Math::GetValue(1) : Math::GetValue(-1));
