///	Copyright (c) 2011-2020
///	Threading Core Render Engine
///
///	作者：彭武阳，彭晔恩，彭晔泽
///	联系作者：94458936@qq.com
///
///	标准：std:c++17
///	引擎版本：0.5.2.4 (2020/11/19 17:03)

#ifndef MATHEMATICS_NUMERICAL_ANALYSIS_BISECT3_ROOT_DETAIL_H
#define MATHEMATICS_NUMERICAL_ANALYSIS_BISECT3_ROOT_DETAIL_H

#include "Bisect3Root.h"

#if !defined(MATHEMATICS_EXPORT_TEMPLATE) || defined(MATHEMATICS_INCLUDED_BISECT3_ROOT_ACHIEVE)

    #include "Bisect3RootAchieve.h"

#endif  // !defined(MATHEMATICS_EXPORT_TEMPLATE) || defined(MATHEMATICS_INCLUDED_BISECT3_ROOT_ACHIEVE)

#endif  // MATHEMATICS_NUMERICAL_ANALYSIS_BISECT3_ROOT_DETAIL_H
