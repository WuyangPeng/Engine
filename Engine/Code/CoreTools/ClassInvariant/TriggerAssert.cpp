...
const size_t CoreTools::TriggerAssert ::sm_DebugPromptLength{ System::Strlen(TriggerAssert::sm_DebugPrompt) };
const char* CoreTools::TriggerAssert ::sm_MessagePrefix{ "\n%s失败在%s(%s,%d):\n" };
const size_t CoreTools::TriggerAssert ::sm_MaxPrefixBytes{ sm_MaxMessageBytes - sm_DebugPromptLength - 1 };
...
void CoreTools::TriggerAssert ::AppendUserArguments(const char* format, va_list arguments) noexcept
{
    ...
    [[maybe_unused]] const auto result = System::VsnPrintf(m_Message.data() + length, sm_MaxPrefixBytes - length, format, arguments);
}
#include STSTEM_WARNING_POP
