...
// ◊˜’ﬂ£∫≈ÌŒ‰—Ù£¨≈ÌÍ ∂˜£¨≈ÌÍ ‘Û
// 
// “˝«Ê∞Ê±æ£∫0.0.0.2 (2019/07/12 18:20)
...
Mathematics::DynamicFindIntersectorTriangle2Triangle2<Real>
	::DynamicFindIntersectorTriangle2Triangle2(const Triangle2& triangle0, const Triangle2& triangle1,Real tmax, const Vector2D& velocity0,const Vector2D& velocity1, const Real epsilon)
...
template <typename Real>
const Mathematics::Triangle2<Real> Mathematics::DynamicFindIntersectorTriangle2Triangle2<Real>
	::GetTriangle0() const
{
    return mTriangle0;
}
...
template <typename Real>
void Mathematics::DynamicFindIntersectorTriangle2Triangle2<Real>
	::Find()
{
    ...
    Vector2D moveV0[3], moveV1[3];
    for (auto i = 0; i < 3; ++i)
    {
		moveV0[i] = mTriangle0.GetVertex()[i] + tfirst* this->GetLhsVelocity();
		moveV1[i] = mTriangle1.GetVertex()[i] + tfirst* this->GetRhsVelocity();
    };
...
template <typename Real>
int Mathematics::DynamicFindIntersectorTriangle2Triangle2<Real>
	::GetQuantity() const
{
    return mQuantity;
}
...
template <typename Real>
const Mathematics::Vector2D<Real>  Mathematics::DynamicFindIntersectorTriangle2Triangle2<Real>
	...
	::ComputeThree(Configuration& cfg, const std::vector<Vector2D>& V,const Vector2D& D,  const Vector2D& P)
{
    ...
    if (d0 <= d1)
    {
        if (d1 <= d2)  // d0 <= d1 <= d2
        {
            if (d0 != d1)
            {
                cfg.Map = (d1 != d2 ? ProjectionMap::M11 : ProjectionMap::M12);
            }
            else
            {
                cfg.Map = ProjectionMap::M21;
            }
        ...
        {
            cfg.Map = (d0 != d1 ? ProjectionMap::M12 : ProjectionMap::M11);
            cfg.Index[0] = 2;
            cfg.Index[1] = 0;
            cfg.Index[2] = 1;
            cfg.Min = d2;
    ...
    else
    {
        ...
        {
            cfg.Map = (d2 != d0 ? ProjectionMap::M11 : ProjectionMap::M12);
            cfg.Index[0] = 1;
            cfg.Index[1] = 2;
            cfg.Index[2] = 0;
            cfg.Min = d1;
	...
	::GetIntersection( const Configuration& cfg0, const Configuration& cfg1, int side,const Vector2D V0[3], const Vector2D V1[3], int& quantity,Vector2D vertex[6])
{
    ...
    {
        if (cfg0.Map == ProjectionMap:: M21 || cfg0.Map == ProjectionMap::M11)
        {
            quantity = 1;
            vertex[0] = V0[cfg0.Index[2]];
        }
        else if (cfg1.Map == ProjectionMap::M12 || cfg1.Map == ProjectionMap::M11)
        {
            quantity = 1;
    ...
    {
        if (cfg1.Map == ProjectionMap::M21 || cfg1.Map == ProjectionMap::M11)
        {
            quantity = 1;
            vertex[0] = V1[cfg1.Index[2]];
        }
        else if (cfg0.Map == ProjectionMap::M12 || cfg0.Map == ProjectionMap::M11)
        {
            quantity = 1;
