...
Network::CacheClientTesting
	::CacheClientTesting(const OStreamShared& osPtr)
...
void Network::CacheClientTesting
	::MainTest()
{
...
void Network::CacheClientTesting
	::CreateMessage()
{
	MESSAGE_MANAGER_SINGLETON.Insert(m_MessageID, MessageTypeCondition{}, NullMessage::Factory);
}
	...
	::BoostAsyncSendingClientTest()
{
	...
	for (auto& client : clientContainer)
	{
		...
		for (;;)
		{
			auto volatile asyncConnectCount = testSocketManager->GetAsyncConnectCount();
	...
	for (;;)
	{
		auto volatile asyncReceiveCount = testSocketManager->GetAsyncReceiveCount();
