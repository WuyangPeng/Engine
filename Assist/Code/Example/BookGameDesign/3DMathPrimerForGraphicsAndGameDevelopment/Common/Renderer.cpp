...
namespace ThreeDMathPrimerForGraphicsAndGameDevelopment
{
    ...
    static LPDIRECT3D9 pD3D = nullptr;

    // Direct3D device interface

    static LPDIRECT3DDEVICE9 pD3DDevice = nullptr;
    ...
    static void setD3DRenderState(D3DRENDERSTATETYPE state, unsigned value) noexcept
    {
        ...
        const HRESULT result = pD3DDevice->SetRenderState(state, value);
    ...
    static void setD3DDirectionalLight() noexcept
    {
        // Make sure device exists
        ...
        const HRESULT result = pD3DDevice->SetLight(0, &d3dDirectionalLight);
    ...
    static bool checkMesh(const void* vertexList, [[maybe_unused]] int vertexCount, const RenderTri* triList, int triCount) noexcept
    {
        // No triangles?
        ...
        if ((vertexList == nullptr) || (triList == nullptr))
        {
            assert(false);
            return false;
        }
        ...
        for (int i = 0; i < triCount; ++i)
        {
            for (int j = 0; j < 3; ++j)
            {
                const int index = triList[i].index[j];
    ...
    int Renderer::getVideoModeCount()
    {
        ...
        for (int pass = 0; pass < 2; ++pass)
        {
            ...
            UINT modeIndex = 0;
            while (modeIndex < pD3D->GetAdapterModeCount(D3DADAPTER_DEFAULT, D3DFMT_A8R8G8B8))
            {
                // Enumerate the next mode.

                D3DDISPLAYMODE mode{};
                const HRESULT result = pD3D->EnumAdapterModes(D3DADAPTER_DEFAULT, D3DFMT_A8R8G8B8, modeIndex, &mode);
                if (FAILED(result))
                {
                    break;
                }
                ...
                else
                {
                    ...
                    {
                        const VideoMode* v = &videoModeList[i];
                        if ((v->xRes == ourMode.xRes) && (v->yRes == ourMode.yRes) &&
                            (v->bitsPerPixel == ourMode.bitsPerPixel) && (v->refreshHz == ourMode.refreshHz))
                        {
                            dup = true;
                            break;
                        }
    ...
    void Renderer::init(const VideoMode& mode)
    {
        ...
        for (;;)
        {
            ...
            result = pD3D->EnumAdapterModes(D3DADAPTER_DEFAULT, D3DFMT_A8R8G8B8, modeIndex, &d3dMode);
            ++modeIndex;
            ...
            if (boost::numeric_cast<int>(d3dMode.Width) != mode.xRes)
                continue;
            if (boost::numeric_cast<int>(d3dMode.Height) != mode.yRes)
                continue;
        ...
        if (mode.bitsPerPixel > 16)
        {
            ...
            if (SUCCEEDED(pD3D->CheckDeviceFormat(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, d3dMode.Format,
                                                  D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE, D3DFMT_D32)))
            {
                depthBufferFormat = D3DFMT_D32;
            }
            else if (SUCCEEDED(pD3D->CheckDeviceFormat(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, d3dMode.Format,
        ...
        result = pD3D->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, gHwndApp,
                                    D3DCREATE_FPU_PRESERVE | D3DCREATE_HARDWARE_VERTEXPROCESSING, &presentParms, &pD3DDevice);
        if (!SUCCEEDED(result))
        {
            // Nope - try falling back on software vertex processing

            result = pD3D->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, gHwndApp,
                                        D3DCREATE_FPU_PRESERVE | D3DCREATE_SOFTWARE_VERTEXPROCESSING, &presentParms, &pD3DDevice);
            if (!SUCCEEDED(result))
            {
        ...
        setD3DRenderState(D3DRS_FOGSTART, *reinterpret_cast<DWORD*>(&fogNear));
        setD3DRenderState(D3DRS_FOGEND, *reinterpret_cast<DWORD*>(&fogFar));
        setD3DRenderState(D3DRS_CULLMODE, D3DCULL_CCW);

        // Set texture wrapping mode

        result = pD3DDevice->SetTextureStageState(0, D3DTSS_COLORARG1, D3DTADDRESS_WRAP);
        assert(SUCCEEDED(result));
        result = pD3DDevice->SetTextureStageState(0, D3DTSS_COLORARG2, D3DTADDRESS_WRAP);
        assert(SUCCEEDED(result));
    ...
    void Renderer::flipPages() noexcept
    {
    ...
    void Renderer::beginScene() noexcept
    {
        ...
        else
        {
            ...
            const HRESULT result = pD3DDevice->BeginScene();
            assert(SUCCEEDED(result));
        }
    }
    ...
    void Renderer::endScene() noexcept
    {
        ...
        else
        {
            ...
            const HRESULT result = pD3DDevice->EndScene();
            assert(SUCCEEDED(result));
        }
    }
    ...
    void Renderer::setCamera(const Vector3& pos, const EulerAngles& orient) noexcept
    {
        ...
        if (pD3DDevice != nullptr)
        {
            ...
            const HRESULT result = pD3DDevice->SetTransform(D3DTS_VIEW, &m);
            assert(SUCCEEDED(result));
        }
    ...
    void Renderer::setZoom(float xZoom, float yZoom) noexcept
    {
    ...
    void Renderer::setNearFarClippingPlanes(float n, float f) noexcept
    {
        // Sanity check
    ...
    void Renderer::setWindow(int x1, int y1, int xSize, int ySize) noexcept
    {
        // Sanity check.
        ...
        if (pD3DDevice != nullptr)
        {
            ...
            viewData.MinZ = 0.0F;
            viewData.MaxZ = 1.0F;
            const HRESULT result = pD3DDevice->SetViewport(&viewData);
            assert(SUCCEEDED(result));
        }
    ...
    void Renderer::setFullScreenWindow() noexcept
    {
    ...
    void Renderer::getWindow(int* x1, int* y1, int* xSize, int* ySize) noexcept
    {
    ...
    void Renderer::instance(const Vector3& pos, const EulerAngles& orient) noexcept
    {
    ...
    void Renderer::instancePop() noexcept
    {
    ...
    void Renderer::setDepthBufferMode(bool readEnabled, bool writeEnabled) noexcept
    {
    ...
    void Renderer::setBlendEnable(bool blend) noexcept
    {
    ...
    void Renderer::setSourceBlendMode(ESourceBlendMode mode) noexcept
    {
        // Check if state changing
    ...
    void Renderer::setDestBlendMode(EDestBlendMode mode) noexcept
    {
        // Check if state changing
    ...
    void Renderer::setRGB(unsigned int rgb) noexcept
    {
    ...
    void Renderer::setOpacity(float a)
    {
        ...
        const int newAlpha = boost::numeric_cast<int>(floor(a * 255.0f + .5f));
        ...
        constantOpacity = boost::numeric_cast<float>(newAlpha / 255.0f);
        ...
        const HRESULT result = pD3DDevice->SetMaterial(&d3dMaterial);
        assert(SUCCEEDED(result));
    }
    ...
    void Renderer::setFogEnable(bool flag) noexcept
    {
        // Check if state changing
    ...
    void Renderer::setFogColor(unsigned rgb) noexcept
    {
        // Mask off alpha value in the input color, if any alpha
        // was specified.
    ...
    void Renderer::setFogDistance(float nearFog, float farFog) noexcept
    {
        ...
        if (nearFog != fogNear)
        {
            ...
            setD3DRenderState(D3DRS_FOGSTART, *reinterpret_cast<DWORD*>(&nearFog));
        }
        ...
        if (farFog != fogFar)
        {
            ...
            setD3DRenderState(D3DRS_FOGEND, *reinterpret_cast<DWORD*>(&farFog));
    ...
    void Renderer::setAmbientLightColor(unsigned rgb) noexcept
    {
        // Mask off alpha value in the input color, if any alpha
        // was specified.
    ...
    void Renderer::setDirectionalLightVector(const Vector3& v) noexcept
    {
    ...
    void Renderer::setDirectionalLightColor(unsigned rgb) noexcept
    {
        // Mask off alpha value in the input color, if any alpha
        // was specified.
    ...
    void Renderer::setLightEnable(bool flag) noexcept
    {
        // Remember new state
    ...
    void Renderer::setBackfaceMode(EBackfaceMode mode) noexcept
    {
        // Check if state changing
    ...
    void Renderer::selectTexture(int handle) noexcept
    {
        ...
        if (handle != currentTextureHandle)
        {
            ...
            assert(pD3DDevice != nullptr);
            const HRESULT result = pD3DDevice->SetTexture(0, t->d3dTexture);
            assert(SUCCEEDED(result));
        }
    }
    ...
    void Renderer::selectTexture(TextureReference& texture) noexcept
    {
    ...
    void Renderer::setTextureClamp(bool flag) noexcept
    {
        HRESULT result{};
        ...
        if (textureClamp != flag)
        {
            ...
            const D3DTEXTUREADDRESS m = flag ? D3DTADDRESS_CLAMP : D3DTADDRESS_WRAP;

            // Set state to D3D for both U and V

            result = pD3DDevice->SetTextureStageState(0, D3DTSS_COLORARG1, m);
            assert(SUCCEEDED(result));
            result = pD3DDevice->SetTextureStageState(0, D3DTSS_COLORARG2, m);
            assert(SUCCEEDED(result));
        }
    }
    ...
    void Renderer::clear(int options) noexcept
    {
        // Make sure we have a device
        ...
        const HRESULT result = pD3DDevice->Clear(0, nullptr, clearWhat, frameBufferARGB, 1.0F, 0);
        assert(SUCCEEDED(result));
    }
    ...
    void Renderer::renderTriMesh(const RenderVertex* vertexList, int vertexCount, const RenderTri* triList, int triCount) noexcept
    {
        HRESULT result{};
        ...
        const DWORD shader[256]{};
        pD3DDevice->CreateVertexShader(shader, &pShader);
        result = pD3DDevice->SetVertexShader(pShader);
        assert(SUCCEEDED(result));

        // Render it using "user pointer" data

        result = pD3DDevice->DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST, 0, vertexCount, triCount,
                                                    triList, D3DFMT_INDEX16, vertexList, sizeof(vertexList[0]));
        assert(SUCCEEDED(result));
    }

    void Renderer::renderTriMesh(const RenderVertexL* vertexList, int vertexCount, const RenderTri* triList, int triCount) noexcept
    {
        HRESULT result{};
        ...
        result = pD3DDevice->DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST, 0, vertexCount, triCount, triList, D3DFMT_INDEX16, vertexList, sizeof(vertexList[0]));
        assert(SUCCEEDED(result));
    }

    void Renderer::renderTriMesh(const RenderVertexTL* vertexList, int vertexCount, const RenderTri* triList, int triCount) noexcept
    {
        HRESULT result{};
        ...
        result = pD3DDevice->DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST, 0, vertexCount, triCount, triList,
                                                    D3DFMT_INDEX16, vertexList, sizeof(vertexList[0]));
        assert(SUCCEEDED(result));
    }
    ...
    void Renderer::dot(int x, int y) noexcept
    {
    ...
    void Renderer::line([[maybe_unused]] int x1, [[maybe_unused]] int y1, [[maybe_unused]] int x2, [[maybe_unused]] int y2) noexcept
    {
        // !FIXME!
    ...
    void Renderer::boxFill(int x1, int y1, int x2, int y2) noexcept
    {
        // Clip to window
    ...
    void Renderer::videoSave() noexcept
    {
    }

    void Renderer::videoRestore() noexcept
    {
    }
    ...
    void Renderer::resetTextureCache()
    {
        ...
        const int kSz = 32;
        const int whiteTextureHandle = allocTexture("white", kSz, kSz);
    ...
    int Renderer::findTexture(const char* name) noexcept
    {
    ...
    int Renderer::allocTexture(const char* name, int xSize, int ySize) noexcept
    {
        TextureCacheEntry* t{};
        ...
        else
        {
            ...
            for (int i = 1; i < nextTextureSlot; ++i)
            {
                if (textureCacheList[i].d3dTexture == nullptr)
                {
                    slot = i;
                    break;
                }
        ...
        const HRESULT result = pD3DDevice->CreateTexture(xSize,  // width
                                                         ySize,  // height
    ...
    void Renderer::freeTexture(int handle) noexcept
    {
    ...
    void Renderer::setTextureImage(int handle, const unsigned* image) noexcept
    {
        HRESULT result{};
        ...
        D3DLOCKED_RECT r{};
        result = t->d3dTexture->LockRect(0,  // level
                                         &r,  // resulting info is placed here
                                         nullptr,  // rectangle to lock - we want the whole thing
                                         0  // flags
        );
        ...
        const unsigned char* srcPtr = reinterpret_cast<const unsigned char*>(image);
        unsigned char* destPtr = static_cast<unsigned char*>(r.pBits);
        const int srcStride = t->xSize * 4;
        ...
        for (int y = 0; y < t->ySize; ++y)
        {
            ...
            if (destPtr)
            {
                const auto temp = t->xSize * 4;
                memcpy(destPtr, srcPtr, temp);
            }
    ...
    int Renderer::cacheTexture(const char* filename) noexcept
    {
        char text[256]{};
        ...
        setTextureImage(slot, static_cast<unsigned*>(bitmap.rawData()));
    ...
    void Renderer::cacheTexture(TextureReference& texture) noexcept
    {
        ...
        if ((texture.handle > 0) && (texture.handle < nextTextureSlot))
        {
            ...
            if ((stricmp(t->name, texture.name) == 0) && (t->d3dTexture != nullptr))
            {
    ...
    int Renderer::computeOutCode(const Vector3& p) noexcept
    {
        ...
        const float x = p.x * modelToClipMatrix._11 + p.y * modelToClipMatrix._21 +
                        p.z * modelToClipMatrix._31 + modelToClipMatrix._41;

        const float y = p.x * modelToClipMatrix._12 + p.y * modelToClipMatrix._22 +
                        p.z * modelToClipMatrix._32 + modelToClipMatrix._42;

        const float z = p.x * modelToClipMatrix._13 + p.y * modelToClipMatrix._23 +
                        p.z * modelToClipMatrix._33 + modelToClipMatrix._43;

        const float w = p.x * modelToClipMatrix._14 + p.y * modelToClipMatrix._24 +
                        p.z * modelToClipMatrix._34 + modelToClipMatrix._44;
    ...
    int Renderer::projectPoint(const Vector3& p, Vector3* result) noexcept
    {
        // Make sure the model-to-clip matrix is computed
    ...
    const Matrix4x3& Renderer::getModelToCameraMatrix() noexcept
    {
    ...
    const Matrix4x3& Renderer::getModelToWorldMatrix() noexcept
    {
    ...
    void Renderer::computeClipMatrix() noexcept
    {
        ...
        if (pD3DDevice != nullptr)
        {
            const HRESULT result = pD3DDevice->SetTransform(D3DTS_PROJECTION, &clipMatrix);
            assert(SUCCEEDED(result));
        }
    ...
    void Renderer::updateModelToWorldMatrix() noexcept
    {
        // Upload this to the rendering API, if we have been initted
        ...
        if (pD3DDevice != nullptr)
        {
            ...
            const HRESULT result = pD3DDevice->SetTransform(D3DTS_WORLDMATRIX(0), &m);
            assert(SUCCEEDED(result));
        }
    ...
    void Renderer::getModelToClipMatrix() noexcept
    {
    ...
    void Renderer::freeAllTextures() noexcept
    {
