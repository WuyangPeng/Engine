...
AncientBooks::IdentityContainer::ConstIdentitySharedPtr AncientBooks::IdentityContainer::GetFirstIdentity() const
{
...
AncientBooks::IdentityContainer::ConstIdentitySharedPtr AncientBooks::IdentityContainer::GetIdentity(int key) const
{
    USER_CLASS_IS_VALID_CONST_9;
    ...
    if (const auto iter = std::ranges::lower_bound(identity, std::make_shared<IdentityBase>(key), function);
        iter != identity.cend() && (*iter)->GetKey() == key)
    {
        return *iter;
    }
    else
...
int AncientBooks::IdentityContainer::GetContainerSize() const
{
    USER_CLASS_IS_VALID_CONST_9;

    return boost::numeric_cast<int>(identity.size());
}
