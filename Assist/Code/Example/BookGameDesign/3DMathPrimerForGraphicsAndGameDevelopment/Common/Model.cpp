...
namespace ThreeDMathPrimerForGraphicsAndGameDevelopment
{
    ...
    {
        partCount = 0;
        partMeshList = nullptr;
        partTextureList = nullptr;
    }
    ...
    void Model::freeMemory() noexcept
    {
        // Free arrays
    ...
    TriMesh* Model::getPartMesh(int index) noexcept
    {
        // Sanity check

        assert(index >= 0);
        assert(index < partCount);
        assert(partMeshList != nullptr);
    ...
    TextureReference* Model::getPartTexture(int index) noexcept
    {
        // Sanity check

        assert(index >= 0);
        assert(index < partCount);
        assert(partTextureList != nullptr);
    ...
    void Model::setPartTextureName(int index, const char* name) noexcept
    {
        // Sanity check
    ...
    void Model::cache() const noexcept
    {
        // Cache all textures
    ...
    void Model::renderPart(int index) const noexcept
    {
        // Sanity check
    ...
    void Model::toEditMesh([[maybe_unused]] const EditTriMesh& mesh) const noexcept
    {
        // !FIXME!
        assert(false);
    }
