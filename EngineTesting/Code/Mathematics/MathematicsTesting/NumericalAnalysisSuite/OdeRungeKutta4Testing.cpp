...
Mathematics::OdeRungeKutta4Testing
	::OdeRungeKutta4Testing(const OStreamShared& osPtr) 
...
void Mathematics::OdeRungeKutta4Testing
	::MainTest()
{
	...
	::SolverTest()
{
	...
	//odeRungeKutta4Test.Update(tIn, xIn, tOut, &xOut[0]);
	...
	for (int i = 0; i < dimension; ++i)
	{
		testXOut[i] = xIn[i] + odeRungeKutta4Test.GetStepSize() / 6 * (firstStepFunctionValue[i] + 2.0 *
			          (secondStepFunctionValue[i] + thirdStepFunctionValue[i]) + fourthStepFunctionValue[i]);
	}
...
const vector<double> Mathematics::OdeRungeKutta4Testing
	::OdeRungeKutta4Function( double tIn, const vector<double>& xIn,const OdeRungeKutta4Testing* odeEulerTesting)
{
...
int Mathematics::OdeRungeKutta4Testing
	::GetDimension() const
{
