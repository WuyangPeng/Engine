...
Rendering::ProjectionViewWorldMatrixConstant::ProjectionViewWorldMatrixConstant(MAYBE_UNUSED CoreTools::DisableNotThrow disableNotThrow)
...
void Rendering::ProjectionViewWorldMatrixConstant::Update(const Visual* visual, const Camera* camera)
{
    ...
    if (visual != nullptr && camera != nullptr)
    {
        ...
        const auto projectionViewWorldMatrix = projectionMatrix * worldMatrix;

        SetRegisters(projectionViewWorldMatrix);
    }
}

void Rendering::ProjectionViewWorldMatrixConstant::SetNumRegisters(int aNumRegisters)
{
    RENDERING_CLASS_IS_VALID_1;
    RENDERING_ASSERTION_2(aNumRegisters == numRegisters, "ProjectionViewWorldMatrixConstant寄存器的数量必须为4");

    ParentType::SetNumRegisters(aNumRegisters);
}

Rendering::ShaderFloatSharedPtr Rendering::ProjectionViewWorldMatrixConstant::Clone() const
{
