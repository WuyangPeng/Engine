...
// ◊˜’ﬂ£∫≈ÌŒ‰—Ù£¨≈ÌÍ ∂˜£¨≈ÌÍ ‘Û
// 
// “˝«Ê∞Ê±æ£∫0.0.2.5 (2020/03/24 16:36)
...
Mathematics::DynamicFindIntersectorTriangle3Triangle3<Real>
	::DynamicFindIntersectorTriangle3Triangle3(const Triangle3& triangle0, const Triangle3& triangle1, Real tmax, const Vector3D& lhsVelocity, const Vector3D& rhsVelocity, const Real epsilon)
...
template <typename Real>
const Mathematics::Triangle3<Real> Mathematics::DynamicFindIntersectorTriangle3Triangle3<Real>
	::GetTriangle0() const
{
	return m_Triangle0;
}
...
template <typename Real>
void Mathematics::DynamicFindIntersectorTriangle3Triangle3<Real>
	::Find()
{
	...
	Vector3D E[3]{ m_Triangle0.GetVertex()[1] - m_Triangle0.GetVertex()[0],
	...
	Vector3D F[3]{ m_Triangle1.GetVertex()[1] - m_Triangle1.GetVertex()[0],
	...
	if (Math::FAbs(Vector3DTools::DotProduct(N, M)) < Math::GetValue(1) - Math::GetZeroTolerance())
	{
		...
		// Direction M.
		if (!FindOverlap(M, tmax, relVelocity, side, tcfg0, tcfg1, tfirst, tlast))
		{
			this->SetIntersectionType(IntersectionType::Empty);
			return;
		}
		...
		for (i1 = 0; i1 < 3; ++i1)
		{
			for (i0 = 0; i0 < 3; ++i0)
			{
				...
				if (!FindOverlap(dir, tmax, relVelocity, side, tcfg0, tcfg1, tfirst, tlast))
				{
					this->SetIntersectionType(IntersectionType::Empty);
					return;
				}
	...
	Triangle3 MTri0{ m_Triangle0.GetVertex()[0] + tfirst * velocity0,
					 m_Triangle0.GetVertex()[1] + tfirst * velocity0,
					 m_Triangle0.GetVertex()[2] + tfirst * velocity0 };
	Triangle3 MTri1{ m_Triangle1.GetVertex()[0] + tfirst * velocity1,
					 m_Triangle1.GetVertex()[1] + tfirst * velocity1,
					 m_Triangle1.GetVertex()[2] + tfirst * velocity1 };
...
template <typename Real>
int Mathematics::DynamicFindIntersectorTriangle3Triangle3<Real>
	::GetQuantity() const
{
	return m_Quantity;
}
...
template <typename Real>
const Mathematics::Vector3D<Real> Mathematics::DynamicFindIntersectorTriangle3Triangle3<Real>
	...
	::ContainsPoint(const Triangle3& triangle, const Plane3& plane, const Vector3D& point)
{
	...
	Vector3D U0 = vector3DOrthonormalBasis.GetUVector();
	Vector3D U1 = vector3DOrthonormalBasis.GetVVector();
	...
	::TestOverlap(const Vector3D& axis, Real tmax, const Vector3D& velocity, Real& tfirst, Real& tlast)
{
	...
	ProjectOntoAxis(m_Triangle0, axis, min0, max0);
	ProjectOntoAxis(m_Triangle1, axis, min1, max1);
	...
	::FindOverlap(const Vector3D& axis, Real tmax, const Vector3D& velocity, ContactSide& side, Configuration& tcfg0, Configuration& tcfg1, Real& tfirst, Real& tlast)
{
	Configuration cfg0;
	Configuration cfg1;
	ProjectOntoAxis(m_Triangle0, axis, cfg0);
	ProjectOntoAxis(m_Triangle1, axis, cfg1);
	auto speed = Vector3DTools::DotProduct(velocity, axis);
	...
	if (side == CS_RIGHT) // tri1 to the right of tri0
	{
		...
		else if (cfg0.mMap == M12)
		{
			if (cfg1.mMap == M21)
			{
				...
				GetEdgeEdgeIntersection(tri0.GetVertex()[cfg0.mIndex[1]], tri0.GetVertex()[cfg0.mIndex[2]],
										tri1.GetVertex()[cfg1.mIndex[0]], tri1.GetVertex()[cfg1.mIndex[1]]);
			}
			else // cfg1.mMap == m3
			{
	...
	::GetEdgeFaceIntersection(const Vector3D& U0, const Vector3D& U1, const Triangle3& tri)
{
	...
	Vector3D dir0 = vector3DOrthonormalBasis.GetUVector();
	Vector3D dir1 = vector3DOrthonormalBasis.GetVVector();
