...
void Mathematics::DistanceRay2Ray2Testing
	::MainTest()
{	
	...
	::BaseTest()
{
	...
	for (auto loop = 0; loop < testLoopCount; ++loop)
	{
		...
		FloatDistanceRay2Ray2 distance(FloatRay2(lhsOrigin, lhsDirection), FloatRay2(rhsOrigin, rhsDirection));
		...
		typedef bool (*ApproximateFunction)(const FloatRay2& lhs, const FloatRay2& rhs, const float epsilon);

		ApproximateFunction approximate = Approximate<float>;
		ASSERT_APPROXIMATE_USE_FUNCTION(approximate, distance.GetLhsRay(), FloatRay2(lhsOrigin, lhsDirection), 1e-8f);
		ASSERT_APPROXIMATE_USE_FUNCTION(approximate, distance.GetRhsRay(), FloatRay2(rhsOrigin, rhsDirection), 1e-8f);
	}
}
	...
	::StaticTest()
{
	...
	for (auto loop = 0; loop < testLoopCount; ++loop)
	{
		...
		DoubleDistanceRay2Ray2 distance(DoubleRay2(lhsOrigin, lhsDirection), DoubleRay2(rhsOrigin,rhsDirection));
		   ...
		   DoubleVector2DTools::Approximate(rhsDifference,rhsDirection,1e-8))
		{
			...
			ASSERT_APPROXIMATE_USE_FUNCTION(DoubleVector2DTools::Approximate, lhsDifference2,lhsDirection, 1e-8);
			    ...
			    DoubleVector2DTools::Approximate(rhsDifference,-rhsDirection,1e-8))
		{
			...
			if(lhsDifference2.IsZero())
			{
				...
				ASSERT_APPROXIMATE(squaredResult.GetDistance(), 
					               DoubleVector2DTools::VectorMagnitudeSquared(lhsOrigin - rhsOrigin),
								   1e-10);
				ASSERT_APPROXIMATE(squaredResult.GetContactTime(),0.0, 1e-10);
				ASSERT_APPROXIMATE(result.GetDistance(), DoubleVector2DTools::VectorMagnitude(lhsOrigin - rhsOrigin), 1e-10);
				ASSERT_APPROXIMATE(result.GetContactTime(), 0.0, 1e-10);
				ASSERT_APPROXIMATE_USE_FUNCTION(DoubleVector2DTools::Approximate, result.GetLhsClosestPoint(), lhsOrigin, 1e-8);
				ASSERT_APPROXIMATE_USE_FUNCTION(DoubleVector2DTools::Approximate, squaredResult.GetRhsClosestPoint(), rhsOrigin, 1e-8);

				ASSERT_TRUE(squaredResult.GetDistance() <= DoubleVector2DTools::VectorMagnitudeSquared(lhsOrigin + distance.GetZeroThreshold() * lhsDirection - rhsOrigin));
			}
			else
			{
				// 最小值为第二个射线的原点到第一个射线
			     ...
			     DoubleVector2DTools::Approximate(rhsDifference, rhsDirection, 1e-8))
		{
			...
			if (rhsDifference2.IsZero())
			{
				...
				ASSERT_TRUE(squaredResult.GetDistance() <= DoubleVector2DTools::VectorMagnitudeSquared(rhsOrigin + distance.GetZeroThreshold() * rhsDirection - lhsOrigin));
			...
			else
			{
				// 最小值为第一个射线的原点到第二个射线
				ASSERT_APPROXIMATE(squaredResult.GetDistance(), 
					               DoubleVector2DTools::VectorMagnitudeSquared(squaredResult.GetRhsClosestPoint() - lhsOrigin),
								   1e-10);
				ASSERT_APPROXIMATE(squaredResult.GetContactTime(),0.0, 1e-10);
				ASSERT_APPROXIMATE(result.GetDistance(), DoubleVector2DTools::VectorMagnitude(squaredResult.GetRhsClosestPoint() - lhsOrigin), 1e-10);
				ASSERT_APPROXIMATE(result.GetContactTime(), 0.0, 1e-10);
				ASSERT_APPROXIMATE_USE_FUNCTION(DoubleVector2DTools::Approximate, squaredResult.GetLhsClosestPoint(), lhsOrigin, 1e-8);

				ASSERT_TRUE(squaredResult.GetDistance() <= DoubleVector2DTools::VectorMagnitudeSquared(lhsOrigin - rhsOrigin));		
				ASSERT_TRUE(squaredResult.GetDistance() <= DoubleVector2DTools::VectorMagnitudeSquared(squaredResult.GetRhsClosestPoint() + 0.1 * rhsDirection - lhsOrigin));
				ASSERT_TRUE(squaredResult.GetDistance() <= DoubleVector2DTools::VectorMagnitudeSquared(squaredResult.GetRhsClosestPoint() - 0.1 * rhsDirection - lhsOrigin));
			}
		}
		else
		{
			ASSERT_APPROXIMATE_USE_FUNCTION(DoubleVector2DTools::Approximate, lhsDifference, -lhsDirection, 1e-8);
			ASSERT_APPROXIMATE_USE_FUNCTION(DoubleVector2DTools::Approximate, rhsDifference,-rhsDirection, 1e-8);

			ASSERT_APPROXIMATE(squaredResult.GetDistance(), 
				               DoubleVector2DTools::VectorMagnitudeSquared(squaredResult.GetRhsClosestPoint() - squaredResult.GetLhsClosestPoint()),
							   1e-10);
			ASSERT_APPROXIMATE(squaredResult.GetContactTime(),0.0, 1e-10);
			ASSERT_APPROXIMATE(result.GetDistance(), DoubleVector2DTools::VectorMagnitude(squaredResult.GetRhsClosestPoint() - squaredResult.GetLhsClosestPoint()), 1e-10);
			ASSERT_APPROXIMATE(result.GetContactTime(), 0.0, 1e-10);
			
			ASSERT_TRUE(squaredResult.GetDistance() <= DoubleVector2DTools::VectorMagnitudeSquared(lhsOrigin - rhsOrigin));	
			ASSERT_TRUE(squaredResult.GetDistance() <= DoubleVector2DTools::VectorMagnitudeSquared(squaredResult.GetRhsClosestPoint() - lhsOrigin) + 1e-8);
			ASSERT_TRUE(squaredResult.GetDistance() <= DoubleVector2DTools::VectorMagnitudeSquared(squaredResult.GetLhsClosestPoint() - rhsOrigin) + 1e-8);
		...
		DoubleDistanceRay2Ray2 firstEqualDistance(DoubleRay2(lhsOrigin, lhsDirection), DoubleRay2(lhsOrigin + lhsDirection, lhsDirection));
		...
		DoubleDistanceRay2Ray2 secondEqualDistance(DoubleRay2(lhsOrigin, lhsDirection), DoubleRay2(lhsOrigin + lhsDirection, -lhsDirection));
		...
		DoubleDistanceRay2Ray2 thirdEqualDistance(DoubleRay2(lhsOrigin, lhsDirection), DoubleRay2(lhsOrigin - lhsDirection, -lhsDirection));

		squaredResult = thirdEqualDistance.GetSquared();
		result = thirdEqualDistance.Get();

		ASSERT_APPROXIMATE(squaredResult.GetDistance(), DoubleVector2DTools::VectorMagnitudeSquared(lhsDirection), 1e-10);
		ASSERT_APPROXIMATE(squaredResult.GetContactTime(), 0.0, 1e-10);
		ASSERT_APPROXIMATE_USE_FUNCTION(DoubleVector2DTools::Approximate, squaredResult.GetLhsClosestPoint(), lhsOrigin, 1e-8);
		ASSERT_APPROXIMATE_USE_FUNCTION(DoubleVector2DTools::Approximate, squaredResult.GetRhsClosestPoint(), lhsOrigin - lhsDirection, 1e-8);
		ASSERT_APPROXIMATE(result.GetDistance(), DoubleVector2DTools::VectorMagnitude(lhsDirection), 1e-10);
		ASSERT_APPROXIMATE(result.GetContactTime(), 0.0, 1e-10);
		ASSERT_APPROXIMATE_USE_FUNCTION(DoubleVector2DTools::Approximate, result.GetLhsClosestPoint(), lhsOrigin, 1e-8);
		ASSERT_APPROXIMATE_USE_FUNCTION(DoubleVector2DTools::Approximate, result.GetRhsClosestPoint(), lhsOrigin - lhsDirection, 1e-8);

		DoubleDistanceRay2Ray2 firstParallelDistance(DoubleRay2(lhsOrigin, lhsDirection), DoubleRay2(rhsOrigin, lhsDirection));

		squaredResult = firstParallelDistance.GetSquared();
		result = firstParallelDistance.Get();
		double lhsDistanceSquared = DoubleVector2DTools::VectorMagnitudeSquared(lhsOrigin - squaredResult.GetRhsClosestPoint());
		...
		DoubleDistanceRay2Ray2 secondParallelDistance(DoubleRay2(lhsOrigin, lhsDirection), DoubleRay2(rhsOrigin, -lhsDirection));

		squaredResult = secondParallelDistance.GetSquared();
		result = secondParallelDistance.Get();
		lhsDistanceSquared = DoubleVector2DTools::VectorMagnitudeSquared(lhsOrigin - squaredResult.GetRhsClosestPoint());
		rhsDistanceSquared = DoubleVector2DTools::VectorMagnitudeSquared(rhsOrigin - squaredResult.GetLhsClosestPoint());
		double originDistanceSquared = DoubleVector2DTools::VectorMagnitudeSquared(rhsOrigin - lhsOrigin);
	...
	for (auto loop = 0; loop < testLoopCount; ++loop)
	{
			    ...
			    DoubleVector2DTools::Approximate(rhsDifference,-rhsDirection,1e-8))
		{
			...
			if(lhsDifference2.IsZero())
			{
				// 最小值为两个射线的原点距离。
				ASSERT_APPROXIMATE(squaredResult.GetDistance(), 
					               DoubleVector2DTools::VectorMagnitudeSquared(rhsOrigin + t * rhsVelocity - (lhsOrigin + t * lhsVelocity)),
								   1e-10);
				ASSERT_APPROXIMATE(squaredResult.GetContactTime(),t, 1e-10);
				ASSERT_APPROXIMATE(result.GetDistance(), DoubleVector2DTools::VectorMagnitude(rhsOrigin + t * rhsVelocity - (lhsOrigin + t * lhsVelocity)), 1e-10);
				ASSERT_APPROXIMATE(result.GetContactTime(), t, 1e-10);
				ASSERT_APPROXIMATE_USE_FUNCTION(DoubleVector2DTools::Approximate, result.GetLhsClosestPoint(), lhsOrigin + t * lhsVelocity, 1e-8);
				...
				ASSERT_TRUE(squaredResult.GetDistance() <= DoubleVector2DTools::VectorMagnitudeSquared(lhsOrigin + t * lhsVelocity + distance.GetZeroThreshold() * lhsDirection - (rhsOrigin + t * rhsVelocity)));
			}
			else
			{
				// 最小值为第二个射线的原点到第一个射线
			     ...
			     DoubleVector2DTools::Approximate(rhsDifference, rhsDirection, 1e-8))
		{
			...
			if (rhsDifference2.IsZero())
			{
				// 最小值为两个射线的原点距离。
				ASSERT_APPROXIMATE(squaredResult.GetDistance(), 
					               DoubleVector2DTools::VectorMagnitudeSquared((lhsOrigin + t * lhsVelocity) - (rhsOrigin + t * rhsVelocity)),
								   1e-10);
				ASSERT_APPROXIMATE(squaredResult.GetContactTime(),t, 1e-10);
				ASSERT_APPROXIMATE(result.GetDistance(), DoubleVector2DTools::VectorMagnitude((lhsOrigin + t * lhsVelocity) - (rhsOrigin + t * rhsVelocity)), 1e-10);
				ASSERT_APPROXIMATE(result.GetContactTime(), t, 1e-10);
				ASSERT_APPROXIMATE_USE_FUNCTION(DoubleVector2DTools::Approximate, result.GetLhsClosestPoint(), lhsOrigin + t * lhsVelocity, 1e-8);
				ASSERT_APPROXIMATE_USE_FUNCTION(DoubleVector2DTools::Approximate, squaredResult.GetRhsClosestPoint(), rhsOrigin + t * rhsVelocity, 1e-8);

				ASSERT_TRUE(squaredResult.GetDistance() <= DoubleVector2DTools::VectorMagnitudeSquared(rhsOrigin + t * rhsVelocity + distance.GetZeroThreshold() * rhsDirection - (lhsOrigin + t * lhsVelocity)));
			...
			else
			{
				// 最小值为第一个射线的原点到第二个射线
				ASSERT_APPROXIMATE(squaredResult.GetDistance(), 
					               DoubleVector2DTools::VectorMagnitudeSquared(squaredResult.GetRhsClosestPoint() - (lhsOrigin + t * lhsVelocity)),
								   1e-10);
				ASSERT_APPROXIMATE(squaredResult.GetContactTime(),t, 1e-10);
				ASSERT_APPROXIMATE(result.GetDistance(), DoubleVector2DTools::VectorMagnitude(squaredResult.GetRhsClosestPoint() - (lhsOrigin + t * lhsVelocity)), 1e-10);
				ASSERT_APPROXIMATE(result.GetContactTime(), t, 1e-10);
				ASSERT_APPROXIMATE_USE_FUNCTION(DoubleVector2DTools::Approximate, squaredResult.GetLhsClosestPoint(), lhsOrigin + t * lhsVelocity, 1e-8);

				ASSERT_TRUE(squaredResult.GetDistance() <= DoubleVector2DTools::VectorMagnitudeSquared(lhsOrigin + t * lhsVelocity - (rhsOrigin + t * rhsVelocity)));
				ASSERT_TRUE(squaredResult.GetDistance() <= DoubleVector2DTools::VectorMagnitudeSquared(squaredResult.GetRhsClosestPoint() + 0.1 * rhsDirection - (lhsOrigin + t * lhsVelocity)));
				ASSERT_TRUE(squaredResult.GetDistance() <= DoubleVector2DTools::VectorMagnitudeSquared(squaredResult.GetRhsClosestPoint() - 0.1 * rhsDirection - (lhsOrigin + t * lhsVelocity)));
			}
		}
		else
		{
			...
			ASSERT_TRUE(squaredResult.GetDistance() <= DoubleVector2DTools::VectorMagnitudeSquared(lhsOrigin + t * lhsVelocity - (rhsOrigin + t * rhsVelocity)));	
			ASSERT_TRUE(squaredResult.GetDistance() <= DoubleVector2DTools::VectorMagnitudeSquared(squaredResult.GetRhsClosestPoint() - (lhsOrigin + t * lhsVelocity)) + 1e-8);
			ASSERT_TRUE(squaredResult.GetDistance() <= DoubleVector2DTools::VectorMagnitudeSquared(squaredResult.GetLhsClosestPoint() - (rhsOrigin + t * rhsVelocity)) + 1e-8);
	...
	::IntervalTest()
{
	...
	for (auto loop = 0; loop < testLoopCount; ++loop)
	{
		...
		ASSERT_APPROXIMATE(DoubleMath::Sqrt(squaredResult.GetDistance()), result.GetDistance(), 1e-6);
