...
bool System::ConditionVariableCriticalSectionTesting::DoProducerThreadProcedure()
{
    ...
    while (IsFull())
    {
        ASSERT_TRUE(SleepConditionVariableCriticalSection(GetBufferNotFull(), &bufferLock, System::EnumCastUnderlying<WindowsDWord>(MutexWait::Infinite)));
    }
...
bool System::ConditionVariableCriticalSectionTesting::DoConsumerThreadProcedure()
{
    ...
    while (IsEmpty())
    {
        ASSERT_TRUE(SleepConditionVariableCriticalSection(GetBufferNotEmpty(), &bufferLock, System::EnumCastUnderlying<WindowsDWord>(MutexWait::Infinite)));
    }
