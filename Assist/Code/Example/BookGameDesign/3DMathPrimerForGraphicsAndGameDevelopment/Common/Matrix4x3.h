...
namespace ThreeDMathPrimerForGraphicsAndGameDevelopment
{
	...
	public:
		...
		void zeroTranslation() noexcept;
		void setTranslation(const Vector3& d) noexcept;
		void setupTranslation(const Vector3& d) noexcept;
		...
		void setupLocalToParent(const Vector3& pos, const EulerAngles& orient) noexcept;
		void setupLocalToParent(const Vector3& pos, const RotationMatrix& orient) noexcept;
		void setupParentToLocal(const Vector3& pos, const EulerAngles& orient) noexcept;
		void setupParentToLocal(const Vector3& pos, const RotationMatrix& orient) noexcept;

		// Setup the matrix to perform a rotation about a cardinal axis

		void setupRotate(int axis, float theta) noexcept;

		// Setup the matrix to perform a rotation about an arbitrary axis

		void setupRotate(const Vector3& axis, float theta) noexcept;

		// Setup the matrix to perform a rotation, given
		// the angular displacement in quaternion form

		void fromQuaternion(const Quaternion& q) noexcept;

		// Setup the matrix to perform scale on each axis

		void setupScale(const Vector3& s) noexcept;

		// Setup the matrix to perform scale along an arbitrary axis

		void setupScaleAlongAxis(const Vector3& axis, float k) noexcept;

		// Setup the matrix to perform a shear

		void setupShear(int axis, float s, float t) noexcept;

		// Setup the matrix to perform a projection onto a plane passing
		// through the origin

		void setupProject(const Vector3& n) noexcept;

		// Setup the matrix to perform a reflection about a plane parallel
		// to a cardinal plane

		void setupReflect(int axis, float k = 0.0f) noexcept;

		// Setup the matrix to perform a reflection about an arbitrary plane
		// through the origin

		void setupReflect(const Vector3& n) noexcept;
	};
	...
	Matrix4x3& operator*=(const Matrix4x3& a, const Matrix4x3& m) noexcept;
