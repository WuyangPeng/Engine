...
// also when setting the video mode.
	...
	struct RenderVertex
	{
		Vector3	p;	// position
		Vector3	n;	// normal
		float u = 0.0f, v = 0.0f;	// texture mapping coordinate
	};
	...
	public:
		...
		int	getScreenX() const noexcept
		{
			return screenX;
		}
		int	getScreenY() const noexcept
		{
			return screenY;
		}
		...
		void videoSave() noexcept;
		void videoRestore() noexcept;

		// Camera specification

			// Camera position/orientation

		void setCamera(const Vector3& pos, const EulerAngles& orient) noexcept;
		const Vector3& getCameraPos() const noexcept
		{
			return cameraPos;
		}
		const EulerAngles& getCameraOrient() const noexcept
		{
			return cameraOrient;
		}
		...
		void setZoom(float xZoom, float yZoom = 0.0f) noexcept;

		// Near/far clipping planes

		void setNearFarClippingPlanes(float n, float f) noexcept;
		...
		void setWindow(int x1, int y1, int xSize, int ySize) noexcept;
		void setFullScreenWindow() noexcept;
		void getWindow(int* x1, int* y1, int* xSize, int* ySize) noexcept;

		// Reference frame

			// Nested reference frame stack

		void instance(const Vector3& pos, const EulerAngles& orient) noexcept;
		void instancePop() noexcept;

		// Low-level texture cache maintenance functions using names and handles directly

		void resetTextureCache();
		int	findTexture(const char* name) noexcept;
		int	allocTexture(const char* name, int xSize, int ySize) noexcept;
		void freeTexture(int handle) noexcept;
		void setTextureImage(int handle, const unsigned* image) noexcept;
		int	cacheTexture(const char *filename) noexcept;
		...
		void cacheTexture(TextureReference& texture) noexcept;

		// Rendering context management functions

		// Zbuffer mode

		void setDepthBufferMode(bool readEnabled, bool writeEnabled) noexcept;
		void setDepthBufferMode() noexcept
		{
			setDepthBufferMode(true, true);
		} // no parms - set defaults
		bool getDepthBufferRead() const noexcept
		{
			return depthBufferRead;
		}
		bool getDepthBufferWrite() const noexcept
		{
		...
		void setBlendEnable(bool blend = false) noexcept;
		bool getBlendEnable() const noexcept
		{
			return blendEnable;
		}
		void setSourceBlendMode(ESourceBlendMode mode = eSourceBlendModeSrcAlpha) noexcept;
		ESourceBlendMode getSourceBlendMode() const noexcept
		{
			return sourceBlendMode;
		}
		void setDestBlendMode(EDestBlendMode mode = eDestBlendModeInvSrcAlpha) noexcept;
		EDestBlendMode getDestBlendMode() const noexcept
		{
		...
		void setRGB(unsigned int rgb) noexcept; // alpha is ignored
		unsigned getARGB() noexcept
		{
			return constantARGB;
		} // returns global opacity in the alpha channel
		...
		void setFogEnable(bool flag = false) noexcept;
		bool getFogEnable() noexcept
		{
			return fogEnable;
		}
		void setFogColor(unsigned rgb) noexcept;
		unsigned getFogColor() const noexcept
		{
			return fogColor;
		}
		void setFogDistance(float nearFog, float farFog) noexcept;
		float getFogNear() const noexcept
		{
			return fogNear;
		}
		float getFogFar() const noexcept
		{
			return fogFar;
		}
		...
		void setAmbientLightColor(unsigned rgb) noexcept;
		unsigned getAmbientLightColor() const noexcept
		{
			return ambientLightColor;
		}

		void setDirectionalLightVector(const Vector3& v) noexcept;
		const Vector3& getDirectionalLightVector() const noexcept
		{
			return directionalLightVector;
		}

		void setDirectionalLightColor(unsigned rgb) noexcept;
		unsigned getDirectionalLightColor() const noexcept
		{
		...
		void setLightEnable(bool flag = true) noexcept;
		bool getLightEnable() const noexcept
		{
			return lightEnable;
		}
		...
		void setBackfaceMode(EBackfaceMode mode = eBackfaceModeCCW) noexcept;
		EBackfaceMode getBackfaceMode() const noexcept
		{
			return backfaceMode;
		}
		...
		void selectTexture(int handle) noexcept;
		int	getCurrentTexture() const noexcept
		{
		...
		void selectTexture(TextureReference& texture) noexcept;

		// Texture clamping

		void setTextureClamp(bool flag = false) noexcept;
		...
		void clear(int options = kClearFrameBuffer | kClearDepthBuffer) noexcept;

		// Render a triangle mesh

		void renderTriMesh(const RenderVertex* vertexList, int vertexCount, const RenderTri* triList, int triCount) noexcept;
		void renderTriMesh(const RenderVertexL* vertexList, int vertexCount, const RenderTri* triList, int triCount) noexcept;
		void renderTriMesh(const RenderVertexTL* vertexList, int vertexCount, const RenderTri* triList, int triCount) noexcept;

		// 2D primitives.  These use the current constant color

		void dot(int x, int y) noexcept;
		void line(int x1, int y1, int x2, int y2) noexcept;
		void boxFill(int x1, int y1, int x2, int y2) noexcept;
		...
		const Matrix4x3& getWorldToCameraMatrix() noexcept
		{
			return worldToCameraMatrix;
		}
		const Matrix4x3& getModelToCameraMatrix() noexcept;
		const Matrix4x3& getModelToWorldMatrix() noexcept;
		...
		int	computeOutCode(const Vector3& p) noexcept;
		...
		int	projectPoint(const Vector3& p, Vector3* result) noexcept;
	...
	private:
		...
		void updateModelToWorldMatrix() noexcept;
		void computeClipMatrix() noexcept;
		void getModelToClipMatrix() noexcept;
		void freeAllTextures() noexcept;
	};
