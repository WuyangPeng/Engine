...
AncientBooks::GeographicContainer::ConstGeographicSharedPtr AncientBooks::GeographicContainer::GetGeographic(int key) const
{
    ...
    if (const auto iter = std::ranges::lower_bound(geographic, std::make_shared<GeographicBase>(key), function);
        iter != geographic.cend() && (*iter)->GetKey() == key)
    {
        return *iter;
    }
    else
...
int AncientBooks::GeographicContainer::GetContainerSize() const
{
    USER_CLASS_IS_VALID_CONST_9;

    return boost::numeric_cast<int>(geographic.size());
}
