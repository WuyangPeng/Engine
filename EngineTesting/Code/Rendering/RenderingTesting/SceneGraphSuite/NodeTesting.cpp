...
// ◊˜’ﬂ£∫≈ÌŒ‰—Ù£¨≈ÌÍ ∂˜£¨≈ÌÍ ‘Û
//
// “˝«Ê≤‚ ‘∞Ê±æ£∫0.0.0.3 (2019/09/05 11:00)
...
{
    auto firstNode = Node::Create();

    ASSERT_EQUAL(firstNode->GetNumControllers(), 0);

    ControllerInterfaceSharedPtr secondNode(Node::Create());
    ...
    ControllerInterfaceSharedPtr thirdNode(Node::Create());
...
void Rendering::NodeTesting::CopyTest()
{
    // 	Node firstNode;
    //
    // 	ASSERT_EQUAL(firstNode.GetNumControllers(),0);
    ...
    // 	Node* secondPtr = CoreTools::StaticCast<Node>(fourthNode.GetData());
    //
...
void Rendering::NodeTesting::TransformTest()
{
    // 	Node firstNode;
    // 	ControllerInterfaceSharedPtr secondNode(new Node);
    // 	firstNode.AttachController(secondNode);
...
void Rendering::NodeTesting::ChildTest()
{
    ...
    // 	CoreTools::ObjectInterfaceSharedPtr firstNode(new Node);
    //
...
void Rendering::NodeTesting::StreamTest()
{
    // 	CoreTools::OutTopLevel outTopLevel;
    // 	CoreTools::ObjectInterfaceSharedPtr firstNode(new Node);
    //
