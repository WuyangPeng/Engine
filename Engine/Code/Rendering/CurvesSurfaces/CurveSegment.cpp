...
Mathematics::AVectorf Rendering::CurveSegment
	::Tangent (float u) const
{
...
Mathematics::AVectorf Rendering::CurveSegment
	::Normal(float u) const
{
    AVector velocity = PU(u);
    AVector acceleration = PUU(u);
	float VDotV = Dot(velocity,velocity);
	float VDotA = Dot(velocity,acceleration);
...
void Rendering::CurveSegment
	::GetFrame (float u, APoint& position, AVector& tangent, AVector& normal, AVector& binormal) const
{
    position = P(u);
    AVector velocity = PU(u);
    AVector acceleration = PUU(u);
	...
	::Curvature (float u) const
{
    ...
    if (speedSqr >= Mathematics::Mathf::sm_ZeroTolerance)
    {
		...
		float denom = Mathematics::Mathf::Pow(speedSqr, 1.5f);
        return numer/denom;
    }
    else
    {
...
Rendering::CurveSegment
	::CurveSegment (LoadConstructor value)
...
uint64_t Rendering::CurveSegment
