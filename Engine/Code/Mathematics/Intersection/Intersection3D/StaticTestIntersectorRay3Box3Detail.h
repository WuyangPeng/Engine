	...
	::Test()
{
    ...
    if (ADdU[0] > mBox.GetExtent(0) && DdU[0]*WdU[0] >= Math<Real>::GetValue(0))
    {
	...
	if (ADdU[1] > mBox.GetExtent(1) && DdU[1] * WdU[1] >= Math<Real>::GetValue(0))
    {
		this->SetIntersectionType(IntersectionType::Empty);
        return;
    }
	...
	if (ADdU[2] > mBox.GetExtent(2) && DdU[2] * WdU[2] >= Math<Real>::GetValue(0))
    {
		this->SetIntersectionType(IntersectionType::Empty);
		return;
    }
