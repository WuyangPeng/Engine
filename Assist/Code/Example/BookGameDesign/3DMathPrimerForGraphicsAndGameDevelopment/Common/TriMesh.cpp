...
namespace ThreeDMathPrimerForGraphicsAndGameDevelopment
{
	...
	void TriMesh::freeMemory() noexcept
	{
	...
	void TriMesh::render() const noexcept
	{
	...
	void TriMesh::computeBoundingBox() noexcept
	{
	...
	void TriMesh::fromEditMesh(const EditTriMesh& mesh)
	{
		...
		for (i = 0; i < triCount; ++i)
		{
			...
			d->index[0] = boost::numeric_cast<unsigned short>(s->v[0].index);
			d->index[1] = boost::numeric_cast<unsigned short>(s->v[1].index);
			d->index[2] = boost::numeric_cast<unsigned short>(s->v[2].index);
		}
	...
	void TriMesh::toEditMesh([[maybe_unused]] const EditTriMesh& mesh) const noexcept
	{
