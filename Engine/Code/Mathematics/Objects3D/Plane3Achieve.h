...
Mathematics::Plane3<Real>::Plane3(const Vector3Type& normal, const Vector3Type& point, Real epsilon) noexcept
    : normal{ normal }, constant{ Vector3ToolsType::DotProduct(normal, point) }, epsilon{ epsilon }
{
    MATHEMATICS_SELF_CLASS_IS_VALID_1;
}
    ...
    : normal{}, constant{}, epsilon{ epsilon }
{
    ...
    normal = Vector3ToolsType::UnitCrossProduct(edge1, edge2);
    constant = Vector3ToolsType::DotProduct(normal, triangle.GetVertex(0));
    ...
    : normal{}, constant{}, epsilon{ epsilon }
{
    ...
    normal = Vector3ToolsType::UnitCrossProduct(edge1, edge2);
    constant = Vector3ToolsType::DotProduct(normal, point0);
...
Real Mathematics::Plane3<Real>::DistanceTo(const Vector3Type& point) const noexcept
{
    ...
    return Vector3ToolsType::DotProduct(normal, point) - constant;
}
...
void Mathematics::Plane3<Real>::SetPlane(const Vector3Type& newNormal, const Vector3Type& point) noexcept
{
    ...
    constant = Vector3ToolsType::DotProduct(normal, point);
}
...
Mathematics::Plane3<Real> Mathematics::Plane3<Real>::GetMove(Real t, const Vector3Type& velocity) const noexcept
{
    ...
    const auto movedConstant = GetConstant() + t * Vector3ToolsType::DotProduct(GetNormal(), velocity);
