...
namespace Mathematics
{
    ...
    public:
        ...
        // 1.0e-4精度
        static Real FastInvSinRoughCalculation(Real value) noexcept(g_Assert < 3 || g_MathematicsAssert < 3);
        // 1.0e-6精度（float）、1.0e-7精度（double）
        static Real FastInvSinPreciseCalculation(Real value) noexcept(g_Assert < 3 || g_MathematicsAssert < 3);

        // 输入必须在范围 [0,1]

        // 1.0e-4精度
        static Real FastInvCosRoughCalculation(Real value) noexcept(g_Assert < 3 || g_MathematicsAssert < 3);
        // 1.0e-6精度（float）、1.0e-7精度（double）
        static Real FastInvCosPreciseCalculation(Real value) noexcept(g_Assert < 3 || g_MathematicsAssert < 3);
