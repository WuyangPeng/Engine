...
bool ImplicitSurface<Real>::ComputePrincipalCurvatureInfo (const Vector3D<Real>& pos, Real& curv0, Real& curv1, Vector3D<Real>& dir0,Vector3D<Real>& dir1)
{
    ...
    // mean curvature
	auto meanCurv = (Real{0.5})*invLength3*(fxx*(fyfy + fzfz) + fyy*(fxfx + fzfz) + fzz*(fxfx + fyfy) - ((Real)2)*(fxy*fxfy + fxz*fxfz +  fyz*fyfz));

    // Gaussian curvature
	auto gaussCurv = invLength4*(fxfx*(fyy*fzz - fyz*fyz) + fyfy*(fxx*fzz - fxz*fxz) + fzfz*(fxx*fyy - fxy*fxy) + ((Real)2)*(fxfy*(fxz*fyz - fxy*fzz) + fxfz*(fxy*fyz - fxz*fyy)  + fyfz*(fxy*fxz - fxx*fyz)));
	...
	auto m00 = ((-(Real)1 + fxfx*invLength2)*fxx)*invLength + (fxfy*fxy)*invLength3 + (fxfz*fxz)*invLength3;
	auto m01 = ((-(Real)1 + fxfx*invLength2)*fxy)*invLength +  (fxfy*fyy)*invLength3 + (fxfz*fyz)*invLength3;
	auto m02 = ((-(Real)1 + fxfx*invLength2)*fxz)*invLength + (fxfy*fyz)*invLength3 + (fxfz*fzz)*invLength3;
	auto m10 = (fxfy*fxx)*invLength3 + ((-(Real)1 + fyfy*invLength2)*fxy)*invLength + (fyfz*fxz)*invLength3;
	auto m11 = (fxfy*fxy)*invLength3 + ((-(Real)1 + fyfy*invLength2)*fyy)*invLength + (fyfz*fyz)*invLength3;
	auto m12 = (fxfy*fxz)*invLength3 + ((-(Real)1 + fyfy*invLength2)*fyz)*invLength + (fyfz*fzz)*invLength3;
	auto m20 = (fxfz*fxx)*invLength3 + (fyfz*fxy)*invLength3 +  ((-(Real)1 + fzfz*invLength2)*fxz)*invLength;
	auto m21 = (fxfz*fxy)*invLength3 + (fyfz*fyy)*invLength3 +  ((-(Real)1 + fzfz*invLength2)*fyz)*invLength;
	auto m22 = (fxfz*fxz)*invLength3 + (fyfz*fyz)*invLength3 +  ((-(Real)1 + fzfz*invLength2)*fzz)*invLength;
