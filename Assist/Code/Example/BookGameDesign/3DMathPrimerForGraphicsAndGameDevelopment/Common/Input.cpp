...
namespace ThreeDMathPrimerForGraphicsAndGameDevelopment
{
	...
	bool Keyboard::isPressed(int keyCode) noexcept
	{
	...
	bool Keyboard::press(int keyCode) noexcept
	{
		if (keyCode < 0 || keyCode >= kNumKeys) return false; //bail and fail
		const bool OldState = m_eKeyDown[keyCode];
	...
	bool Keyboard::debounce(int keyCode) noexcept
	{
		if (keyCode < 0 || keyCode >= kNumKeys) return false; //bail and fail
		const bool OldState = m_eKeyDown[keyCode];
	...
	void Keyboard::debounceAll() noexcept
	{
	...
	bool Keyboard::getNextEvent(KeyEvent *event) noexcept
	{
	...
	void Keyboard::queueEvent(const KeyEvent &event) noexcept
	{
	...
	void Keyboard::clearEventQueue() noexcept
	{
		keyEventFilled = 0;
	}
	...
	int	Keyboard::getModifiers() noexcept
	{
