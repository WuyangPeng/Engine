...
// ◊˜’ﬂ£∫≈ÌŒ‰—Ù£¨≈ÌÍ ∂˜£¨≈ÌÍ ‘Û
// 
// “˝«Ê≤‚ ‘∞Ê±æ£∫0.0.0.2 (2019/08/26 17:29)
...
void Mathematics::RationalVector3Testing
	::MainTest()
{
	...
	::ConstructionTest()
{
	...
	RationalVector3<7> secondVector(firstVector);
	...
	SignRational<7> thirdRational(firstRandomDistribution(generator), firstRandomDistribution(generator));

	RationalVector3<7> thirdVector(firstRational, secondRational, thirdRational);
	...
	RationalVector3<7> fifthVector(fourthVector);
	...
	::CalculateTest()
{
	...
	SignRational<15> fifthRational(firstRandomDistribution(generator),firstRandomDistribution(generator));

	SignRational<15> sixthRational(firstRandomDistribution(generator),firstRandomDistribution(generator));

	RationalVector3<15> firstVector(firstRational,secondRational,thirdRational);
	RationalVector3<15> secondVector(fourthRational,fifthRational,sixthRational);

	SignRational<15> squaredLength = firstRational * firstRational + secondRational * secondRational +thirdRational * thirdRational;	

	ASSERT_EQUAL(squaredLength, firstVector.SquaredLength());

	SignRational<15> dot = firstRational * fourthRational + secondRational * fifthRational +thirdRational * sixthRational;	

	ASSERT_EQUAL(dot, Dot(firstVector, secondVector));

	RationalVector3<15> thirdVector(secondRational * sixthRational -  thirdRational * fifthRational,
	...
	SignRational<15> seventhRational(firstRandomDistribution(generator), firstRandomDistribution(generator));

	SignRational<15> eighthRational(firstRandomDistribution(generator),firstRandomDistribution(generator));

	SignRational<15> ninthRational(firstRandomDistribution(generator),firstRandomDistribution(generator));

	RationalVector3<15> fourthVector(seventhRational,eighthRational,ninthRational);
	...
	::OperatorTest()
{
	...
	SignRational<12> fifthRational(firstRandomDistribution(generator),firstRandomDistribution(generator));

	SignRational<12> sixthRational(firstRandomDistribution(generator), firstRandomDistribution(generator));

	RationalVector3<12> firstVector(firstRational,secondRational,thirdRational);	
	RationalVector3<12> secondVector(fourthRational,fifthRational,sixthRational);	
	...
	::CompareTest()
{
	...
	RationalVector3<3> firstVector(SignRational<3>(0), firstRational, secondRational);
	RationalVector3<3> secondVector(SignRational<3>(1), secondRational, firstRational);
