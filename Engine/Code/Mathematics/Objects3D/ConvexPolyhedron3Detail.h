...
Mathematics::ConvexPolyhedron3<Real>::ConvexPolyhedron3(int numVertices, Vector3DPtr vertices, int numTriangles, typename IntPtr indices, PlanePtr planes)
...
template <typename Real>
Mathematics::ConvexPolyhedron3<Real>::~ConvexPolyhedron3()
{
    MATHEMATICS_SELF_CLASS_IS_VALID_3;
}
...
template <typename Real>
typename const Mathematics::ConvexPolyhedron3<Real>::PlanePtr Mathematics::ConvexPolyhedron3<Real>::GetPlanes() const
{
...
template <typename Real>
typename const Mathematics::ConvexPolyhedron3<Real>::Plane3& Mathematics::ConvexPolyhedron3<Real>::GetPlane(int index) const
{
    MATHEMATICS_CLASS_IS_VALID_CONST_3;
    MATHEMATICS_ASSERTION_1(m_SharingTriangles.empty(), "需要更新平面！");
...
void Mathematics::ConvexPolyhedron3<Real>::SetVertex(int index, const Vector3D& vertex)
{
    ...
    for (auto j = 0; j < numTriangles; ++j)
    {
        ...
        const auto v0 = *indices++;
        const auto v1 = *indices++;
        const auto v2 = *indices++;
...
template <typename Real>
bool Mathematics::ConvexPolyhedron3<Real>::IsUpdatePlanes() const
{
