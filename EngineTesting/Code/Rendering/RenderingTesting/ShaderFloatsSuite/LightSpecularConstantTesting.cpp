...
#include "LightSpecularConstantTesting.h"
...
Rendering::LightSpecularConstantTesting::LightSpecularConstantTesting(const OStreamShared& stream)
    : ParentType{ stream }
{
    RENDERING_SELF_CLASS_IS_VALID_1;
}

CLASS_INVARIANT_PARENT_IS_VALID_DEFINE(Rendering, LightSpecularConstantTesting)

void Rendering::LightSpecularConstantTesting::DoRunUnitTest()
{
    ASSERT_NOT_THROW_EXCEPTION_0(MainTest);
}

void Rendering::LightSpecularConstantTesting::MainTest()
{
...
void Rendering::LightSpecularConstantTesting::InitTest()
{
    ...
    for (int loop = 0; loop < GetTestLoopCount(); ++loop)
    {
        ...
        const LightSpecularConstant secondShaderFloat(firstLight);
        ...
        LightSpecularConstant thirdShaderFloat(firstLight);
...
void Rendering::LightSpecularConstantTesting::CopyTest()
{
    std::default_random_engine generator;
    const std::uniform_real<float> firstFloatRandomDistribution(-100.0f, 100.0f);
    std::uniform_real<float> secondFloatRandomDistribution(0.0f, 1.0f);
    ...
    for (int loop = 0; loop < GetTestLoopCount(); ++loop)
    {
        ...
        LightSpecularConstant secondShaderFloat(firstShaderFloat);
...
void Rendering::LightSpecularConstantTesting::StreamTest() noexcept
{
}

void Rendering::LightSpecularConstantTesting::UpdateTest() noexcept
{
}
