...
namespace Physics
{
    ...
    LCPPolyDist<Dimension, FVector, DVector>::LCPPolyDist(int numPoints1, FVector* points1, int numFaces1, ITuple* faces1, int numPoints2,
          ...
          mVerifyMinDifference(verifyMinDifference), mRandomWidth(randomWidth)
    {
        ...
        mPoints1 = nullptr;  //NEW1<DVector>(mNumPoints1);
        ...
        mPoints2 = nullptr;  //NEW1<DVector>(mNumPoints2);
        ...
        mA1 = nullptr;  // NEW1<DVector>(mNumFaces1);
        mB1 = nullptr;  //NEW1<double>(mNumEquations);
        GenerateHalfSpaceDescription(mNumPoints1, mPoints1, mNumFaces1, mFaces1, mA1, mB1);

        mA2 = nullptr;  //NEW1<DVector>(mNumFaces2);
        mB2 = nullptr;  //NEW1<double>(mNumEquations);
    ...
    template <int Dimension, typename FVector, typename DVector>
    LCPPolyDist<Dimension, FVector, DVector>::LCPPolyDist(int numFaces1, FVector* A1, float* B1, int numFaces2, FVector* A2, float* B2,
          ...
          mRandomWidth(randomWidth)
    {
        ...
        mA1 = nullptr;  // NEW1<DVector>(mNumFaces1);
        mB1 = nullptr;  //NEW1<double>(mNumFaces1);
        ...
        mA2 = nullptr;  //NEW1<DVector>(mNumFaces2);
        mB2 = nullptr;  //NEW1<double>(mNumFaces2);
    ...
    {
        int i;
        double* Q = nullptr;  // NEW1<double>(mNumEquations);
        double** M = nullptr;  //NEW2<double>(mNumEquations, mNumEquations);
        ...
        while (processing)
        {
            ...
            double* wResult = nullptr;  // NEW1<double>(mNumEquations);
            int lcpStatusCode;
...
//         DELETE1(Q);
    ...
    void LCPPolyDist<Dimension, FVector, DVector>::PrintMatrices(double** M, double* Q)
    {
        ...
        mLog << "There are " << mNumEquations
             << " rows and columns in this problem." << std::endl
             << std::endl;
    ...
    {
        mLog << std::endl
             << "This polyhedron has " << numPoints;
        mLog << " vertices." << std::endl
             << "They are:" << std::endl;
        int i, j;
    ...
    template <int Dimension, typename FVector, typename DVector>
    void LCPPolyDist<Dimension, FVector, DVector>::SetupTestPoints(int index, int numFaces, int numPoints, const DVector& closest, const DVector* A,
    ...
    {
        DVector* testPoints1 = nullptr;  //NEW1<DVector>(13);
        DVector* testPoints2 = nullptr;  //NEW1<DVector>(13);
        ...
        if (distance > diffSq)
        {
            ...
            if ((distance - diffSq > mVerifyMinDifference) && (iLine != 12 || jLine != 12))
            {
