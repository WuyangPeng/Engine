书·其他语言·数据库模块单元测试

1. 数据库系统基础教程（A First Course in Database Systems）
（第3版）

官方网站：
http://infolab.stanford.edu/~ullman/fcdb.html

★
作者：Jeffrey D.Ullman、Jennifer Widom

第1章 数据库系统世界
第一部分 关系数据库模型
	第2章 关系数据模型
	第3章 关系数据库设计理论
	第4章 高级数据库模型
第二部分 关系数据库程序设计
	第5章 代数和逻辑查询语言
	第6章 数据库语言SQL
	第7章 约束与触发器
	第8章 视图与索引
	第9章 服务器环境下的SQL
	第10章 关系数据库的新课题
第三部分 半结构化数据的建模和程序设计
	第11章 半结构化数据模型
	第12章 XML程序设计语言

2.   数据库系统概念（Database System Concepts）
（第6版）

官方网站：
http://www.db-book.com、 http://www.mhhe.com/silberschatz

★
作者：Abraham Silberschatz、Henry F.Korth、S.Sudarshan

第1章 引言
第一部分 关系数据库
	第2章 关系模型介绍
	第3章 SQL
	第4章 中级SQL
	第5章 高级SQL
	第6章 形式化关系查询语言
第二部分 数据库设计
	第7章 数据库设计和E-R模型
	第8章 关系数据库设计
	第9章 应用设计和开发
第三部分 数据存储和查询
	第10章 存储和文件结构
	第11章 索引与散列
	第12章 查询处理
	第13章 查询优化
第四部分 事务管理
	第14章 事务
	第15章 并发控制
	第16章 恢复系统
第五部分 系统体系结构
	第17章 数据库系统体系结构
	第18章 并行数据库
	第19章 分布式数据库
第六部分 数据仓库、数据挖掘与信息检索
	第20章 数据仓库与数据挖掘
	第21章 信息检索
第七部分 特种数据库
	第22章 基于对象的数据库
	第23章 XML
第八部分 高级主题
	第24章 高级应用开发
	第25章 时空数据和移动性
	第26章 高级事务处理
第九部分 实例研究
	第27章 PostgreSQL
	第28章 Oracle
	第29章 旧MDB2UniversalDatabase
	第30章 Microsoft SQLServer

3.  数据库系统实现 （Database System Implementation）
（第2版） 

官方网站：
http://infolab.stanford.edu/~ullman/fcdb.html

★
作者： Hector Garcia-Molina、Jeffrey D.Ullman、Jennifer Widom

第1章 DBMS系统概述
第2章 辅助存储管理
第3章 索引结构
第4章 查询执行
第5章 查询编译器
第6章 系统故障对策
第7章 并发控制
第8章 再论事务管理
第9章 并行与分布式数据库
第10章 信息集成 
第11章 数据挖掘
第12章 数据库系统与互联网

4.  数据库重构（Refactoring Databases Evolutionary Database Design）
 
★★
作者：Scott W.Ambler、Pramod J.Sadalage 

第1章 演进式数据库开发
第2章 数据库重构
第3章 数据库重构过程
第4章 部署到生产环境
第5章 数据库重构策略
第6章 结构重构
第7章 数据质量重构
第8章 参照完整性重构
第9章 架构重构
第10章 方法重构
第11章 转换

5. 数据库设计与关系理论（Database Design & Relational Theory）

☆
作者：C.J.Date

官方网站：
http://www.justsql.co.uk/

第一部分 设置环境
	第1章 篇首语
	第2章 预备知识
第二部分 函数依赖、BOYCE/CODD范式及相关事宜
	第3章 规范化：一些通则
	第4章 函数依赖和BCNF（非正式的）
	第5章 函数依赖和BCNF（正式的）
	第6章 保持函数依赖
	第7章 FD公理化
	第8章 反规范化
第三部分 连接依赖、第五范式及其他相关事项
	第9章 连接依赖及5NF（非正式的）
	第10章 连接依赖及5NF（正式的）
	第11章 隐式依赖关系
	第12章 多值依赖和4NF
	第13章 额外的范式
第四部分 正交
	第14章 正交
第五部分 冗余
	第15章 我们需要更多的科学 
第六部分 附录
	附录A 主键是良好的，但不是必需的
	附录B 冗余回顾
	附录C 重要论文回顾
	附录D 习题答案

6. 七周七数据库（Seven Databases in Seven Weeks）

☆
作者：Eric Redmond、Jim R.Wilson

第1章 概述
第2章 PostgreSQL
第3章 Riak
第4章 HBase
第5章 MongoDB
第6章 CouchDB
第7章 Neo4j
第8章 Redis
第9章 结束语

7.数据库系统全书（Database Systems The Complets Book）

★★
作者： Hector Garcia-Molina、Jeffrey D.Ullman、Jennifer Widom

官方网站：
http://infolab.stanford.edu/~ullman/dscb.html

第1章 数据库的系统世界
第2章 实体-联系数据模型
第3章 关系数据模型
第4章 其他数据类型
第5章 关系代数
第6章 数据库语言sql
第7章 约束和触发器 
第8章 模式中的存储过程
第9章 面向对象查询语言
第10章 逻辑查询语言
第11章 数据存储
第12章 数据元素的表示
第13章 索引结构
第14章 多维索引和位图索引
第15章 查询执行
第16章 查询编译器
第17章 系统故障对策
第18章 并发控制
第19章 再论事务管理
第20章 信息集成

8.数据挖掘概念与技术（Data Mining Concepts and Techniques）
（第3版） 

☆
作者：Jiawei Han、Micheline Kamber、Jian Pei

官方网站：
http://www.booksite.mkp.com/datamining3e/ 、http://www.cs.uiuc.edu/~hanj/bk3

第1章 引论
第2章 认识数据
第3章 数据预处理
第4章 数据仓库与联机分析处理
第5章 数据立方体技术
第6章 挖掘频繁模式、关联和相关性：基本概念和方法
第7章 高级模式挖掘
第8章 分类：基本概念
第9章 分类：高级方法
第10章 聚类分析：基本概念和方法
第11章 高级聚类分析
第12章 离群点检测
第13章 数据挖掘的发展趋势和研究前沿

9. 数据挖掘：实用机器学习工具与技术（Data Mining:Practical Machine Learning Tools and Techniques）
（第3版） 

☆
作者：Ian H. Witten、Eibe Frank

官方网站：
http://www.cs.waikato.ac.nz/ml/weka

第一部分 数据挖掘简介
	第1章 绪论
	第2章 输入：概念、实例和属性
	第3章 输出：知识表达
	第4章 算法：基本方法
	第5章 可信度：评估学习结果
第二部分 高级数据挖掘
	第6章 实现：真正的机器学习方案
	第7章 数据转换
	第8章 集成学习
	第9章 继续：扩展和应用		 
第三部分 Weka机器学习平台
	第10章 Weka简介
	第11章 Explorer界面
	第12章 Knowledge Flow界面
	第13章 Experimenter界面
	第14章 命令行界面
	第15章 嵌入式机器学习
	第16章 编写新学习方案
	第17章 Weka Explorer的辅导练习

10.事务处理：概念与技术（Transaction Processing:Concepts and Techniques）

☆
作者：Jim Gray、Andreas Reuter

第一部分 事务处理基础
	第1章 概述
	第2章 计算机系统基本术语
第二部分 容错基础知识
	第3章 容错
第三部分 面向事务的计算
	第4章 事务模型
	第5章 事务处理监控器：概述
	第6章 事务处理监控器
第四部分 并发控制 
	第7章 隔离性的概念
	第8章 锁的实现
第五部分 恢复
	第9章 日志管理程序
	第10章 事务管理器概念
	第11章 事务管理器结构
	第12章 高级事务管理器主题
第六部分 事务型文件系统：一个资源管理器实例
	第13章 文件和缓冲区管理
	第14章 面向元组的文件系统
	第15章 存取路径
第七部分 系统概览
	第16章 TP系统概览
第八部分 附录
	附录A 参考文献
	附录B 数据结构和接口
	附录C 词汇表

11.社交网站的数据挖掘与分析（Mining the Social Web）
（第2版） 

☆
作者：Mattbew A.Russell

第一部分 社交网络导引
	序幕
	第1章 挖掘Twitter：探索热门话题、发现人们的谈论内容等
	第2章 挖掘Facebook：分析粉丝页面、查看好友关系等
	第3章 挖掘Linkedln：分组职位、聚类同行类
	第4章 挖掘Google+：计算文档相似度、提取搭配等
	第5章 挖掘网页：使用自然语言处理理解人类语言、总结博客内容等
	第6章 挖掘邮箱：分析谁和谁说什么以及说的频率等
	第7章 挖掘GitHub：检查软件协同习惯、构建兴趣图谱等
	第8章 挖掘带标记语义网：提取微格式、推断资源描述框架等
第二部分 Twitter实用指南
	第9章 Twitter实用指南
第三部分 附录
	附录A 关于本书虚拟机体验的信息
	附录B OAuth入门
	附录C Python和IPython Notebook的使用技巧
