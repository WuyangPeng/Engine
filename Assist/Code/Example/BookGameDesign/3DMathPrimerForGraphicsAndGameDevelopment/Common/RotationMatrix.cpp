...
namespace ThreeDMathPrimerForGraphicsAndGameDevelopment
{
	...
	void RotationMatrix::identity() noexcept
	{
	...
	void RotationMatrix::setup(const EulerAngles& orientation) noexcept
	{
	...
	void RotationMatrix::fromInertialToObjectQuaternion(const Quaternion& q) noexcept
	{
	...
	void RotationMatrix::fromObjectToInertialQuaternion(const Quaternion& q) noexcept
	{
	...
	Vector3	RotationMatrix::inertialToObject(const Vector3& v) const noexcept
	{

		// Perform the matrix multiplication in the "standard" way.

		return Vector3(m11*v.x + m21 * v.y + m31 * v.z, m12*v.x + m22 * v.y + m32 * v.z, m13*v.x + m23 * v.y + m33 * v.z);
	}
	...
	Vector3	RotationMatrix::objectToInertial(const Vector3& v) const noexcept
	{

		// Multiply by the transpose

		return Vector3(m11*v.x + m12 * v.y + m13 * v.z, m21*v.x + m22 * v.y + m23 * v.z, m31*v.x + m32 * v.y + m33 * v.z);
	}
}