///	Copyright (c) 2010-2021
///	Threading Core Render Engine
///
///	作者：彭武阳，彭晔恩，彭晔泽
///	联系作者：94458936@qq.com
///
///	标准：std:c++17
///	引擎辅助测试版本：0.7.1.1 (2021/03/06 21:41)

#ifndef BOOK_ADVANCED_CROSS_PLATFORM_DEVELOPMENT_IN_CPP_PREFACE_SUITE_H
#define BOOK_ADVANCED_CROSS_PLATFORM_DEVELOPMENT_IN_CPP_PREFACE_SUITE_H

#include "CrossPlatformDevelopmentInCppFibonacciTesting.h"
#include "CrossPlatformDevelopmentInCppHelloWorldTesting.h"
#include "CrossPlatformDevelopmentInCppOperatingSystemInterfaceTesting.h"

#endif  // BOOK_ADVANCED_CROSS_PLATFORM_DEVELOPMENT_IN_CPP_PREFACE_SUITE_H