...
AssistTools::LoadBmpUseAlpha ::LoadBmpUseAlpha(const System::TChar* name, unsigned char alpha, int& width, int& height, unsigned char*& texels)
{
    ...
    if (dibSection.dsBm.bmBitsPixel == 24)
    {
        ...
        texels = nullptr;  //    NEW1<unsigned char>(numBytes);
    ...
    {
        texels = nullptr;  //     NEW1<unsigned char>(4 * width * height);
