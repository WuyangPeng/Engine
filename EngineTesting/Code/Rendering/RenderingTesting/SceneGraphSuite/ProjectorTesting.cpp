...
void Rendering::ProjectorTesting::AxesAlignBoundingBoxTest()
{
    ...
    for (int loop = 0; loop < GetTestLoopCount(); ++loop)
    {
        ...
        Camera::AxesAlignBoundingBox2D resultAabb = Mathematics::AxesAlignBoundingBox2D(xmin, xmax, ymin, ymax);

        ASSERT_TRUE(Mathematics::Vector2ToolsF::Approximate(resultAabb.GetMinPoint(), aabb.GetMinPoint(), 1e-5f));
        ASSERT_TRUE(Mathematics::Vector2ToolsF::Approximate(resultAabb.GetMaxPoint(), aabb.GetMaxPoint(), 1e-5f));
    }
}

void Rendering::ProjectorTesting::StreamTest()
{
    std::default_random_engine generator;
    std::uniform_real<float> firstFloatRandomDistribution(-100.0f, 100.0f);
