...
（1）资源基类：实现资源基类。★★★
（2）顶点缓冲区：实现顶点缓冲区的读取和写入。★★★
（3）索引缓冲区：实现索引缓冲区的读取和写入。★★★
（4）纹理对象：实现1D、2D、3D和Cube纹理对象。 
（5）渲染目标：实现渲染目标的数据封装。
（6）顶点格式：实现顶点格式的数据封装。★★★
