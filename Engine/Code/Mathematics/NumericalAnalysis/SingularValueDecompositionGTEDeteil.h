...
// ◊˜’ﬂ£∫≈ÌŒ‰—Ù£¨≈ÌÍ ∂˜£¨≈ÌÍ ‘Û
// 
// “˝«Ê∞Ê±æ£∫0.0.0.2 (2019/07/10 09:11)
...
namespace Mathematics
{
	...
	void SingularValueDecompositionGTE<Real>::GetU(Real* uMatrix) const
	{
		...
		std::fill(uMatrix, uMatrix + mNumRows * mNumRows, Real{});
	...
	void SingularValueDecompositionGTE<Real>::GetV(Real* vMatrix) const
	{
		...
		// Start with the identity matrix.
		std::fill(vMatrix, vMatrix + mNumCols * mNumCols, Real{});
	...
	void SingularValueDecompositionGTE<Real>::Bidiagonalize()
	{
		...
		for (int i = 0, ip1 = 1; i < mNumCols; ++i, ++ip1)
		{
			...
			{
				Real& u1 = mUVector[i];
				Real sgn = (u1 >= Real{} ? (Real)1 : (Real)-1);
			...
			if (i < mNumCols - 2)
			{
				...
				if (length > Real{})
				{
					...
					Real sgn = (v1 >= Real{} ? (Real)1 : (Real)-1);
	...
	void SingularValueDecompositionGTE<Real>::DoGolubKahanStep(int imin, int imax)
	{
		...
		Real f0 = (imax >= (Real)1 ? mSuperdiagonal[imax - 1] : Real{});
		...
		Real sgn = (dif >= Real{} ? (Real)1 : (Real)-1);
		Real a01sqr = a01 * a01;
