...
namespace ThreeDMathPrimerForGraphicsAndGameDevelopment
{
	...
	void Quaternion::setToRotateAboutX(float theta) noexcept
	{
	...
	void Quaternion::setToRotateAboutY(float theta) noexcept
	{
	...
	void Quaternion::setToRotateAboutZ(float theta) noexcept
	{
	...
	void Quaternion::setToRotateAboutAxis(const Vector3& axis, float theta) noexcept
	{
	...
	void Quaternion::setToRotateObjectToInertial(const EulerAngles& orientation) noexcept
	{
	...
	void Quaternion::setToRotateInertialToObject(const EulerAngles& orientation) noexcept
	{
	...
	Quaternion Quaternion::operator *(const Quaternion& a) const noexcept
	{
		Quaternion result;
	...
	Quaternion& Quaternion::operator *=(const Quaternion& a) noexcept
	{
	...
	void Quaternion::normalize() noexcept
	{
		...
		const float	mag = sqrt(w*w + x * x + y * y + z * z);
	...
	float Quaternion::getRotationAngle() const noexcept
	{

		// Compute the half angle.  Remember that w = cos(theta / 2)

		const float thetaOver2 = safeAcos(w);
	...
	Vector3	Quaternion::getRotationAxis() const noexcept
	{
		...
		const float	oneOverSinThetaOver2 = 1.0f / sqrt(sinThetaOver2Sq);
	...
	Quaternion slerp(const Quaternion& q0, const Quaternion& q1, float t) noexcept
	{
	...
	Quaternion pow(const Quaternion& q, float exponent) noexcept
	{
