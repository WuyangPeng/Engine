...
namespace ThreeDMathPrimerForGraphicsAndGameDevelopment
{
    ...
    static int vertexCompareByMark(const void* va, const void* vb) noexcept
    {
        // Cast pointers

        const EditTriMesh::Vertex* a = static_cast<const EditTriMesh::Vertex*>(va);
        const EditTriMesh::Vertex* b = static_cast<const EditTriMesh::Vertex*>(vb);
    ...
    static int triCompareByMaterial(const void* va, const void* vb) noexcept
    {
        // Cast pointers

        const EditTriMesh::Tri* a = static_cast<const EditTriMesh::Tri*>(va);
        const EditTriMesh::Tri* b = static_cast<const EditTriMesh::Tri*>(vb);
    ...
    static bool skipLine(FILE* f) noexcept
    {
        for (;;)
        {
            const int c = fgetc(f);
    ...
    void EditTriMesh::Vertex::setDefaults() noexcept
    {
        // Reset everything
    ...
    void EditTriMesh::Tri::setDefaults() noexcept
    {
        // Reset everything
    ...
    bool EditTriMesh::Tri::isDegenerate() const noexcept
    {
        return (v[0].index == v[1].index) || (v[1].index == v[2].index) || (v[0].index == v[2].index);
    }
    ...
    int EditTriMesh::Tri::findVertex(int vertexIndex) const noexcept
    {
        // Search vertices.  Let's unroll the loop here...

        if (v[0].index == vertexIndex)
            return 0;
        if (v[1].index == vertexIndex)
            return 1;
        if (v[2].index == vertexIndex)
            return 2;
    ...
    void EditTriMesh::Material::setDefaults() noexcept
    {
        // Reset everything
    ...
    void EditTriMesh::Part::setDefaults() noexcept
    {
        // Reset everything
    ...
    void EditTriMesh::OptimizationParameters::setDefaults() noexcept
    {
    ...
    void EditTriMesh::OptimizationParameters::setEdgeAngleToleranceInDegrees(float degrees) noexcept
    {
    ...
    EditTriMesh::EditTriMesh() noexcept
    {
        construct();
    }
    ...
    EditTriMesh::EditTriMesh(const EditTriMesh& x) noexcept
    {
    ...
    void EditTriMesh::construct() noexcept
    {
        // Reset the lists
    ...
    EditTriMesh& EditTriMesh::operator=(const EditTriMesh& src) noexcept
    {
        ...
        if (src.triCount() > 0)
        {
            ...
            const int bytes = src.triCount() * sizeof(*tList);

            // Allocate it.  We don't use setVertexCount(), since
            // that initializes the list, which we don't need

            tList = static_cast<Tri*>(::malloc(bytes));
            if (tList == nullptr)
            {
                ABORT("Out of memory");
            }
    ...
    EditTriMesh::Vertex& EditTriMesh::vertex(int vertexIndex) noexcept
    {
    ...
    const EditTriMesh::Vertex& EditTriMesh::vertex(int vertexIndex) const noexcept
    {
        assert(vertexIndex >= 0);
        assert(vertexIndex < vCount);
        return vList[vertexIndex];
    ...
    EditTriMesh::Tri& EditTriMesh::tri(int triIndex) noexcept
    {
        assert(triIndex >= 0);
    ...
    const EditTriMesh::Tri& EditTriMesh::tri(int triIndex) const noexcept
    {
        assert(triIndex >= 0);
        assert(triIndex < tCount);
        return tList[triIndex];
    ...
    EditTriMesh::Material& EditTriMesh::material(int materialIndex) noexcept
    {
    ...
    const EditTriMesh::Material& EditTriMesh::material(int materialIndex) const noexcept
    {
        assert(materialIndex >= 0);
        assert(materialIndex < mCount);
        return mList[materialIndex];
    ...
    EditTriMesh::Part& EditTriMesh::part(int partIndex) noexcept
    {
        assert(partIndex >= 0);
    ...
    const EditTriMesh::Part& EditTriMesh::part(int partIndex) const noexcept
    {
        assert(partIndex >= 0);
        assert(partIndex < pCount);
        return pList[partIndex];
    ...
    void EditTriMesh::empty() noexcept
    {
        // Free vertices
    ...
    void EditTriMesh::setVertexCount(int vc) noexcept
    {
        assert(vc >= 0);
        ...
        if (vc > vCount)
        {
            ...
            if (vc > vAlloc)
            {
                ...
                vAlloc = vc * 4 / 3 + 10;
                auto temp = static_cast<Vertex*>(::realloc(vList, vAlloc * sizeof(*vList)));
                if (temp)
                {
                    vList = temp;
                }
        ...
        else if (vc < vCount)
        {
            ...
            {
                Tri* t = &tri(i);
                if ((t->v[0].index >= vc) || (t->v[1].index >= vc) || (t->v[2].index >= vc))
                {
    ...
    void EditTriMesh::setTriCount(int tc) noexcept
    {
        assert(tc >= 0);
        ...
        if (tc > tCount)
        {
            ...
            if (tc > tAlloc)
            {
                ...
                tAlloc = tc * 4 / 3 + 10;
                auto temp = static_cast<Tri*>(::realloc(tList, tAlloc * sizeof(*tList)));
                if (temp)
                {
                    tList = temp;
                }
    ...
    void EditTriMesh::setMaterialCount(int mc) noexcept
    {
        assert(mc >= 0);
        ...
        if (mc > mCount)
        {
            ...
            auto temp = static_cast<Material*>(::realloc(mList, mc * sizeof(*mList)));
            if (temp)
            {
                mList = temp;
            }
    ...
    void EditTriMesh::setPartCount(int pc) noexcept
    {
        assert(pc >= 0);
        ...
        if (pc > pCount)
        {
            ...
            auto temp = static_cast<Part*>(::realloc(pList, pc * sizeof(*pList)));
            if (temp)
            {
                pList = temp;
            }
    ...
    int EditTriMesh::addTri() noexcept
    {
    ...
    int EditTriMesh::addTri(const Tri& t) noexcept
    {
        // Fetch index of the new one we will add
    ...
    int EditTriMesh::addVertex() noexcept
    {
        // Fetch index of the new one we will add
    ...
    int EditTriMesh::addVertex(const Vertex& v) noexcept
    {
        // Fetch index of the new one we will add
    ...
    int EditTriMesh::dupVertex(int srcVertexIndex) noexcept
    {
        // Fetch index of the new one we will add
    ...
    int EditTriMesh::addMaterial(const Material& m) noexcept
    {
        // Grow list
    ...
    int EditTriMesh::addPart(const Part& p) noexcept
    {
        // Grow list
    ...
    void EditTriMesh::markAllVertices(int mark) noexcept
    {
    ...
    void EditTriMesh::markAllTris(int mark) noexcept
    {
        for (int i = 0; i < triCount(); ++i)
        {
            tri(i).mark = mark;
    ...
    void EditTriMesh::markAllMaterials(int mark) noexcept
    {
        for (int i = 0; i < materialCount(); ++i)
        {
    ...
    void EditTriMesh::markAllParts(int mark) noexcept
    {
        for (int i = 0; i < partCount(); ++i)
        {
            part(i).mark = mark;
    ...
    void EditTriMesh::deleteVertex(int vertexIndex) noexcept
    {
        ...
        --vCount;
        const auto temp = (vCount - vertexIndex);
        memmove(&vList[vertexIndex], &vList[vertexIndex + 1], temp * sizeof(*vList));
    ...
    void EditTriMesh::deleteTri(int triIndex) noexcept
    {
        // Check index.  Warn in debug build, don't crash release
        ...
        --tCount;
        const auto temp = (tCount - triIndex);
        memmove(&tList[triIndex], &tList[triIndex + 1], temp * sizeof(*tList));
    }
    ...
    void EditTriMesh::deleteMaterial(int materialIndex) noexcept
    {
        // Check index.  Warn in debug build, don't crash release
        ...
        --mCount;
        const auto temp = (mCount - materialIndex);
        memmove(&mList[materialIndex], &mList[materialIndex + 1], temp * sizeof(*mList));
    ...
    void EditTriMesh::deletePart(int partIndex) noexcept
    {
        // Check index.  Warn in debug build, don't crash release
        ...
        --pCount;
        const auto temp = (pCount - partIndex);
        memmove(&pList[partIndex], &pList[partIndex + 1], temp * sizeof(*pList));
    ...
    void EditTriMesh::deleteUnusedMaterials() noexcept
    {
    ...
    void EditTriMesh::deleteEmptyParts() noexcept
    {
    ...
    void EditTriMesh::deleteMarkedTris(int mark) noexcept
    {
    ...
    void EditTriMesh::deleteDegenerateTris() noexcept
    {
    ...
    void EditTriMesh::detachAllFaces() noexcept
    {
        ...
        const int newVertexCount = triCount() * 3;

        // Allocate a new vertex list

        Vertex* newVertexList = static_cast<Vertex*>(::malloc(newVertexCount * sizeof(Vertex)));
    ...
    void EditTriMesh::transformVertices(const Matrix4x3& m) noexcept
    {
        for (int i = 0; i < vertexCount(); ++i)
        {
            vertex(i).p *= m;
    ...
    void EditTriMesh::extractParts(EditTriMesh* meshes) noexcept
    {
    ...
    void EditTriMesh::extractOnePartOneMaterial(int partIndex, int materialIndex, EditTriMesh* result) noexcept
    {
    ...
    void EditTriMesh::computeOneTriNormal(int triIndex) noexcept
    {
        computeOneTriNormal(tri(triIndex));
    }

    void EditTriMesh::computeOneTriNormal(Tri& t) noexcept
    {
    ...
    void EditTriMesh::computeTriNormals() noexcept
    {
        for (int i = 0; i < triCount(); ++i)
        {
    ...
    void EditTriMesh::computeVertexNormals() noexcept
    {
    ...
    AABB3 EditTriMesh::computeBounds() const noexcept
    {
    ...
    void EditTriMesh::optimizeVertexOrder(bool removeUnusedVertices) noexcept
    {
    ...
    void EditTriMesh::sortTrisByMaterial() noexcept
    {
    ...
    void EditTriMesh::weldVertices([[maybe_unused]] const OptimizationParameters& opt) noexcept
    {
        // !FIXME!
    }
    ...
    void EditTriMesh::copyUvsIntoVertices() noexcept
    {
        ...
        for (int triIndex = 0; triIndex < triCount(); ++triIndex)
        {
            ...
            for (int i = 0; i < 3; ++i)
            {
                ...
                const int vIndex = triPtr->v[i].index;
                ...
                if ((vPtr->u == triPtr->v[i].u) && (vPtr->v == triPtr->v[i].v))
                {
                ...
                for (int newIndex = 0; newIndex < vertexCount(); ++newIndex)
                {
                    ...
                    if ((newPtr->p != vPtr->p) || (newPtr->normal != vPtr->normal))
                    {
                        continue;
                    }
                    ...
                    if ((newPtr->u == triPtr->v[i].u) && (newPtr->v == triPtr->v[i].v))
                    {
    ...
    void EditTriMesh::optimizeForRendering() noexcept
    {
        ...
        for (int partIndex = 0; partIndex < numParts; ++partIndex)
        {
            ...
            for (int i = 0; i < part(partIndex).mark; ++i)
            {
                ...
                if (fscanf(f, "%d , %d , %f , %f , %d , %f , %f , %d , %f , %f\n",
                           ...
                           &t->v[2].index, &t->v[2].u, &t->v[2].v) != 10)
                {
    ...
    void EditTriMesh::exportS3d([[maybe_unused]] const char* filename) noexcept
    {
    }
    ...
    void EditTriMesh::validityCheck() noexcept
    {
        char errMsg[256];
    ...
    bool EditTriMesh::validityCheck([[maybe_unused]] const char* returnErrMsg) noexcept
    {
        return true;
    }

    bool EditTriMesh::failed(FILE* f) noexcept
    {
        empty();
        if (f != nullptr)
        {
    ...
    bool EditTriMesh::corrupt(char* returnErrMsg, FILE* f) noexcept
    {
