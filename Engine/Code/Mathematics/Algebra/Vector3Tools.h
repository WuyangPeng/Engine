...
namespace Mathematics
{
    ...
    public:
        ...
        NODISCARD static Vector3Orthonormalize Orthonormalize(const ContainerType& vectors, const Real epsilon = Math::GetZeroTolerance());

        // 输入值nonzeroVector（W）必须是一个非零向量。
        // 返回值是一个标准正交基{U，V, W}。
        // 返回的W为通过此函数正则化的nonzeroVector。
        // 如果你已知W是单位长度，使用GenerateComplementBasis来计算U和V。
        NODISCARD static Vector3OrthonormalBasis GenerateOrthonormalBasis(const Vector3& nonzeroVector, const Real epsilon = Math::GetZeroTolerance());

        // 输入值nonzeroVector（W）必须是一个单位向量。
        // 返回值中的向量{U,V}是单位长度且互相垂直，
        // 且{U,V,W}是一个标准正交基。
        NODISCARD static Vector3OrthonormalBasis GenerateComplementBasis(const Vector3& unitVector, const Real epsilon = Math::GetZeroTolerance());
