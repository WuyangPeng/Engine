...
{
    ASSERT_NOT_THROW_EXCEPTION_0(FormatLastErrorUseBufferTest);
}

void System::FormatErrorMessageUseBufferTesting::FormatLastErrorUseBufferTest()
{
    ...
    for (auto flag = WindowError::Success; flag <= WindowError::AppHang; ++flag)
    {
        ...
        {
            const auto size = FormatErrorMessage(flag, buffer.data(), boost::numeric_cast<WindowsDWord>(buffer.size()));
            ASSERT_LESS(0u, size);
