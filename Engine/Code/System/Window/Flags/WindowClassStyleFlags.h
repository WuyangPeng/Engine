...
#ifndef SYSTEM_WINDOW_WINDOW_CLASS_STYLE_FLAGS_H
#define SYSTEM_WINDOW_WINDOW_CLASS_STYLE_FLAGS_H
...
namespace System
{
    ...
    enum class WindowClassStyle
    {
        ...
        // 表示该窗口类是一个应用程序全局类，允许调用CreateWindow或CreateWindowEx函数和传递给RegisterClass的instance参数不同。
        // 如果不指定该风格则这两个instance必须相同。
        GlobalWindow = CS_GLOBALCLASS,

        // 使一个窗口有阴影效果。当效果被激活时，通过SPI_SETDROPSHADOW关闭。
        // (该特性对于小且短暂的窗口比较合适，比如菜单用来强调自己和其他窗口的z顺序关系。)
    ...
    ENUM_ORABLE_OPERATOR_DEFINE(WindowClassStyle)
}

#endif  // SYSTEM_WINDOW_WINDOW_CLASS_STYLE_FLAGS_H
