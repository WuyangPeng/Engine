...
void Rendering::LoadVisualEffectTesting::VertexShaderTest()
{
    ...
    ConstVertexShaderSharedPtr sixthVertexShader(std::make_shared<VertexShader>("VertexShader2", 4, 3, 4, 6));
    ...
    for (auto i = 0; i < 6; ++i)
    {
        ...
        ASSERT_APPROXIMATE_USE_FUNCTION(Approximate<float>, firstVertexShader->GetBorderColor(i),
                                        Colour<float>(static_cast<float>(i) / 10.0f,
                                                      static_cast<float>(i + 1) / 10.0f,
                                                      static_cast<float>(i + 2) / 10.0f,
                                                      static_cast<float>(i + 3) / 10.0f),
                                        1e-8f);
...
void Rendering::LoadVisualEffectTesting::PixelShaderTest()
{
    ...
    ConstPixelShaderSharedPtr sixthPixelShader(std::make_shared<PixelShader>("PixelShader2", 2, 8, 6, 7));
...
void Rendering::LoadVisualEffectTesting::AlphaStateTest()
{
    ...
    ConstAlphaStateSharedPtr sixthAlphaState(std::make_shared<AlphaState>(CoreTools::DisableNotThrow::Disable));
...
void Rendering::LoadVisualEffectTesting::CullStateTest()
{
    ...
    auto firstCullState = visualEffect->GetCullState(0, 0);
    auto secondCullState = visualEffect->GetCullState(0, 1);
    auto thirdCullState = visualEffect->GetCullState(1, 0);
    auto fourthCullState = visualEffect->GetCullState(1, 1);
    auto fifthCullState = visualEffect->GetCullState(1, 2);

#include STSTEM_WARNING_PUSH
#include SYSTEM_WARNING_DISABLE(26414)

    auto sixthCullState = std::make_shared<CullState>(CoreTools::DisableNotThrow::Disable);
...
void Rendering::LoadVisualEffectTesting::DepthStateTest()
{
    ...
    auto firstDepthState = visualEffect->GetDepthState(0, 0);
    auto secondDepthState = visualEffect->GetDepthState(0, 1);
    auto thirdDepthState = visualEffect->GetDepthState(1, 0);
    auto fourthDepthState = visualEffect->GetDepthState(1, 1);
    auto fifthDepthState = visualEffect->GetDepthState(1, 2);

#include STSTEM_WARNING_PUSH
#include SYSTEM_WARNING_DISABLE(26414)

    auto sixthDepthState = std::make_shared<DepthState>(CoreTools::DisableNotThrow::Disable);
...
void Rendering::LoadVisualEffectTesting::OffsetStateTest()
{
    ...
    auto firstOffsetState = visualEffect->GetOffsetState(0, 0);
    auto secondOffsetState = visualEffect->GetOffsetState(0, 1);
    auto thirdOffsetState = visualEffect->GetOffsetState(1, 0);
    auto fourthOffsetState = visualEffect->GetOffsetState(1, 1);
    auto fifthOffsetState = visualEffect->GetOffsetState(1, 2);
    auto sixthOffsetState = std::make_shared<OffsetState>(CoreTools::DisableNotThrow::Disable);
...
void Rendering::LoadVisualEffectTesting::StencilStateTest()
{
    ...
    ConstStencilStateSharedPtr sixthStencilState(std::make_shared<StencilState>(CoreTools::DisableNotThrow::Disable));
...
#include SYSTEM_WARNING_DISABLE(26414)
    ...
    ConstWireStateSharedPtr sixthWireState(std::make_shared<WireState>(CoreTools::DisableNotThrow::Disable));
