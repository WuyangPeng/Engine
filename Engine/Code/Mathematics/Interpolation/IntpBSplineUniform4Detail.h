...
namespace Mathematics
{
	...
	IntpBSplineUniform4<Real>::IntpBSplineUniform4(int degree, const int* dim,Real* data)
	...
	Real IntpBSplineUniform4<Real>::operator() (Real x, Real y, Real z, Real w)
	{
		...
		{
			result += mPoly[0][index[0]] * mPoly[1][index[1]] *	mPoly[2][index[2]] * mPoly[3][index[3]] * mInter[k];
	...
	template <typename Real>
	Real IntpBSplineUniform4<Real>::operator() (int dx, int dy, int dz, int dw,	Real x, Real y, Real z, Real w)
	{
		mBase[0] = (int)Math<Real>::Floor(x);
		mBase[1] = (int)Math<Real>::Floor(y);
		mBase[2] = (int)Math<Real>::Floor(z);
