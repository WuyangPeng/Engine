...
namespace Mathematics
{
	...
	public:
		...
		Real ComputeSegmentLength (const VariableLengthVector<Real>& point0, const VariableLengthVector<Real>& point1);

		// Compute the total length of the polyline.  The lengths of the segments
		// are computed relative to the metric tensor.
		Real ComputeTotalLength (int quantity, const VariableLengthVector<Real>* path);
		...
		void ComputeGeodesic (const VariableLengthVector<Real>& end0, const VariableLengthVector<Real>& end1, int& quantity, VariableLengthVector<Real>*& path);
		...
		Real ComputeSegmentCurvature (const VariableLengthVector<Real>& point0,
			const VariableLengthVector<Real>& point1);

		// Compute the total curvature of the polyline.  The curvatures of the
		// segments are computed relative to the metric tensor.
		Real ComputeTotalCurvature (int quantity, const VariableLengthVector<Real>* path);

	protected:
		// Support for ComputeSegmentCurvature.
		Real ComputeIntegrand (const VariableLengthVector<Real>& pos,
			const VariableLengthVector<Real>& der);
		...
		int mDimension;
		VariableMatrix<Real> mMetric;
		VariableMatrix<Real> mMetricInverse;
		VariableMatrix<Real>* mChristoffel1;
		VariableMatrix<Real>* mChristoffel2;
		VariableMatrix<Real>* mMetricDerivative;
