...
void Rendering::VertexFormatTesting
	::MainTest()
{ 
	...
	::BaseTest()
{
	...
	{
		ASSERT_EQUAL(firstVertexFormat->GetStreamIndex(i), 0u);
	...
	ASSERT_EQUAL(firstVertexFormat->GetStreamIndex(0), 0u);
	ASSERT_EQUAL(firstVertexFormat->GetOffset(0), 0u);
	...
	ASSERT_EQUAL(firstVertexFormat->GetUsageIndex(0),2u);
}
	...
	::StreamTest()
{
	...
	for (unsigned i = 0; i < firstVertexFormatElement.size(); ++i)
	{
