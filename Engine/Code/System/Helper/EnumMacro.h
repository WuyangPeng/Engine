...
#define ENUM_MULTIPLICATION_OPERATOR_DEFINE(className) \
		template<typename T> constexpr T operator*(className lhs, T rhs) noexcept { \
		return System::EnumCastUnderlying(lhs) * rhs; } \
		template<typename T> constexpr T operator*(T lhs, NumericalValueSymbol rhs) noexcept { \
        return rhs * lhs; }
...
#define ENUM_LEFT_SHIFTABLE_OPERATOR_DEFINE(className) \
        inline std::ostream& operator<<(std::ostream& os, className lhs) noexcept \
        { os << System::EnumCastUnderlying(lhs); return os; }

#define ENUM_RIGHT_SHIFTABLE_OPERATOR_DEFINE(className) \
        inline std::istream& operator>>(std::istream& is, className& lhs) noexcept \
	    { using UnderlyingType = std::underlying_type<className>::type; \
        UnderlyingType value{};is >> value;lhs = std::move(static_cast<className>(value));return is; }

#define ENUM_LEFT_SHIFTABLE_OPERATOR_WCHAR_DEFINE(className) \
        inline std::wostream& operator<<(std::wostream& os, className lhs) noexcept \
        { os << System::EnumCastUnderlying(lhs); return os; }

#define ENUM_RIGHT_SHIFTABLE_OPERATOR_WCHAR_DEFINE(className) \
        inline std::wistream& operator>>(std::wistream& is, className& lhs) noexcept \
	    { using UnderlyingType = std::underlying_type<className>::type; \
        UnderlyingType value{};is >> value;lhs = std::move(static_cast<className>(value));return is; }
