...
// 		}
		...
		Vector3 operator /(float a) const noexcept
		{
			const float	oneOverA = 1.0f / a; // NOTE: no check for divide by zero here
		...
		void normalize() noexcept
		{
			...
			{ // check for divide-by-zero
				const float oneOverMag = 1.0f / sqrt(magSq);
				x *= oneOverMag;
				y *= oneOverMag;
				z *= oneOverMag;
			}
		...
		float operator *(const Vector3& a) const noexcept
		{
			return x * a.x + y * a.y + z * a.z;
		}
	};
	...
	inline float vectorMag(const Vector3& a) noexcept
	{
	...
	inline Vector3 crossProduct(const Vector3& a, const Vector3& b) noexcept
	{
	...
	inline Vector3 operator *(float k, const Vector3& v) noexcept
	{
	...
	inline float distanceSquared(const Vector3& a, const Vector3& b) noexcept
	{
		const float dx = a.x - b.x;
		const float dy = a.y - b.y;
		const float dz = a.z - b.z;
	...
	constexpr Vector3 kZeroVector(0.0f, 0.0f, 0.0f);
}
/////////////////////////////////////////////////////////////////////////////
