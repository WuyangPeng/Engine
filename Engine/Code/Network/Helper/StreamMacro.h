...
#define NETWORK_FACTORY_DEFINE(namespaceName, className)                                                                                   \
    ...
    {                                                                                                                                      \
        Network::MessageInterfaceSharedPtr                                                                                                 \
            object{ std::make_shared<className>(LoadConstructor::ConstructorLoader, messageID) };                                          \
        object->Load(source);                                                                                                              \
        return object;                                                                                                                     \
    }
...
#define NETWORK_DEFAULT_STREAM_DEFINE(namespaceName, className)                                                             \
    namespaceName::className::className(LoadConstructor value, int64_t messageID) noexcept : ParentType{ value, messageID } \
    {                                                                                                                       \
...
#define NETWORK_END_STREAM_LOAD(source)                                                                 \
    ...
    const auto usedLoad = endLoad - beginLoad + CORE_TOOLS_STREAM_SIZE(int32_t{});                      \
