...
#define UNIT_TEST_SUBCLASS_DECLARE_USE_PARENT_TYPE(className,parentType) \
		using ClassType = className;using ParentType = parentType; explicit className(std::ostream* osPtr = &std::cout); virtual ~className()
...
#define UNIT_TEST_SUBCLASS_COMPLETE_DECLARE(className) \
		UNIT_TEST_SUBCLASS_DECLARE(className);CLASS_INVARIANT_VIRTUAL_OVERRIDE_DECLARE

#define UNIT_TEST_SUBCLASS_COMPLETE_DECLARE_USE_PARENT_TYPE(className,parentType) \
		UNIT_TEST_SUBCLASS_DECLARE_USE_PARENT_TYPE(className,parentType);CLASS_INVARIANT_VIRTUAL_OVERRIDE_DECLARE
...
#define UNIT_TEST_SUBCLASS_CONSTRUCTOR_DEFINE(namespaceName,className) \
	    namespaceName::className::className(std::ostream* osPtr) :ParentType{ osPtr } { SELF_CLASS_IS_VALID_0; } 
...
#define UNIT_TEST_SUBCLASS_COMPLETE_DEFINE_USE_PARAMETER_0(namespaceName,className) \
		namespaceName::SYSTEM_CONCATENATOR(className,Testing)::SYSTEM_CONCATENATOR(className,Testing)(std::ostream* osPtr) \
...
#define UNIT_TEST_DO_RUN_DEFINE(namespaceName,className,suiteName) \
	    int namespaceName::className::DoRun() { m_SuitePtr = std::make_shared<Suite>("【"##suiteName##"】单元测试套件",GetOStream()); \
...
#define ADD_TEST_BEGIN(suite,suiteName) \
	    auto osPtr = GetOStream(); auto printRun = IsPrintRun(); Suite suite("【"##suiteName##"组件】单元测试套件",osPtr,printRun)

#define SUITE_ADD_TEST(suite,testing) \
	    AddTest((#suite),(suite),(#testing),(std::make_shared<testing>(osPtr)))

#define SUITE_ADD_TEST_USE_PARAMETER_1(suite,testing,parameter) \
	    AddTest((#suite),(suite),(#testing),(std::make_shared<testing>(parameter,osPtr)))

#define SUITE_ADD_TEST_USE_PARAMETER_2(suite,testing,parameter1,parameter2) \
	    AddTest((#suite),(suite),(#testing),(std::make_shared<testing>(parameter1,parameter2,osPtr)))
...
#define CMAIN_FUNCTION_TESTING_HELPER_SUBCLASS_DECLARE(className) \
		...
		using Suite = CoreTools::Suite;	public:	className(int argc,char* argv[]); virtual ~className()    

#define CMAIN_FUNCTION_TESTING_HELPER_SUBCLASS_COMPLETE_DECLARE(className) \
		CMAIN_FUNCTION_TESTING_HELPER_SUBCLASS_DECLARE(className); CLASS_INVARIANT_VIRTUAL_OVERRIDE_DECLARE
