...
// ◊˜’ﬂ£∫≈ÌŒ‰—Ù£¨≈ÌÍ ∂˜£¨≈ÌÍ ‘Û
// 
// “˝«Ê≤‚ ‘∞Ê±æ£∫0.0.0.3 (2019/09/06 15:23)

#include "LightDiffuseConstantTesting.h"
...
UNIT_TEST_SUBCLASS_COMPLETE_DEFINE(Rendering, LightDiffuseConstantTesting) 
#include SYSTEM_WARNING_DISABLE(26440)
#include SYSTEM_WARNING_DISABLE(26446)
#include SYSTEM_WARNING_DISABLE(26409)
#include SYSTEM_WARNING_DISABLE(26496)
void Rendering::LightDiffuseConstantTesting
	::MainTest()
{ 
	...
	::InitTest()
{
	...
	for (int loop = 0; loop < GetTestLoopCount(); ++loop)
	{
		...
		LightDiffuseConstant firstShaderFloat(firstLight);
		...
		const LightDiffuseConstant secondShaderFloat(firstLight);
		...
		LightDiffuseConstant thirdShaderFloat(firstLight);
	...
	::CopyTest()
{
	...
	for (int loop = 0; loop < GetTestLoopCount(); ++loop)
	{
		...
		LightDiffuseConstant secondShaderFloat(firstShaderFloat);
...
// 		LightDiffuseConstantSharedPtr firstShaderFloat(new LightDiffuseConstant(firstLight));
// 		
// 		for (int i = 0; i < 4;++i)
// 		{	 
...
// 		
// 		LightDiffuseConstantSharedPtr secondShaderFloat =
// 			inTopLevel[0].PolymorphicDowncastObjectSharedPtr<LightDiffuseConstantSharedPtr>();
// 		
...
// 
// 		ASSERT_APPROXIMATE(firstLight->GetDiffuse().GetRed(), firstShaderFloat[0], 1e-8f);
// 		ASSERT_APPROXIMATE(firstLight->GetDiffuse().GetGreen(), firstShaderFloat[1], 1e-8f);
// 		ASSERT_APPROXIMATE(firstLight->GetDiffuse().GetBlue(), firstShaderFloat[2], 1e-8f);
// 		ASSERT_APPROXIMATE(firstLight->GetDiffuse().GetAlpha(), firstShaderFloat[3], 1e-8f);
// 	}
}
