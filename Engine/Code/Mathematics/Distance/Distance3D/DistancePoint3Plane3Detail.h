...
Mathematics::DistancePoint3Plane3<Real>
...
template <typename Real>
const Mathematics::Vector3D<Real>& Mathematics::DistancePoint3Plane3<Real>
	::GetPoint() const
{
	return mPoint;
}
...
template <typename Real>
const Mathematics::Plane3<Real>& Mathematics::DistancePoint3Plane3<Real>
	::GetPlane() const
{
	return mPlane;
}
...
template <typename Real>
typename const Mathematics::DistancePoint3Plane3<Real>::DistanceResult Mathematics::DistancePoint3Plane3<Real>
	::GetSquared() const
{
	auto signedDistance = Vector3DTools::DotProduct(mPlane.GetNormal(), mPoint) - mPlane.GetConstant();

	return DistanceResult{ Math::FAbs(signedDistance), Math<Real>::GetValue(0), mPoint, mPoint - signedDistance * mPlane.GetNormal() };
}
	...
	::GetSquared(Real t, const Vector3D& lhsVelocity, const Vector3D& rhsVelocity) const
{
	...
	return DistancePoint3Plane3<Real>{ movedPoint, movedPlane }.GetSquared();
}
