	...
	::Test()
{
    ...
    int onConeSide = 0;
    Real p0Test = Math<Real>::GetZero(), p1Test = Math<Real>::GetZero(), p2Test = Math<Real>::GetZero();
        ...
        if (c2 < Math<Real>::GetZero())
        {
            ...
            if (onConeSide & 1)
            {
                if (onConeSide & 2)
                {
                    ...
                    if (Math<Real>::GetZero() <= c1 && c1 <= -c2 && c1*c1 >= p0Test*c2)
                    {
						this->SetIntersectionType(IntersectionType::Other);
						return;
                    }
                ...
                else
                {
                    ...
                    if (Math<Real>::GetZero() <= c1 && c2*AdD0 <= c1*AdE &&  c1*c1 >= p0Test*c2)
                    {
						this->SetIntersectionType(IntersectionType::Other);
						return;
                    }
    ...
    if (onConeSide > 0)
    {
        ...
        if (NdA >= Math<Real>::GetZero())
        {
            if (NcUdE0 <= Math<Real>::GetZero())
            {
				NcUdE1 = Vector3DTools::DotProduct(NcU,edge1);
                if (NcUdE1 >= Math<Real>::GetZero())
                {
