...
Mathematics::EllipseByArcs2<Real>
	::EllipseByArcs2( Real begin, Real end, int numArcs )
	:m_Points(numArcs + 1),m_Circle(numArcs)
{
	MATHEMATICS_ASSERTION_2(2 <= m_Circle.size(), "必须至少指定两个圆弧\n");
	MATHEMATICS_ASSERTION_2(Real{} <= begin, "begin必须大于或等于0\n");
	MATHEMATICS_ASSERTION_2(Real{} <= end, " end必须大于或等于0\n");
...
{ 
	...
	m_Points[0] = Vector2D{ begin, Real{} };
	m_Points[numArcs] = Vector2D{ Real{}, end };
	...
	{
		ScribeCircle2Circumscribe<Real> circumscribe{ m_Points[index - 1], m_Points[index], m_Points[index + 1] };
...
{
	MATHEMATICS_CLASS_IS_VALID_CONST_1;
	MATHEMATICS_ASSERTION_0(0 <= index && index < static_cast<int>(m_Circle.size()), "索引错误！");

	Arc2 arc{ m_Circle[index].GetCenter(), m_Circle[index].GetRadius(), m_Points[index], m_Points[index + 1] };
