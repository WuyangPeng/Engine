...
#ifndef PHYSICS_FLUID_FLUID2DA_DETAIL_H
#define PHYSICS_FLUID_FLUID2DA_DETAIL_H

#include "Fluid2Da.h"

#if !defined(PHYSICS_EXPORT_TEMPLATE) || defined(PHYSICS_INCLUDED_FLUID2DA_DETAIL)
...
Physics::Fluid2Da<Real>::Fluid2Da(Real x0, Real y0, Real x1, Real y1, Real dt,
...
template <typename Real>
Physics::Fluid2Da<Real>::~Fluid2Da()
{
    EXCEPTION_TRY
    {
//         DELETE1(mX);
...
template <typename Real>
void Physics::Fluid2Da<Real>::Initialize()
{
...
template <typename Real>
void Physics::Fluid2Da<Real>::DoSimulationStep()
{
    UpdateDensitySource();
...
template <typename Real>
void Physics::Fluid2Da<Real>::UpdateDensitySource()
{
    for (int j = 1; j < mJMax; ++j)
    {
        for (int i = 1; i < mIMax; ++i)
    ...
    for (int iter = 0; iter < mNumGaussSeidelIterations; ++iter)
    {
        for (int j = 1; j < mJMax; ++j)
        {
            for (int i = 1; i < mIMax; ++i)
            {
                ...
                mDensity1[j][i] = mDenGamma0 * mDensity0[j][i] + mDenGammaX * sumX + mDenGammaY * sumY;
            }
        }
        UpdateDensityBoundary();
    }
...
template <typename Real>
void Physics::Fluid2Da<Real>::UpdateDensityAdvection()
{
    for (int j = 1; j < mJMax; ++j)
    {
        for (int i = 1; i < mIMax; ++i)
...
template <typename Real>
void Physics::Fluid2Da<Real>::UpdateVelocitySource()
{
    for (int j = 1; j < mJMax; ++j)
    {
        for (int i = 1; i < mIMax; ++i)
        {
            mVelocity1[j][i] = mVelocity0[j][i] + mDt * SourceVelocity(mTime, mX[i], mY[j], i, j);
        }
    }
...
template <typename Real>
void Physics::Fluid2Da<Real>::UpdateVelocityDiffusion()
{
    for (int iter = 0; iter < mNumGaussSeidelIterations; ++iter)
    {
        for (int j = 1; j < mJMax; ++j)
        {
            for (int i = 1; i < mIMax; ++i)
            {
                mVelocity1[j][i] = mVelGamma0 * mVelocity0[j][i] + mVelGammaX * (mVelocity1[j][i + 1] + mVelocity1[j][i - 1]) +
...
template <typename Real>
void Physics::Fluid2Da<Real>::UpdateVelocityAdvection()
{
    for (int j = 1; j < mJMax; ++j)
    {
        for (int i = 1; i < mIMax; ++i)
...
template <typename Real>
void Physics::Fluid2Da<Real>::GetLerpInfo(int i, int j, int& i0, int& i1, Real& a0, Real& a1, int& j0, int& j1, Real& b0, Real& b1)
{
...
template <typename Real>
void Physics::Fluid2Da<Real>::AdjustVelocity()
{
    int i = 0, j = 0;
...
template <typename Real>
void Physics::Fluid2Da<Real>::DirichletBoundaryZero(Real** data) noexcept
{
    // x-edge-interior
    for (int j = 1; j < mJMax; ++j)
    {
...
template <typename Real>
void Physics::Fluid2Da<Real>::UpdateDensityBoundary() noexcept
{
...
template <typename Real>
void Physics::Fluid2Da<Real>::UpdateVelocityBoundary()
{
...
template <typename Real>
Mathematics::Vector2<Real>** Physics::Fluid2Da<Real>::GetVelocity() const noexcept
{
    return mVelocity0;
}
