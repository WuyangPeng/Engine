...
Framework::WindowApplicationInformationTesting
	::WindowApplicationInformationTesting(const OStreamShared& stream, HInstance instance)
	...
	::InformationTest()
{
	...
	ASSERT_APPROXIMATE(information.GetAspectRatio(), boost::numeric_cast<float>(width) / boost::numeric_cast<float>(height), Mathematics::FloatMath::sm_Epsilon);
	ASSERT_ENUM_EQUAL(information.GetStyle(), System::WindowStyles::Default);
	...
	::NotAllowedResizeTest()
{
	...
	ASSERT_EQUAL(information.GetWindowSize(), size);
	ASSERT_APPROXIMATE(information.GetAspectRatio(), boost::numeric_cast<float>(width) / boost::numeric_cast<float>(height), Mathematics::FloatMath::sm_Epsilon);
	ASSERT_ENUM_EQUAL(information.GetStyle(), System::WindowStyles::Overlapped | System::WindowStyles::Caption | System::WindowStyles::SysMenu | System::WindowStyles::MinimizeBox | System::WindowStyles::Visible);
	ASSERT_EQUAL(information.GetWindowName().GetWindowClassName(), windowName);
	ASSERT_TRUE(information.GetWindowName().GetWindowMenuName().empty());
	...
	::RendererParameterTest() 
{
	...
	ASSERT_EQUAL(information.GetWindowPictorial().GetHBrush(), System::GetSystemStockObject(System::WindowBrushTypes::BlackBrush));
