...
（14）字节流宏：供流系统使用的宏。
（15）模板工具宏：对模板工具所需要的定义进行简化所使用的宏。
（16）单元测试套件宏：单元测试框架所需要的宏。
（17）子类智能指针宏：子类智能指针系统使用的宏。
（18）异常宏：抛出和捕获异常所需要的宏。
（19）属性宏：属性系统所需要的宏。
（20）用户宏：用于是否使用静态库、断言和类不变式等用户设定。

2.  ClassInvariant（类不变式）

封装了类不变式的实现。所有类在OPEN_CLASS_INVARIANT开启时都会使用它，所以它是需要高性能的类。
类使用枚举，而不是继承来区分仅断言后置条件的类不变式（const成员调用）和完全的类不变式（非const成员调用）。
仅断言前置条件的类不变式，目前没有使用，仅为对称而保留。
由于FunctionDescribed保存的是__func__和__FILE__，故使用const char*而不是string来传递参数。 
目前包含：
（1） 函数描述：封装了函数名字、文件名字和行数。
...
封装了断言的实现，在开启OPEN_ASSERT时才会使用断言。断言类使用可变数目的参数。
目前仅在MSVC编译器中实现，其他编译器待实现。
目前包含：
（1）断言：断言工具类。

4.  ImplHelper（Impl帮助）

封装了Impl帮助类的实现，这个类通过静态断言使Impl类正确使用复制构造函数、延迟复制和非常量成员函数。
目前包含：
（1）Impl特征类：Impl特征类帮助静态断言实现导出类。

5.  VersionInformation（版本信息）

封装了版本信息帮助类的实现。这个类通过静态成员函数获取主版本号、次版本号、内部版本号和修订号。
目前包含：
（1）版本信息：获取主版本号、次版本号、内部版本号和修订号。

6.	Base（基本功能）

封装了单例基类的实现，这个基类需要手动控制创建和销毁。这个实现用于控制单例的初始化和析构的顺序，以避免由于类初始化或析构顺序不同而导致的错误。
封装了UniqueIDManager，这个类生成需要的唯一ID。
目前包含：
...
文本解析类，对配置进行读取。
待实现功能：
（1）boost文本解析类：对boost::property_tree进行封装。 
（2）二维表解析类：对二维表解析进行封装。 
...
（3）最小堆：封装了最小堆的实现。
待实现功能：
（1）无锁数据结构：封装了一个无锁数据结构的实现。

16.	MemoryTools（内存工具）

封装了内存管理、智能指针和内存池、MD5和随机数的实现。由于引擎使用std::shared_ptr，仅在必须时使用这里的智能指针。
（如继承于ObjectInterface的类，需要派生类指针转换成基类指针的情况下。）由于内存管理在多处地方使用，它们需要高性能的实现。
目前包含：
...
（3）定制时间器：另一个计时器的实现。
待实现功能：
（1）时间信息：实现通过时间戳获取相关时间的信息，包含年、月、日，当天零点时间戳，本周日零点时间戳等相关信息。
...
封装了CoreTools库所需要的主函数帮助类，包括main和DllMain。main主函数默认初始化三个单例：日志、内存管理、智能指针。
目前包含：
...
封装了句柄管理类。资源句柄使用索引和魔术数的组合，句柄管理器对资源类使用句柄进行管理。
目前包含：
（1）句柄类型管理：封装了句柄管理类。
（2）资源句柄：使用索引和魔术数的组合封装资源句柄。

25.	ResourceManager（资源管理）

封装了资源管理类。根据优先级，对资源进行获取和销毁。对正在使用的资源进行锁定，以防止被管理器销毁。
目前包含：
（1）资源：封装了基础资源类。
（2）资源管理：根据优先级，对资源进行获取和销毁。
（3）资源工厂：封装资源的生成。
（4）资源锁：对正在使用的资源进行锁定，以防止被管理器销毁。

26.	FrameMemorySystem（帧内存系统）

封装了帧内存系统。帧内存系统对高位和低位的内存进行后创建先销毁的内存管理模式，当内存不足时抛出异常。
目前包含：
（1）帧内存共享指针：帧内存系统所使用的指针。
（2）基础帧内存系统：帧内存系统的内存分配。
（3）帧内存系统管理：帧内存系统的内存管理。

27.	BitArray（位数组）

封装了位数组。封装了一维、二维的位数组按位的基本操作。
目前包含：
（1）一维的位数组：封装了一维的位数组按位的基本操作。
（2）二维的位数组：封装了二维的位数组按位的基本操作。
（3）一维的两位位数组：封装了一维的两位位数组按位的基本操作。
