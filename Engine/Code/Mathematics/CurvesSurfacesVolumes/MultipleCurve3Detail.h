...
// ◊˜’ﬂ£∫≈ÌŒ‰—Ù£¨≈ÌÍ ∂˜£¨≈ÌÍ ‘Û
// 
// “˝«Ê∞Ê±æ£∫0.0.0.2 (2019/07/17 18:56)
...
{
	auto multi = *(MultipleCurve3**)data;
...
void MultipleCurve3<Real>::InitializeLength () const
{
    ...
    {
        mLengths[key] = GetLengthKey(key, Math<Real>::GetValue(0),
...
template <typename Real>
Real MultipleCurve3<Real>::GetTime (Real length, int iterations,Real tolerance) const
{
    if (!mLengths)
    {
        InitializeLength();
