...
Mathematics::DistanceRectangle3Rectangle3<Real>
	::DistanceRectangle3Rectangle3(const Rectangle3& rkRectangle0, const Rectangle3& rkRectangle1)
	:ParentType{}, mRectangle0{ rkRectangle0 }, mRectangle1{ rkRectangle1 }
{
}

template <typename Real>
const Mathematics::Rectangle3<Real>& Mathematics::DistanceRectangle3Rectangle3<Real>
	::GetRectangle0() const
{
	return mRectangle0;
}
...
template <typename Real>
typename const Mathematics::DistanceRectangle3Rectangle3<Real>::DistanceResult Mathematics::DistanceRectangle3Rectangle3<Real>
	::GetSquared() const
{
	...
	for (auto i1 = 0; i1 < 2; ++i1)
	{
		for (auto i0 = -1; i0 <= 1; i0 += 2)
		{
			...
			DistanceSegment3Rectangle3<Real> querySR{ edge, mRectangle1 };
	...
	for (auto i1 = 0; i1 < 2; ++i1)
	{
		for (auto i0 = -1; i0 <= 1; i0 += 2)
		{
			...
			DistanceSegment3Rectangle3<Real> querySR{ edge, mRectangle0 };
			auto sqrDistTmp = querySR.GetSquared();
			if (sqrDistTmp.GetDistance() < sqrDist)
			{
				mClosestPoint0 = sqrDistTmp.GetLhsClosestPoint();
	...
	::GetSquared(Real t, const Vector3D& lhsVelocity, const Vector3D& rhsVelocity) const
{
	...
	return DistanceRectangle3Rectangle3<Real>{ movedRect0, movedRect1 }.GetSquared();
}
