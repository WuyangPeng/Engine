...
// 
// ÒýÇæ°æ±¾£º0.0.0.3 (2019/07/29 11:24)
...
void RendererData::DrawCharacter (const BitmapFont& font, char c)
{
    ...
    glBitmap(bfc->mXSize, bfc->mYSize, (float)bfc->mXOrigin,(float)bfc->mYOrigin, (float)bfc->mXSize, 0.0f,(const GLubyte*)bfc->mBitmap);
				  ...
				  const OffsetState* ostate, const StencilState* sstate,const WireState* wstate)
{
    ...
    mFillEnabled ? glEnable(GL_POLYGON_OFFSET_FILL) : glDisable(GL_POLYGON_OFFSET_FILL);
    mLineEnabled ? glEnable(GL_POLYGON_OFFSET_LINE) :  glDisable(GL_POLYGON_OFFSET_LINE);
    mPointEnabled ? glEnable(GL_POLYGON_OFFSET_POINT) : glDisable(GL_POLYGON_OFFSET_POINT);
