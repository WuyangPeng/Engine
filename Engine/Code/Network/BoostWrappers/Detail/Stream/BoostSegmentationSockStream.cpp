...
Network::BoostSegmentationSockStream
...
int Network::BoostSegmentationSockStream
...
int Network::BoostSegmentationSockStream
	::ReceiveHead(const MessageBufferSharedPtr& messageBuffer)
{
...
int Network::BoostSegmentationSockStream
	::ReceiveContent(const MessageBufferSharedPtr& messageBuffer)
{
	...
	if (0 < receiveSize)
	{
		...
		{
			THROW_EXCEPTION(SYSTEM_TEXT("接收消息内容错误！"));
		}
	}
...
void Network::BoostSegmentationSockStream
...
void Network::BoostSegmentationSockStream
	::AsyncReceiveHead(const EventInterfaceSharedPtr& eventInterface, const MessageBufferSharedPtr& messageBuffer)
{
	messageBuffer->ClearCurrentIndex();
	messageBuffer->CheckingMessageHeadSize();
	messageBuffer->SetMessageHeadReceiveCount();
...
void Network::BoostSegmentationSockStream
	::AsyncReceiveContent(const EventInterfaceSharedPtr& eventInterface, const MessageBufferSharedPtr& messageBuffer, int originalWriteIndex)
{
	messageBuffer->CheckingMessageContentSize();
	messageBuffer->SetMessageContentReceiveCount();
...
void Network::BoostSegmentationSockStream
	::SubclassAsyncReceiveHeadEvent(const ErrorCodeType& errorCode, const EventInterfaceSharedPtr& eventInterface, const MessageBufferSharedPtr& messageBuffer, int originalWriteIndex)
{
	...
	{
		THROW_EXCEPTION(SYSTEM_TEXT("接收消息头长度错误！"));
	}
}

void Network::BoostSegmentationSockStream
	::SubclassAsyncReceiveContentEvent(const ErrorCodeType& errorCode, const EventInterfaceSharedPtr& eventInterface, const MessageBufferSharedPtr& messageBuffer, int originalWriteIndex)
{
	m_StreamReceive = StreamReceive::Head;
