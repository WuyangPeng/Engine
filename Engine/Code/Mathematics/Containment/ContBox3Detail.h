...
Mathematics::ContBox3<Real>::ContBox3()
{
...
typename const Mathematics::ContBox3<Real>::Box3 Mathematics::ContBox3<Real>::ContOrientedBox(const Points& points)
{
    ...
    for (const auto& point : points)
    {
        ...
        auto firstDot = Vector3DTools<Real>::DotProduct(diff, box.GetAxis0());
        firstDotCollection.push_back(firstDot);

        auto secondDot = Vector3DTools<Real>::DotProduct(diff, box.GetAxis1());
        secondDotCollection.push_back(secondDot);

        auto thirdDot = Vector3DTools<Real>::DotProduct(diff, box.GetAxis2());
...
template <typename Real>
bool Mathematics::ContBox3<Real>::InBox(const Vector3D& point, const Box3& box)
{
    auto diff = point - box.GetCenter();
    ...
    auto thirdCoeff = Vector3DTools<Real>::DotProduct(diff, box.GetAxis2());
...
typename const Mathematics::ContBox3<Real>::Box3 Mathematics::ContBox3<Real>::MergeBoxes(const Box3& lhs, const Box3& rhs)
{
    ...
    std::vector<Vector3D> lhsRotationColumn{ lhs.GetAxis0(), lhs.GetAxis1(), lhs.GetAxis2() };

    std::vector<Vector3D> rhsRotationColumn{ rhs.GetAxis0(), rhs.GetAxis1(), rhs.GetAxis2() };
