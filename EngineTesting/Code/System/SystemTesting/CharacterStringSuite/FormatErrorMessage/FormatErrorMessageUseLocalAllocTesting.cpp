...
{
    ASSERT_NOT_THROW_EXCEPTION_0(FormatLastErrorUseLocalAllocTest);
}

void System::FormatErrorMessageUseLocalAllocTesting::FormatLastErrorUseLocalAllocTest()
{
    for (auto flag = WindowError::Success; flag <= WindowError::AppHang; ++flag)
    {
        ...
        {
            ASSERT_TRUE(FormatErrorMessage(errorMessage, flag));
            ASSERT_UNEQUAL_NULL_PTR(errorMessage);
        }
        else
        {
            if (FormatErrorMessage(errorMessage, flag))
            {
                ASSERT_UNEQUAL_NULL_PTR(errorMessage);
            }
            else
