...
Network::ReactiveServerTesting
	::ReactiveServerTesting(const OStreamShared& osPtr)
...
void Network::ReactiveServerTesting
	::MainTest()
{
...
void Network::ReactiveServerTesting
	::CreateMessage()
{
	MESSAGE_MANAGER_SINGLETON.Insert(m_ClientSendMessageID, MessageTypeCondition{}, NullMessage::Factory);
}
	...
	::ReactiveServerTest()
{
	...
	ConfigurationStrategy serverConfigurationStrategy{ WrappersStrategy::ACE, ConnectStrategy::TCP, ServerStrategy::Reactive,
													   ...
													   subStrategy, ConfigurationParameter{}, SocketSendMessage::Default, "127.0.0.1", m_Port + m_Increase };
	...
	auto loopCount = GetTestLoopCount();
	...
	{
		threadGroup.create_thread(boost::bind(&ClassType::ClientThread, this, client));
	}

	auto runCount = loopCount * 10;
	for (int i = 0; i < runCount; ++i)
	{
		ASSERT_TRUE(server->RunServer());

		auto callBackTime = loopCount * m_ClientSendMessageID;
...
void Network::ReactiveServerTesting
	::ClientThread(ClientSharedPtr client)
{
	TestSocketManagerSharedPtr testSocketManager{ make_shared<TestSocketManager>(m_ClientSendMessageID) };

	auto connectTime = 100;
	uint64_t socketID{ 0 };
