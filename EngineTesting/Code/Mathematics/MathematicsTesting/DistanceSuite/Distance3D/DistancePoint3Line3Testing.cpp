	...
	::BaseTest()
{
	...
	for (auto loop = 0; loop < testLoopCount; ++loop)
	{
		...
		DistancePoint3Line3<float> distance(point, FloatLine3(rhsOrigin, rhsDirection));
		...
		ASSERT_APPROXIMATE_USE_FUNCTION(Approximate<float>, distance.GetLine(), FloatLine3(rhsOrigin, rhsDirection), 1e-8f);
	}
}
	...
	::StaticTest()
{
	...
	for (auto loop = 0; loop < testLoopCount; ++loop)
	{
		...
		DoubleDistancePoint3Line3 distance(point, DoubleLine3(rhsOrigin, rhsDirection));
