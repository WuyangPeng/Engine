...
// ◊˜’ﬂ£∫≈ÌŒ‰—Ù£¨≈ÌÍ ∂˜£¨≈ÌÍ ‘Û
// 
// “˝«Ê∞Ê±æ£∫0.0.0.2 (2019/07/12 19:37)
	...
	::Find()
{
    ...
    Real coeff[6];
	coeff[0] = Vector2DTools::DotProduct( M1C0mC1,C0mC1) - (Real)1;
	coeff[1] = ((Real)2)*mEllipse0.GetExtent0()*Vector2DTools::DotProduct(M1A0,C0mC1);
	coeff[2] = ((Real)2)*mEllipse0.GetExtent1()*Vector2DTools::DotProduct(M1A1,C0mC1);
	coeff[3] = mEllipse0.GetExtent0()*mEllipse0.GetExtent0() * Vector2DTools::DotProduct(M1A0,mEllipse0.GetAxis0());
	coeff[4] = ((Real)2)*mEllipse0.GetExtent0()*mEllipse0.GetExtent1() * Vector2DTools::DotProduct(M1A0,mEllipse0.GetAxis1());
...
::GetClassification() const
{
	...
	Matrix2<Real> D0{ ((Real)1) / (mEllipse0.GetExtent0()*mEllipse0.GetExtent0()),
					  ((Real)1) / (mEllipse0.GetExtent1()*mEllipse0.GetExtent1()) };
	...
	Matrix2<Real> D1{ ((Real)1) / (mEllipse1.GetExtent0()*mEllipse1.GetExtent0()),
					  ((Real)1) / (mEllipse1.GetExtent1() * mEllipse1.GetExtent1()) };

	// Compute K2.
	Matrix2<Real> D0NegHalf{ mEllipse0.GetExtent0(),mEllipse0.GetExtent1() };
	Matrix2<Real> D0Half{ ((Real)1) / mEllipse0.GetExtent0(),((Real)1) / mEllipse0.GetExtent1()};
    ...
    Matrix2<Real> R, D;
	auto matrix2EigenDecomposition =
    M2.EigenDecomposition();
	...
	::RefinePoint(const Real coeff[6], Vector2D& point, Real& q0, Real& q1, Real& angle0)
{
	...
	auto df0 = -coeff[1]*sn + coeff[2]*cs +   ((Real)2)*(coeff[5] - coeff[3])*cs*sn + coeff[4]*(cs*cs - sn*sn);
    ...
    for (i = 0; i < maxIterations; ++i)
    {
        ...
        df1 = -coeff[1]*sn + coeff[2]*cs + ((Real)2)*(coeff[5] - coeff[3])*cs*sn +  coeff[4]*(cs*cs - sn*sn);
    ...
    if (i < maxIterations)
    {
		...
		auto tmp1 = ((Real)DIGITS_ACCURACY)*Math::Log((Real)10);
		auto arg = (tmp0 + tmp1)/Math::Log((Real)2);
