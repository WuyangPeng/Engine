...
// ◊˜’ﬂ£∫≈ÌŒ‰—Ù£¨≈ÌÍ ∂˜£¨≈ÌÍ ‘Û
// 
// “˝«Ê∞Ê±æ£∫0.0.0.2 (2019/07/13 12:52)
	...
	::FindJustEdgeIntersection (Real cy, Real ex,Real ey, Real ez, Real dx, Real dz, Real vx, Real vy, Real vz,Real& ix, Real& iy, Real& iz)
{
    ...
    if (crossZ >= Real{} && crossX >= Real{} &&  crossX*crossX + crossZ*crossZ >  vy*vy*mSphere.GetRadius()*mSphere.GetRadius())
    {
	...
	::FindVertexRegionIntersection (Real ex, Real ey,Real ez, Real cx, Real cy, Real cz, Real vx, Real vy, Real vz,Real& ix, Real& iy, Real& iz)
{
    ...
    if ((crossY <  Real{} && crossZ >= Real{} && crY2 + crZ2 <= rsqr*vx2) ||  (crossZ <  Real{} && crossX <  Real{} && crX2 + crZ2 <= rsqr*vy2) ||  (crossY >= Real{} && crossX >= Real{} && crX2 + crY2 <= rsqr*vz2))
    {
