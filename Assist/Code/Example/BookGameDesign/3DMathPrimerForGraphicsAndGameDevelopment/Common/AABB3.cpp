...
{
	Vector3	AABB3::corner(int i) const noexcept
	{
	...
	void AABB3::add(const Vector3& p) noexcept
	{
	...
	void AABB3::add(const AABB3& box) noexcept
	{
	...
	void AABB3::setToTransformedBox(const AABB3& box, const Matrix4x3& m) noexcept
	{
	...
	bool AABB3::isEmpty() const noexcept
	{
	...
	bool AABB3::contains(const Vector3& p) const noexcept
	{
	...
	Vector3	AABB3::closestPointTo(const Vector3& p) const noexcept
	{
	...
	bool AABB3::intersectsSphere(const Vector3& center, float radius) const noexcept
	{

		// Find the closest point on box to the point

		const Vector3 closestPoint = closestPointTo(center);
	...
	float AABB3::rayIntersect(const Vector3& rayOrg, // orgin of the ray
	...
	) const noexcept
	{
		...
		constexpr float kNoIntersection = 1e30f;
		...
		{
			const float y = rayOrg.y + rayDelta.y*t;
			if (y < min.y || y > max.y) return kNoIntersection;
			const float z = rayOrg.z + rayDelta.z*t;
		...
		{
			const float x = rayOrg.x + rayDelta.x*t;
	...
	int	AABB3::classifyPlane(const Vector3& n, float d) const noexcept
	{
	...
	float AABB3::intersectPlane(const Vector3& n, float planeD, const Vector3& dir) const noexcept
	{
	...
	bool intersectAABBs(const AABB3& box1, const AABB3& box2, AABB3* boxIntersect) noexcept
	{
	...
	float intersectMovingAABB(const AABB3& stationaryBox, const AABB3& movingBox, const Vector3& d) noexcept
	{
