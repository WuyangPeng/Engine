...
Rendering::Triangles
	::Triangles(VisualPrimitiveType type) 
	:ParentType{ type }
{
...
const Rendering::TrianglePosition Rendering::Triangles
	::GetModelTriangle(int index) const
{
	RENDERING_CLASS_IS_VALID_CONST_1;

	auto triangleIndex = GetTriangle(index);
	auto firstIndex = triangleIndex.GetFirstIndex();
	auto secondIndex = triangleIndex.GetSecondIndex();
	auto thirdIndex = triangleIndex.GetThirdIndex();
...
const Rendering::TrianglePosition Rendering::Triangles
	::GetWorldTriangle(int index) const 
{
	RENDERING_CLASS_IS_VALID_CONST_1;
...
const Rendering::Triangles::Vector3D Rendering::Triangles
	::GetPosition(int vertexIndex) const 
{
	RENDERING_CLASS_IS_VALID_CONST_1;
	...
	{
		THROW_EXCEPTION(SYSTEM_TEXT("GetPosition 错误！"));
	}	
}

void Rendering::Triangles
...
// private
void Rendering::Triangles
	::UpdateModelNormals(const VertexBufferAccessor& vertexBufferAccessor) 
{ 
	// 从包含顶点的平面加权平均计算法线。
...
// private
void Rendering::Triangles
	::UpdateModelTangentsUseGeometry(const VertexBufferAccessor& vertexBufferAccessor)
{      
	// 计算矩阵的法线的导数。
	...
	auto numVertices = vertexBufferAccessor.GetNumVertices();
...
// private
void Rendering::Triangles
	::UpdateModelTangentsUseTextureCoords(const VertexBufferAccessor& vertexBufferAccessor) 
{ 
	// 每个顶点可以多次访问,所以只在第一次访问时计算切线空间。
	// 使用零向量作为标志不计算切向量。
	auto hasTangent = vertexBufferAccessor.HasTangent();
    ...
    {  
        ...
        {
			auto currentTriangleIndex = triangleIndex[current];
		...
		for (auto current = 0; current < 3; ++current)
        {
			...
			auto prev = ((current + 2) % 3);
			auto next = ((current + 1) % 3);
...
// private
const Rendering::Triangles::AVector Rendering::Triangles
	::ComputeTangent(const APoint& position0, const Vector2D& textureCoord0,const APoint& position1, const Vector2D& textureCoord1,const APoint& position2, const Vector2D& textureCoord2)
{ 
	// 计算顶点P0位置的变化。
...
const Rendering::PickRecordContainer Rendering::Triangles
	::ExecuteRecursive(const APoint& origin, const AVector& direction,float tMin, float tMax) const 
{
	...
	if (GetWorldBound().TestIntersection(origin, direction, tMin, tMax))
	{
		...
		auto modelOrigin = modelOriginPoint.GetVector3D();

		auto modelDirectionVector = worldInverseTransform * direction;
		auto modelDirection = modelDirectionVector.GetVector3D();

		Line3 line{ modelOrigin, modelDirection };
		...
		for (auto i = 0; i < numTriangles; ++i)
		{
			try
			{
				auto triangleIndex = GetTriangle(i);
				...
				Triangle3 triangle{ vertex0, vertex1, vertex2 };
