...
// ◊˜’ﬂ£∫≈ÌŒ‰—Ù£¨≈ÌÍ ∂˜£¨≈ÌÍ ‘Û
// 
// “˝«Ê∞Ê±æ£∫0.0.0.2 (2019/07/15 09:35)
	...
	::Find(const Vector3D& origin,const Vector3D& dir,const Capsule3& capsule, Real t[2])
{
    ...
    if (Math::FAbs(dz) >= (Real)1 - Math::sm_ZeroTolerance)
    {
	...
	a1 -= ((Real)2)*extent*D.GetZCoordinate();
	a0 -= ((Real)4)*extent*P.GetZCoordinate();
    discr = a1*a1 - a0;
    if (discr > Math::sm_ZeroTolerance)
    {
        root = Math::Sqrt(discr);
