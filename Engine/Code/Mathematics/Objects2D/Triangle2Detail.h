...
Mathematics::Triangle2<Real>::Triangle2(const Vector2Type& vector0, const Vector2Type& vector1, const Vector2Type& vector2) noexcept
...
template <typename Real>
Real Mathematics::Triangle2<Real>::DistanceTo(const Vector2Type& point) const
{
    MATHEMATICS_CLASS_IS_VALID_CONST_9;
...
#include SYSTEM_WARNING_POP
    ...
    const auto a00 = Vector2ToolsType::GetLengthSquared(edge0);
    const auto a01 = Vector2ToolsType::DotProduct(edge0, edge1);
    const auto a11 = Vector2ToolsType::GetLengthSquared(edge1);
    const auto b0 = Vector2ToolsType::DotProduct(diff, edge0);
    const auto b1 = Vector2ToolsType::DotProduct(diff, edge1);
    const auto c = Vector2ToolsType::GetLengthSquared(diff);
    ...
    if (s + t <= det)
    {
        ...
        {
            // 最小是在三角形的内部点。
            const auto invDet = MathType::GetValue(1) / det;
            s *= invDet;
            t *= invDet;
            sqrDistance = s * (a00 * s + a01 * t + MathType::GetValue(2) * b0) + t * (a01 * s + a11 * t + MathType::GetValue(2) * b1) + c;
        }
    }
    else
    {
