...
对动态链接库模块进行测试：
（1）加载资源工具：FindResourceWindowsTesting、
                   LoadResourceWindowsTesting、
                   LoadStringWindowsTesting。
（2）枚举资源工具：EnumResourceToolsTesting、
                   ResourceEnumTesting。
...
对Windows模块进行测试：
（1） Windows创建：CreateSystemWindowTesting、
                   GetHInstanceTesting、
...
（5） Window系统：GetActiveWindowTesting、
                  GetSystemClientRectTesting。
（6） Windows用户：WindowsUserTesting。
